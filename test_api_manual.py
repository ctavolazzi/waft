...
def test_api():
    ...
    try:
        ...
        print(
            "🔗 Test 8: Link Work Effort to Project (POST /api/projects/{project_id}/work-efforts/{we_id})"
        )
        response = client.post(f"/api/projects/{project_id}/work-efforts/{we_id}", headers=headers)
        assert response.status_code == 204, f"Expected 204, got {response.status_code}"
        print("   ✅ Work effort linked to project\n")

        # Test 9: Verify link
        print("🔍 Test 9: Verify Work Effort Link")
        response = client.get(f"/api/projects/{project_id}")
        project = response.json()
        assert we_id in project["related_work_efforts"], (
            "Work effort should be in related_work_efforts"
        )
        print(f"   ✅ Work effort {we_id} is linked to project\n")

        # Test 10: Unlink Work Effort
        print(
            "🔓 Test 10: Unlink Work Effort (DELETE /api/projects/{project_id}/work-efforts/{we_id})"
        )
        response = client.delete(
            f"/api/projects/{project_id}/work-efforts/{we_id}", headers=headers
        )
        assert response.status_code == 204, f"Expected 204, got {response.status_code}"
        print("   ✅ Work effort unlinked from project\n")

        # Test 11: Delete Work Effort
        print("🗑️  Test 11: Delete Work Effort (DELETE /api/work-efforts/{we_id})")
        response = client.delete(f"/api/work-efforts/{we_id}", headers=headers)
        assert response.status_code == 204, f"Expected 204, got {response.status_code}"
        print("   ✅ Work effort deleted\n")
        ...
        response = client.post("/api/projects", json={"title": "Should Fail"})
        ...
        print("\n📊 Test Summary:")
        print("   - Projects CRUD: ✅")
        print("   - Work Efforts CRUD: ✅")
        print("   - Authentication: ✅")
        print("   - Error Handling: ✅")
        print("   - Work Effort Linking: ✅")
