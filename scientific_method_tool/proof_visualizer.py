...
# Page config
st.set_page_config(
    page_title="Scientific Method Tool - Proof Visualizer",
    page_icon="🔬",
    layout="wide"
)
...
st.sidebar.metric("Average Confidence", 
...
# Tabs
tab1, tab2, tab3, tab4 = st.tabs(["📈 Overview", "🔬 Experiments", "📊 Data Comparison", "📁 Files"])
...
with tab2:
    ...
    selected_proof = st.selectbox(
        "Select Proof to View",
        range(1, len(proofs) + 1),
        format_func=lambda x: f"Proof #{x}"
    )
    ...
    with col1:
        ...
        st.write(f"**State Changed:** {'✅ Yes' if proof['initial_state_hash'] != proof['final_state_hash'] else '❌ No'}")
