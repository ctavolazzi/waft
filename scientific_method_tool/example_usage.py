...
def run_tavern_experiment(experiment):
    ...
    prediction_match = results.get("fitness_gained", 0.0) > 15.0  # Hypothesis: higher skill = better results
...
def main():
    ...
    console.print("[bold bright_blue]║[/bold bright_blue]  [bold white]SCIENTIFIC METHOD TOOL DEMO[/bold white]  [bold bright_blue]║[/bold bright_blue]")
    console.print("[bold bright_blue]║[/bold bright_blue]  [dim]Hypothesis Testing with Iterative Experiments[/dim]  [bold bright_blue]║[/bold bright_blue]")
    ...
    for result in results:
        table.add_row(
            ...
            "✓" if result.hypothesis_verified else "✗" if result.hypothesis_verified is False else "?",
