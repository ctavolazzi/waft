...
def parse_work_effort_metadata(we_dir: Path) -> dict[str, Any] | None:
...
def get_chronicler_observations(
    project_path: Path, start: datetime, end: datetime
) -> dict[str, Any]:
...
def main():
    ...
    time_range_str = (
        f"{start_time.strftime('%B %d, %Y %H:%M')} to {end_time.strftime('%B %d, %Y %H:%M')}"
    )
    ...
    with col_left:
        ...
        if observations["total"] > 0:
            ...
            for observer, obs_list in observations["by_observer"].items():
                ...
                st.markdown(
                    f"**{observer.title()}**: {genesis_count} genesis, {exodus_count} exodus, {mutation_count} mutations"
                )
            ...
            if observations["genesis"]:
                with st.expander(
                    f"Recent Creations ({len(observations['genesis'])} total)", expanded=False
                ):
                    for obs in observations["genesis"][:20]:
                        ...
                        try:
                            ...
                            time_str = ""
                        st.markdown(
                            f"<span class='genesis-badge'>GENESIS</span> <small>{time_str}</small> {path}",
                            unsafe_allow_html=True,
                        )
            ...
            if observations["exodus"]:
                with st.expander(
                    f"Recent Deletions ({len(observations['exodus'])} total)", expanded=False
                ):
                    for obs in observations["exodus"][:20]:
                        ...
                        try:
                            ...
                            time_str = ""
                        st.markdown(
                            f"<span class='exodus-badge'>EXODUS</span> <small>{time_str}</small> {path}",
                            unsafe_allow_html=True,
                        )
        else:
            ...
            if st.button("📖 Learn about TheChronicler"):
                st.info(
                    "Run: `waft chronicler` or `/chronicle start` to begin monitoring system activity."
                )
        ...
        if work_efforts["active"] > 0:
            ...
            active_we = [
                we for we in work_efforts["recent"] if we.get("status", "").lower() == "active"
            ]
            if active_we:
                ...
                for we in active_we[:5]:
                    ...
                    status_class = (
                        "status-active" if status.lower() == "active" else "status-completed"
                    )
                    st.markdown(
                        f"""
    ...
    with col_right:
        ...
        for system, info in system_health.items():
            ...
            if system == "chronicler" and available:
                ...
                st.markdown(
                    f"{status_icon} **{system.title()}**: {running_icon} {'Running' if running else 'Available'}"
                )
                if stats:
                    st.caption(
                        f"Today: {stats.get('genesis_count', 0)} genesis, {stats.get('exodus_count', 0)} exodus"
                    )
            elif system == "empirica" and available:
                initialized = info.get("initialized", False)
                init_icon = "✓" if initialized else "○"
                st.markdown(
                    f"{status_icon} **{system.title()}**: {init_icon} {'Initialized' if initialized else 'Available'}"
                )
                ...
                st.markdown(
                    f"{status_icon} **{system.title()}**: {'Available' if available else 'Inactive'}"
                )
    ...
    st.divider()
    st.markdown(
        f"<div style='text-align: center; color: #666;'>WAFT Ecosystem Entry Point | Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>",
        unsafe_allow_html=True,
    )
