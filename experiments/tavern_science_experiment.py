...
def run_tavern_scenario(character: DnD5eCharacter, being: Being) -> Dict[str, Any]:
    ...
    try:
        ...
        if success:
            session.add_event("skill_check", "Perception check succeeded", {
                "roll": roll,
                "modifier": perception_mod,
                "total": total,
                "dc": dc
            })
            session.add_event("discovery", "Noticed details about the tavern", {
                "observation": "The tavern is quiet, with only a few patrons"
            })
        else:
            session.add_event("skill_check", "Perception check failed", {
                "roll": roll,
                "modifier": perception_mod,
                "total": total,
                "dc": dc
        ...
        # Read note (choice y)
        session.add_event("action", "Read mysterious note", {
            "content": "Meet at the old mill at midnight"
        })
        
        # Final choice (choice 1)
        session.add_event("decision", "Decided to investigate the note", {
            "choice": "Investigate the old mill"
        })
...
def generate_comprehensive_report(
    ...
    total_data_points = sum(len(s.get_values()) for s in data_series_dict.values()) if isinstance(data_series_dict, dict) else 0
...
"""
    ...
    console.print(f"[yellow]→[/yellow] Generating ArXiv-style academic PDF...")
...
def main():
    ...
    try:
        ...
        console.print(f"   ✓ Conclusions: {analysis.conclusions[0] if analysis.conclusions else 'None'}\n")
        
        # Step 8: Generate comprehensive report
        console.print("[bold]Step 8: Generate Comprehensive PDF Report[/bold]\n")
        pdf_path = generate_comprehensive_report(
            hypothesis, experiment, results, analysis, manager,
            initial_state, experiment.final_state
        )
        ...
        if pdf_path and pdf_path.exists():
            ...
            if platform.system() == "Darwin":  # macOS
                ...
                console.print("[dim]💡 To print, use: lpr {pdf_path}[/dim]".format(pdf_path=pdf_path))
            else:
