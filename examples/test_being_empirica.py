...
async def test_being_empirica():
    ...
    console.print("[bold bright_blue]║[/bold bright_blue]  [bold white]BEING EMPIRICA INTEGRATION TEST[/bold white]  [bold bright_blue]║[/bold bright_blue]")
    ...
    if being.empirica_manager and being.empirica_session_id:
        console.print(f"[green]✓[/green] Empirica enabled!")
    ...
    console.print(f"[dim]  Decision Fatigue: {being.decision_fatigue}/{being.decision_quota_max}[/dim]\n")
    ...
    for i in range(5):
        try:
            ...
            # Display result
            gate_display = f" [bold magenta]Gate: {gate_result}[/bold magenta]" if gate_result else ""
    ...
    if being.empirica_manager:
        ...
        console.print(f"  Preflight/Postflight assessments: ✅")
        console.print(f"  Check gates: ✅")
        console.print(f"  Finding/Unknown logging: ✅")
    else:
        console.print(f"\n[yellow]⚠ Empirica not enabled[/yellow]")
        console.print(f"  Check that Empirica is initialized in the project")
