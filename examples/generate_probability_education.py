...
def generate_probability_lesson(
    concept: str, examples: list[dict[str, Any]], output_path: Path
) -> Path:
    ...
    if concept == "Hand Probabilities":
        ...
        content_lines.append(
            "Expected value: The average outcome if you repeat an action many times."
        )
        content_lines.append("")
        content_lines.append("### Example: Betting")
        content_lines.append("")
        content_lines.append(
            "If you bet $10 with a 10% chance to win $100, your expected value is:"
        )
    ...
    for example in examples:
        if "cards" in example:
            players.append(Player(name=example.get("name", "Example"), cards=example["cards"]))
...
def example_hand_probabilities():
    ...
    pdf_path = generate_probability_lesson(
        concept="Hand Probabilities", examples=examples, output_path=output_path
    )
