...
async def main():
    ...
    console.print(
        "[bold bright_blue]║[/bold bright_blue]  [bold white]HELPER BEINGS DEMO[/bold white]  [bold bright_blue]║[/bold bright_blue]"
    )
    console.print(
        "[bold bright_blue]╚════════════════════════════════════════╝[/bold bright_blue]\n"
    )
    ...
    if not beings:
        ...
        being = being_system.spawn_being(
            reality_id="demo_reality", initial_skills={"coding": 40.0, "analysis": 35.0}
        )
        beings.append(being)
    ...
    for being in beings:
        if being.skills:
            ...
            table.add_row(
                being.being_id[:28] + "...", f"{being.stamina:.1f}", f"{top[0]}: {top[1]:.1f}"
            )
    ...
    for cycle in range(1, 11):
        for being in beings:
            ...
            try:
                ...
                if decision == "learn_skill" and not result.get("experience", {}).get(
                    "stamina_depleted"
                ):
                    if being.skills:
