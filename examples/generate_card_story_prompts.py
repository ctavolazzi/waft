...
def generate_story_prompt_pdf(
    cards: List[str],
    output_path: Path,
    title: str = "Story Prompt from Cards"
) -> Path:
    ...
    if story.get("protagonist"):
        p = story["protagonist"]
        prompt_parts.append(f"a {p['trait'].lower()} character from the {p['domain'].lower()} domain")
