...
def extract_arxiv_metadata(md_content: str) -> Dict:
    ...
    if frontmatter:
        ...
        if "affiliations" in frontmatter:
            ...
            if isinstance(affil_value, str):
                metadata["affiliations"] = [a.strip() for a in re.split(r'[,\n]', affil_value) if a.strip()]
            ...
            if '**authors**' in line_lower or '**author**' in line_lower:
                ...
                if ':' in line:
                    ...
                    if authors_str:
                        ...
                        metadata["authors"] = [{"name": author} for author in authors_list if author]
                break
        ...
        for line in lines:
            ...
            if line_lower.startswith('## references') or line_lower.startswith('# references'):
                ...
                # Handle plain text references (if they look substantial)
                    ...
                    if any(indicator in line_stripped for indicator in [',', '.', '(', ')', 'et al', 'Journal', 'Proceedings']):
    ...
    if not metadata["abstract"]:
        ...
        collecting = False
        in_code_block = False
        skipped_sections = ['date', 'time', 'session', 'what we', 'key insights', 'next steps', 'created', 'modified']
        code_indicators = ['=', '(', ')', '[', ']', '{', '}', 'def ', 'class ', 'import ', 'from ', 'return ', 'print(']
        ...
        for line in lines:
            ...
            if (line_stripped and 
                ...
                not line_stripped.startswith('**') and
                not line_stripped.startswith('---') and
                not line_stripped.startswith('- ') and  # Skip list items
                not line_stripped.startswith('* ') and  # Skip list items
                not line_stripped.startswith('`') and  # Skip inline code markers
                not any(section in line_lower for section in skipped_sections) and
                ':' not in line_stripped and  # Skip metadata-like lines
                not any(indicator in line_stripped for indicator in code_indicators) and  # Skip code-like lines
                not line_stripped.startswith('```')):  # Skip code block markers
                ...
                is_likely_code = (
                    any(indicator in line_stripped for indicator in ['=', '(', ')', '[', ']', '{', '}', 'def ', 'class ', 'import ', 'from ', 'return ', 'print(', '->', '::'])
                )
                
                has_natural_language = (
                    any(word in line_lower for word in ['the', 'and', 'or', 'is', 'are', 'was', 'were', 'a', 'an', 'to', 'of', 'in', 'on', 'at', 'for', 'with', 'we', 'this', 'that', 'system', 'provides', 'features'])
                )
                
                if (len(line_stripped) > 80 and 
                    not line_stripped[0].isdigit() and  # Skip numbered items
                    not is_likely_code and  # Skip obvious code
                    (has_natural_language or len(line_stripped) > 120)):  # Either has natural language or is long enough
                    collecting = True
        ...
        if not metadata["abstract"]:
            for line in lines:
                ...
                if (line_stripped and 
                    ...
                    not line_stripped.startswith('- ') and
                    not line_stripped.startswith('* ') and
                    not '=' in line_stripped and  # Skip code-like
                    not line_stripped.startswith('`') and
                    ' ' in line_stripped):  # Has spaces (not all one word)
...
def process_markdown_content(md_content: str, metadata: Dict) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if (not skip_metadata_lines and 
            any(keyword in line_lower for keyword in ['**authors**', '**author**', '**email**', 
                                                      '**affiliation**', '**date**', '**version**',
                                                      '**category**', '**keywords**']) and
            ':' in line):
            ...
            if (line_stripped == '' or 
                ...
                if ':' in line and any(keyword in line_lower for keyword in ['**', 'date', 'time', 'session']):
                    continue
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate ArXiv-ready academic paper PDF from markdown")
    parser.add_argument(
        "input",
        nargs="?",
