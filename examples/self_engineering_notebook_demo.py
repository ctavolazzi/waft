...
def demo_notebook_system():
    ...
    console.print("[bold bright_blue]║[/bold bright_blue]  [bold white]SELF-ENGINEERING NOTEBOOK DEMO[/bold white]  [bold bright_blue]║[/bold bright_blue]")
    console.print("[bold bright_blue]║[/bold bright_blue]  [dim]Journal → Reflect → Create Actionables[/dim]  [bold bright_blue]║[/bold bright_blue]")
    ...
    if empirica.is_initialized():
        ...
        if epistemic_context:
            uncertainty = epistemic_context.get("epistemic_state", {}).get("vectors", {}).get("uncertainty", 0.5)
    ...
    if entries:
        ...
        console.print(f"[green]✓[/green] Work effort data prepared:")
        ...
        console.print(f"[dim]  mcp_work-efforts_create_work_effort(...)[/dim]")
    ...
    console.print(f"  Reflections Created: 1")
    console.print(f"  Actionable Suggestions: {len(reflection.actionable_suggestions)}")
    console.print(f"\n[dim]Notebook Location: {notebook_dir}[/dim]")
    console.print(f"[dim]Check _notebook/entries/ and _notebook/reflections/ for details[/dim]\n")
