...
def main():
    ...
    print("✓ Variant 1 (Compact)")
    print(f"  - ID: {variant_compact.genome_id[:16]}...")
    print(f"  - Name: {variant_compact.scientific_name}")
    print(
        f"  - Parent: {variant_compact.parent_id[:16] if variant_compact.parent_id else 'None'}..."
    )
    ...
    print("✓ Variant 2 (Spacious)")
    print(f"  - ID: {variant_spacious.genome_id[:16]}...")
    print(f"  - Name: {variant_spacious.scientific_name}")
    print(
        f"  - Parent: {variant_spacious.parent_id[:16] if variant_spacious.parent_id else 'None'}..."
    )
    ...
    print("✓ Variant 3 (Two-Column)")
    print(f"  - ID: {variant_two_column.genome_id[:16]}...")
    print(f"  - Name: {variant_two_column.scientific_name}")
    print(
        f"  - Parent: {variant_two_column.parent_id[:16] if variant_two_column.parent_id else 'None'}..."
    )
    ...
    if scint_1:
        ...
        print(
            f"  - Between: {variant_compact.scientific_name} ↔ {variant_spacious.scientific_name}"
        )
    ...
    if scint_2:
        ...
        print(
            f"  - Between: {variant_compact.scientific_name} ↔ {variant_two_column.scientific_name}"
        )
    ...
    if best_genome:
        ...
        for i, ancestor in enumerate(lineage):
            indent = "  " * i
            fitness_str = (
                f" (fitness: {ancestor.fitness_score:.3f})" if ancestor.fitness_score else ""
            )
