...
class TavernGameSession:
    ...
    def log_choice(self, choice_number: int, choice_text: str, decision: str):
        ...
        self.log_event("choice", f"Made choice: {choice_text}", {
            "choice_number": choice_number,
            "decision": decision
        })
    
    def log_roll(self, roll_type: str, die_roll: int, modifier: int, total: int, dc: int, success: bool):
        """Log a dice roll."""
        self.rolls.append({
            "roll_type": roll_type,
            "die_roll": die_roll,
        ...
        self.log_event("roll", f"{roll_type} check: {total} vs DC {dc} ({'Success' if success else 'Failure'})", {
            "die_roll": die_roll,
            "modifier": modifier,
            "total": total,
            "dc": dc,
    ...
    def finish(self):
        ...
        self.log_event("session_end", "Game session completed", {
...
def play_tavern_scenario_with_being(session: TavernGameSession, auto_choices: List[str] = None):
    ...
    session.log_event("game_start", "Tavern scenario begins", {
    ...
    if skill_type == "perception":
        ...
        if success:
            ...
            outcome = "You find loose coins and trinkets, but nothing that explains how you got here."
            found_note = False
        ...
        if success:
            ...
            outcome = "You remember bits and pieces: A tavern, a meeting, voices. But details are lost."
            found_note = False
        else:
    ...
    final_choice_map = {
        "1": ("Follow the note", "You decide to follow the note. As you leave the tavern, you feel eyes watching you. The old mill awaits. What secrets does it hold? Your adventure has just begun."),
        "2": ("Ask around town", "You decide to gather more information first. The townsfolk might know something about 'The Shadow' or the old mill. But time is running out... Your investigation begins."),
        "3": ("Ignore it", "You decide to leave town and forget this ever happened. But as you walk away, you can't shake the feeling that this isn't over. The Shadow will find you again. Or maybe... you'll find them first.")
    }
    ...
    session.log_event("game_end", "Tavern scenario completed", {
        "final_hp": character.hp,
        "final_ac": character.ac
    })
...
The Being demonstrated {'strategic' if len([c for c in session.choices if c['decision'] == '1']) > 0 else 'exploratory'} decision-making throughout the scenario. The Being made {len(session.choices)} major decisions, with {'a preference for' if len([c for c in session.choices if c['decision'] == '1']) > 0 else 'varied'} approaches.

### 4.2 Skill Check Performance

The Being performed {len(session.rolls)} skill checks, with {len([r for r in session.rolls if r['success']])} successes and {len([r for r in session.rolls if not r['success']])} failures. This represents a {len([r for r in session.rolls if r['success']]) / len(session.rolls) * 100:.1f}% success rate.

### 4.3 Narrative Impact

The Being's choices and skill check results directly influenced the narrative outcomes. {'The Being successfully discovered clues' if any('note' in str(o.get('data', {})).lower() for o in session.outcomes) else 'The Being encountered challenges'} throughout the scenario.
...
"""
    ...
    base_name = f"being_tavern_report_{session.being.being_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
{html_content}
        ...
        console.print("[yellow]⚠ WeasyPrint not available. PDF not generated. Markdown saved.[/yellow]")
        console.print(f"[dim]To generate PDF, install: pip install weasyprint[/dim]")
...
def main():
    ...
    console.print("[bold bright_blue]║[/bold bright_blue]  [bold white]BEING PLAYS TAVERN GAME[/bold white]  [bold bright_blue]║[/bold bright_blue]")
    console.print("[bold bright_blue]║[/bold bright_blue]  [dim]With Scientific Report Generation[/dim]  [bold bright_blue]║[/bold bright_blue]")
    ...
    console.print(f"[bold green]Being Spawned![/bold green]")
    ...
    output_dir = project_path / "_work_efforts" / "WE-260112-kgqt_being_plays_tavern_game_with_scientific_report_generation"
