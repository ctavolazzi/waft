...
class TownWorld:
        ...
        for agent_id, agent in self.agents.items():
            ...
            if agent.current_conversation:
                ...
                if agent.personality["sociability"] > 0.6 and random.random() < 0.2:
                    ...
                    available = [aid for aid in nearby if aid not in self.active_conversations]
                    if available:
    ...
    def _start_conversation(self, agent1_id: str, agent2_id: str):
        ...
        conversation.add_message(
            agent2_id, agent2.name, f"Hi {agent1.name}! I'm doing well, thanks!"
        )
...
async def main():
    ...
    for conv_id, conversation in list(town.conversations.items())[:5]:
        ...
        print(
            f"    Participants: {', '.join([town.agents[p].name for p in conversation.participants])}"
        )
