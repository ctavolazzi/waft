...
class Shop:
    ...
    def display_shop(cls, game_state: GameState):
        ...
        while True:
            choice = Prompt.ask(
                "What would you like to buy? (item name or 'leave')",
                default="leave"
            ).lower()
            ...
            if game_state.gold < item["cost"]:
                console.print(f"[red]Not enough gold! You need {item['cost']} gp but only have {game_state.gold} gp.[/red]")
                continue
            ...
            if item["type"] == "weapon":
                ...
                if "ac" in item:
                    game_state.character.armor_type = ArmorType.MEDIUM if item["ac"] >= 14 else ArmorType.LIGHT
                ...
                console.print(f"[green]✓[/green] Equipped {item['name']}! AC is now {game_state.character.ac}")
...
class CombatEncounter:
    ...
    def run_combat(cls, game_state: GameState, enemy_type: str = None) -> bool:
        ...
        console.print(f"You: {game_state.character.name} (AC {game_state.character.ac}, HP {game_state.character.hp}/{game_state.character.max_hp})\n")
        ...
        while enemy["current_hp"] > 0 and game_state.character.hp > 0:
            ...
            # Player turn
            console.print(f"[cyan]Your turn![/cyan]")
            action = Prompt.ask(
            ...
            if hit:
                ...
                if critical:
                    damage *= 2
                    console.print(f"[bold yellow]CRITICAL HIT![/bold yellow]")
                
                enemy["current_hp"] -= damage
                console.print(f"[green]✓[/green] You hit for {damage} damage! ({enemy['name']} HP: {enemy['current_hp']}/{enemy['hp']})")
            else:
            ...
            if enemy["current_hp"] > 0:
                ...
                if enemy_attack >= game_state.character.ac:
                    ...
                    console.print(f"[red]✗[/red] {enemy['name']} hits you for {enemy_damage} damage! (Your HP: {game_state.character.hp}/{game_state.character.max_hp})")
        ...
        # Victory!
        if enemy["current_hp"] <= 0:
            console.print(f"[bold green]🎉 VICTORY! You defeated the {enemy['name']}! 🎉[/bold green]\n")
            ...
            if game_state.encounters_completed > 0 and game_state.encounters_completed % 3 == 0:
                ...
                hp_gain = DnDRoller.roll(f"1d{game_state.character.hit_die}") + game_state.character.con_modifier
                game_state.character.max_hp += hp_gain
                game_state.character.hp += hp_gain
                console.print(f"[bold cyan]✨ LEVEL UP! You are now level {game_state.character.level}! ✨[/bold cyan]")
                console.print(f"[green]+[/green] Gained {hp_gain} HP (now {game_state.character.hp}/{game_state.character.max_hp})")
...
def create_character() -> DnD5eCharacter:
    ...
    console.print(f"  [cyan]STR:[/cyan] {strength:2d}  [cyan]DEX:[/cyan] {dexterity:2d}  [cyan]CON:[/cyan] {constitution:2d}")
    console.print(f"  [cyan]INT:[/cyan] {intelligence:2d}  [cyan]WIS:[/cyan] {wisdom:2d}  [cyan]CHA:[/cyan] {charisma:2d}")
...
def load_game(filename: str = "dnd_game_save.json") -> Optional[GameState]:
...
def main_game_loop(game_state: GameState):
    ...
    while True:
        ...
        if choice == "quit":
            ...
            console.print(f"[green]✓[/green] Restored {hp_gained} HP (now {game_state.character.hp}/{game_state.character.max_hp})")
