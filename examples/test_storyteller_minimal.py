...
def test_text_input():
    ...
    storyteller = Storyteller.from_text(
        text=text, narrative_style="simple", story_structure="linear"
    )
    ...
    pdf_path = storyteller.tell_story(
        output_path=output_path, title="Alice's Adventure", open_pdf=False
    )
...
def test_structured_data():
    ...
    data = {
        ...
        "settings": {"Office": {"name": "Office", "type": "workspace"}},
        "events": [
            {
                "character": "Alice",
                "action": "started work",
                "time": "morning",
                "description": "Alice began working on the project",
            },
            {
                "character": "Alice",
                "action": "encountered bug",
                "time": "afternoon",
                "description": "A subtle bug appeared",
            },
            {
                "character": "Bob",
                "action": "helped debug",
                "time": "afternoon",
                "description": "Bob provided guidance",
            },
            {
                "character": "Alice",
                "action": "fixed bug",
    ...
    storyteller = Storyteller.from_data(
        data=data, narrative_style="simple", story_structure="linear"
    )

    output_path = Path("_temp/test_storyteller_structured.pdf")
    pdf_path = storyteller.tell_story(
        output_path=output_path, title="Developer's Journey", open_pdf=False
    )
...
def test_events_input():
    ...
    events = [
        ...
        },
        {
            ...
            "action": "encountered bug",
            "time": "afternoon",
            "description": "A challenge appeared",
        },
        {
            "character": "Developer",
            "action": "solved bug",
            "time": "evening",
            "description": "Success was achieved",
        },
    ]

    storyteller = Storyteller.from_events(
        events=events, narrative_style="simple", story_structure="linear"
    )

    output_path = Path("_temp/test_storyteller_events.pdf")
    pdf_path = storyteller.tell_story(output_path=output_path, title="Simple Story", open_pdf=False)
...
def test_medium_complexity():
    ...
    storyteller = Storyteller.from_text(
        text=text, narrative_style="medium", story_structure="linear"
    )

    output_path = Path("_temp/test_storyteller_medium.pdf")
    pdf_path = storyteller.tell_story(
        output_path=output_path, title="Medium Complexity Story", open_pdf=False
    )
