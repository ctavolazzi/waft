...
async def run_autonomous_environment(num_cycles: int = 20):
    ...
    console.print("[bold bright_blue]║[/bold bright_blue]  [bold white]AUTONOMOUS BEING ENVIRONMENT[/bold white]  [bold bright_blue]║[/bold bright_blue]")
    ...
    for cycle in range(1, num_cycles + 1):
        ...
        for being in beings:
            ...
            try:
                ...
                cycle_data["decisions"].append({
                    "being_id": being.being_id[:20] + "..." if len(being.being_id) > 20 else being.being_id,
        ...
        if cycle % 5 == 0 or cycle == num_cycles:
            ...
            console.print(f"  Awake: {len(cycle_data['awake'])}, Sleeping: {len(cycle_data['sleeping'])}")
            
            # Show sample decisions
            if cycle_data["decisions"]:
                sample = cycle_data["decisions"][:3]
                for decision in sample:
                    console.print(f"    [dim]{decision['being_id']}: {decision['decision']} ({decision['quality']})[/dim]")
            console.print()
