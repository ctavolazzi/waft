...
def main():
    ...
    console.print(
        "[bold bright_blue]║[/bold bright_blue]  [bold white]SELF-ENGINEERING DEMO[/bold white]  [bold bright_blue]║[/bold bright_blue]"
    )
    console.print(
        "[bold bright_blue]║[/bold bright_blue]  [dim]The Meta-Game: System Engineering Itself[/dim]  [bold bright_blue]║[/bold bright_blue]"
    )
    console.print(
        "[bold bright_blue]╚════════════════════════════════════════╝[/bold bright_blue]\n"
    )
    ...
    loop = SelfEngineeringLoop(
        scenario_runner=run_tavern_scenario, project_path=Path(__file__).parent.parent
    )
    ...
    if diagnoses:
        ...
        for diagnosis in diagnoses:
            table.add_row(
                ...
                diagnosis.explanation[:50] + "..."
                if len(diagnosis.explanation) > 50
                else diagnosis.explanation,
            )
        console.print(table)
        console.print()
    ...
    if solutions:
        ...
        for solution in solutions:
            table.add_row(
                ...
                if len(solution.description) > 50
                else solution.description,
            )
        console.print(table)
        console.print()
    ...
    if result.improvements:
        ...
        for improvement in result.improvements:
            table.add_row(
                ...
                improvement.solution.description[:40] + "..."
                if len(improvement.solution.description) > 40
                else improvement.solution.description,
    ...
    console.print("  The system tried to play itself")
