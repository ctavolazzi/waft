...
def tavern_scenario(character: DnD5eCharacter, session: GameSession):
    ...
    if choice == "1":
        ...
        session.add_event(
            "choice",
            "Perception Check: Stand up and look around",
            {
                "roll": roll,
                "modifier": wis_mod,
                "total": total,
        ...
        session.add_event(
            "choice",
            "Persuasion Check: Ask the bartender",
            {
                "roll": roll,
                "modifier": cha_mod,
                "total": total,
    ...
    if final_choice == "1":
        ...
        print_scenario(ending)
        session.add_event(
            "decision", "Ask around town - Gather more information", {"ending": ending}
        )
    else:
        ending = (
            "\n[bold]The End?[/bold]\n\n"
...
def generate_pdf_story(session: GameSession) -> Path:
    ...
    try:
        ...
        console.print("[green]✓[/green] PDF opened on desktop!")
        return pdf_path
...
def main():
    ...
    console.print(
        "[bold bright_blue]║[/bold bright_blue]  [bold white]TOWN TAVERN SCENARIO[/bold white]  [bold bright_blue]║[/bold bright_blue]"
    )
    console.print(
        "[bold bright_blue]║[/bold bright_blue]  [dim]A D&D 5e Adventure[/dim]  [bold bright_blue]║[/bold bright_blue]"
    )
    console.print(
        "[bold bright_blue]╚════════════════════════════════════════╝[/bold bright_blue]\n"
    )
