...
class ObstacleCourse:
    ...
    def obstacle_1_basic_functionality(self):
        ...
        self.test("1.2 /think Ability", lambda: 
            "status" in self.pyrite.execute_ability("/think")
        )
        
        # Test 1.3: /status ability
        self.test("1.3 /status Ability", lambda:
            "personality" in self.pyrite.execute_ability("/status")
        )
        
        # Test 1.4: Personality summary
        self.test("1.4 Personality Summary", lambda:
            len(self.pyrite.get_personality_summary()) > 0
        )
        
        # Test 1.5: Work effort graph
        self.test("1.5 Work Effort Graph", lambda:
            len(self.pyrite._work_effort_graph) > 0
        )
    ...
    def obstacle_2_locking(self):
        ...
        we_id = list(self.pyrite._work_effort_graph.keys())[0] if self.pyrite._work_effort_graph else None
        if not we_id:
            print("  ⚠️  No work efforts available for locking tests")
            return
        
        # Test 2.1: Acquire lock
        self.test("2.1 Acquire Lock", lambda:
            self.pyrite.acquire_lock(we_id, "test-lock-1", timeout=5.0)
        )
        
        # Test 2.2: Check if locked
        self.test("2.2 Is Locked", lambda:
            self.pyrite.is_locked(we_id)
        )
        
        # Test 2.3: Get lock holder
        self.test("2.3 Get Lock Holder", lambda:
            self.pyrite.get_lock_holder(we_id) == "test-lock-1"
        )
        
        # Test 2.4: Fail to acquire second lock
        self.test("2.4 Fail Second Lock", lambda:
            not self.pyrite.acquire_lock(we_id, "test-lock-2", timeout=1.0)
        )
        
        # Test 2.5: Release lock
        self.test("2.5 Release Lock", lambda:
            self.pyrite.release_lock(we_id, "test-lock-1")
        )
        
        # Test 2.6: Verify unlocked
        self.test("2.6 Verify Unlocked", lambda:
            not self.pyrite.is_locked(we_id)
        )
    ...
    def obstacle_3_concurrent_locking(self):
        ...
        self.test("3.1 Sequential Locks", lambda:
            all(self.pyrite.acquire_lock(we_id, f"seq-{i}", timeout=1.0) and
                self.pyrite.release_lock(we_id, f"seq-{i}")
                for i in range(5))
        )
        ...
        self.test("3.2 Concurrent Locks Serialize", lambda:
            successful == 5  # All should eventually succeed
        )
    ...
    def obstacle_4_monitoring(self):
        ...
        self.test("4.1 Monitor All", lambda:
            "total_work_efforts" in self.pyrite.execute_ability("/monitor")
        )
        
        # Test 4.2: Monitor specific
        we_id = list(self.pyrite._work_effort_graph.keys())[0] if self.pyrite._work_effort_graph else None
        if we_id:
            self.test("4.2 Monitor Specific", lambda:
                "we_id" in self.pyrite.execute_ability("/monitor", we_id)
            )
        
        # Test 4.3: State history
        if we_id:
            self.test("4.3 State History", lambda:
                isinstance(self.pyrite.get_state_history(we_id), list)
            )
        
        # Test 4.4: Metrics
        self.test("4.4 Metrics", lambda:
            isinstance(self.pyrite.get_metrics(), dict)
        )
    ...
    def obstacle_5_organization(self):
        ...
        # Test 5.1: Organize
        self.test("5.1 Organize", lambda:
            "total_nodes" in self.pyrite.execute_ability("/organize")
        )
        
        # Test 5.2: Get work effort
        we_id = list(self.pyrite._work_effort_graph.keys())[0] if self.pyrite._work_effort_graph else None
        if we_id:
            self.test("5.2 Get Work Effort", lambda:
                self.pyrite.get_work_effort(we_id) is not None
            )
        
        # Test 5.3: Get children
        if we_id:
            self.test("5.3 Get Children", lambda:
                isinstance(self.pyrite.get_children(we_id), list)
            )
        
        # Test 5.4: Get ancestors
        if we_id:
            self.test("5.4 Get Ancestors", lambda:
                isinstance(self.pyrite.get_ancestors(we_id), list)
            )
    ...
    def obstacle_6_evolution(self):
        ...
        self.test("6.1 Initiate Evolution", lambda:
            result.get("status") == "success" or "error" in result
        )
        
        # Test 6.2: Evolutionary history
        self.test("6.2 Evolutionary History", lambda:
            isinstance(self.pyrite.get_evolutionary_history(we_id), list)
        )
        
        # Test 6.3: Different strategies
        strategies = ["conservative", "aggressive", "adaptive", "exploratory"]
        for strategy in strategies:
            self.test(f"6.3 Strategy: {strategy}", lambda s=strategy:
                self.pyrite.initiate_evolution(we_id, EvolutionaryStrategy(s), 2) is not None
            )
    ...
    def obstacle_7_personality(self):
        ...
        self.test("7.1 Get Attributes", lambda:
            len(self.pyrite._attributes) > 0
        )
        ...
        self.test("7.2 Update Attribute", lambda:
            new_wisdom > initial_wisdom
        )
        ...
        self.test("7.3 Grow Attributes", lambda:
            all(after[name] >= before[name] for name in before)
        )
        
        # Test 7.4: Personality summary
        self.test("7.4 Personality Summary", lambda:
            len(self.pyrite.get_personality_summary()) > 0
        )
    ...
    def obstacle_8_secrets(self):
        ...
        secret_id = self.pyrite.create_secret(
            {"test": "data"},
            {"visible": "metadata"}
        )
        self.test("8.1 Create Secret", lambda:
            secret_id is not None and len(secret_id) > 0
        )
        
        # Test 8.2: Get metadata
        self.test("8.2 Get Secret Metadata", lambda:
            self.pyrite.get_secret_metadata(secret_id) is not None
        )
        
        # Test 8.3: List secrets
        self.test("8.3 List Secrets", lambda:
            len(self.pyrite.list_secrets()) > 0
        )
        
        # Test 8.4: Multiple secrets
        secret_ids = [self.pyrite.create_secret({"data": i}, {"index": i}) for i in range(3)]
        self.test("8.4 Multiple Secrets", lambda:
            len(secret_ids) == 3 and all(sid for sid in secret_ids)
        )
    ...
    def obstacle_9_status_management(self):
        ...
        self.test("9.1 Update Status", lambda:
            self.pyrite.update_work_effort_status(we_id, WorkEffortStatus.ACTIVE)
        )
        
        # Test 9.2: Verify status changed
        node_after = self.pyrite.get_work_effort(we_id)
        self.test("9.2 Verify Status Changed", lambda:
            node_after.status == WorkEffortStatus.ACTIVE
        )
    ...
    def obstacle_10_edge_cases(self):
        ...
        self.test("10.1 Lock Non-Existent", lambda:
            not self.pyrite.acquire_lock("NON-EXISTENT", "test", timeout=0.1)
        )
        
        # Test 10.2: Monitor non-existent
        result = self.pyrite.execute_ability("/monitor", "NON-EXISTENT")
        self.test("10.2 Monitor Non-Existent", lambda:
            "error" in result
        )
        ...
        we_id = list(self.pyrite._work_effort_graph.keys())[0] if self.pyrite._work_effort_graph else None
        if we_id:
            self.test("10.3 Release Lock Not Held", lambda:
                not self.pyrite.release_lock(we_id, "not-held-lock")
            )
        
        # Test 10.4: Evolve non-existent
        result = self.pyrite.execute_ability("/evolve", "NON-EXISTENT", "adaptive", 1)
        self.test("10.4 Evolve Non-Existent", lambda:
            result.get("status") == "failed"
        )
        
        # Test 10.5: Get work effort non-existent
        self.test("10.5 Get Work Effort Non-Existent", lambda:
            self.pyrite.get_work_effort("NON-EXISTENT") is None
        )
    ...
    def obstacle_11_empirica(self):
        ...
        self.test("11.1 Empirica Initialized", lambda:
            hasattr(self.pyrite, 'empirica')
        )
        
        # Test 11.2: Session exists
        self.test("11.2 Empirica Session", lambda:
            self.pyrite._empirica_session_id is not None or
            not self.pyrite.empirica.is_initialized()
        )
        
        # Test 11.3: /think includes Empirica
        result = self.pyrite.execute_ability("/think")
        self.test("11.3 /think Includes Empirica", lambda:
            "empirica" in result
        )
        ...
        if we_id and not self.pyrite.is_locked(we_id):
            ...
            self.test("11.4 /evolve Uses Empirica", lambda:
                "empirica" in result or result.get("status") == "failed"
            )
