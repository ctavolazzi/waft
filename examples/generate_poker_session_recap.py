...
def generate_poker_session_recap(
    session: PokerSession, output_path: Path, include_rules: bool = False
) -> Path:
    """
    ...
    # Session header
    content_lines.append("# Poker Session Recap")
    ...
    if stats:
        ...
        if stats.get("most_wins"):
            content_lines.append(
                f"**Most Wins**: {stats['most_wins']} ({stats['wins_by_player'][stats['most_wins']]} hands)"
            )
        ...
        if stats.get("wins_by_player"):
            ...
            for player, wins in sorted(
                stats["wins_by_player"].items(), key=lambda x: x[1], reverse=True
            ):
...
def example_session():
    ...
    pdf_path = generate_poker_session_recap(
        session=session, output_path=output_path, include_rules=True
    )
