...
from src.waft.evolution.styling_genome import StylingGenomeRegistry, StylingGenome, StylingGene, FontGene, MarginGene, ColorGene, LayoutGene
...
def test_integration():
    ...
    grouped_metrics = [c for c in components if c.metadata.get('component_subtype') == 'grouped_metrics']
    git_summary = [c for c in components if c.metadata.get('component_subtype') == 'git_summary']
    work_efforts = [c for c in components if c.metadata.get('component_subtype') == 'work_efforts_summary']
...
def test_pdf_generation():
    ...
    distilled = distiller.distill_text(
        "WAFT Kernel Status Report (Phase 4 Enhanced Display)",
        title="Status Report"
    )
