...
app = FastAPI(
    title="Cute Test Runner",
    description="A cute little test runner 🧪",
    version="1.0.0"
)
...
def run_pytest() -> Dict[str, Any]:
    ...
    try:
        ...
        result = subprocess.run(
            ["pytest", "-v", "--tb=short", "--json-report", "--json-report-file=/tmp/pytest-report.json"],
            cwd=PROJECT_ROOT,
            capture_output=True,
            text=True,
            timeout=60
...
def parse_pytest_output(stdout: str, stderr: str, returncode: int, duration: float) -> Dict[str, Any]:
    ...
    for line in lines:
        ...
        if line.startswith("test_"):
            ...
            if len(parts) >= 2:
                ...
                status = "passed" if "PASSED" in status_str else ("failed" if "FAILED" in status_str else "skipped")
