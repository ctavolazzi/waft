...
def gather_late_night_status() -> dict[str, Any]:
    ...
    if work_efforts_dir.exists():
        ...
        all_we = sorted(
            work_efforts_dir.rglob("*.md"), key=lambda p: p.stat().st_mtime, reverse=True
        )
        today_we = [
            f for f in all_we if datetime.fromtimestamp(f.stat().st_mtime).date() == now.date()
        ]
    ...
    if devlog_path.exists():
        try:
            ...
            status["recent_devlog"] = (
                devlog_content[-500:] if len(devlog_content) > 500 else devlog_content
            )
        except Exception:
            status["recent_devlog"] = ""
    else:
...
def build_late_night_content(status: dict[str, Any], insights: str | None = None) -> str:
    ...
    if status["late_night_work_efforts_count"] > 0:
        accomplishments.append(
            f"🌙 {status['late_night_work_efforts_count']} work efforts updated during late-night session"
        )
    ...
    if status["today_work_efforts_count"] > 0:
        accomplishments.append(
            f"✅ {status['today_work_efforts_count']} work effort files created/updated today"
        )
...
def main():
    ...
    parser.add_argument(
        "--insights", default=None, help="Late-night session insights or key learnings"
    )
    ...
    doc = BriefDocument(
        ...
        cover_signature={"role": "AUTHORIZED BY", "name": "Site-Delta-9", "date": status["date"]},
        ...
        chat_context={
            ...
            "key_decisions": [
                "Late-night status captured",
                "Insights documented",
                "Tomorrow priorities identified",
            ],
            "next_steps": [
