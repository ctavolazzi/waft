            ...
            if item.is_dir() and item.name.startswith("WE-") and today_pattern in item.name:
                ...
                if reasoning_file.exists():
                    try:
                        ...
                        trace.append({
                            "timestamp": datetime.fromtimestamp(reasoning_file.stat().st_mtime).isoformat(),
                            "decision": f"Work effort: {item.name}",
                            "reasoning": reasoning_content[:500] + "..." if len(reasoning_content) > 500 else reasoning_content,
