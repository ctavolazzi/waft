...
def main():
    ...
    parser.add_argument("--plan", type=Path, help="Path to plan file (default: most recent)")
    parser.add_argument("--plan-name", type=str, help="Name of plan to find (partial match)")
    parser.add_argument(
        "--critique", type=Path, help="Path to critique file (default: most recent)"
    )
    parser.add_argument("--dry-run", action="store_true", help="Show revisions without applying")
    parser.add_argument(
        "--severity",
        choices=["CRITICAL", "HIGH", "MEDIUM", "LOW"],
        help="Only revise criticisms of this severity",
    )
    parser.add_argument("--rollback", action="store_true", help="Rollback last revision")
