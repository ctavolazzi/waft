...
from template_metadata_extractor import TemplateMetadata, extract_metadata, format_metadata_markdown
...
class TypstTemplateBrowser:
    ...
    def get_template_readme(self, template_name: str) -> str | None:
        """
    ...
    def check_wrapper_exists(self, template_name: str) -> tuple[bool, str | None]:
        """
    ...
    def generate_report(self, output_path: Path) -> None:
        ...
        for template_name in sorted(self.templates.keys()):
            ...
            wrapper_col = (
                f"[{Path(template.wrapper_path).name}]({template.wrapper_path})"
                if template.wrapper_path
                else "-"
            )
            description = (
                template.metadata.description if template.metadata else "No description available"
            )
        ...
        for template_name in sorted(self.templates.keys()):
            ...
            if template.metadata:
                ...
                lines.extend(
                    [
                        "**Description:**",
                        template.readme_content[:500] + "..."
                        if len(template.readme_content) > 500
                        else template.readme_content,
                        "",
                    ]
                )
            else:
