...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create Quest from Plan document"
    )
    parser.add_argument(
        "plan_path",
        nargs="?",
        type=Path,
        help="Path to plan file (optional if using --all)"
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Create quests for all plans"
    )
    parser.add_argument(
        "--plans-dir",
        type=Path,
        help="Plans directory (default: _work_efforts/Plans/)"
    )
    parser.add_argument(
        "--project-path",
        type=Path,
