...
def generate_quest_card_html(quest_id, quest, checkpoints, tests, project_path):
    ...
    achievements_html = "\n                                    ".join(
        [f"<li>{ach}</li>" for ach in quest.achievements]
    )

    # Generate prerequisites HTML
    prereqs_html = (
        "\n                                    ".join(
            [f'<li><a href="#{prereq}">{prereq}</a></li>' for prereq in quest.prerequisites]
        )
        if quest.prerequisites
        else "<li>None - Start here!</li>"
    )
