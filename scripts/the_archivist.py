...
class TheArchivist:
    ...
    def collect_todays_pdfs(self, date: Optional[str] = None) -> List[Path]:
    ...
    def archive_daily_pdfs(self, date: Optional[str] = None) -> Dict[str, Any]:
    ...
    def generate_morning_report(self, date: Optional[str] = None) -> Path:
        ...
        work_efforts = [p for p in pdfs if "_work_efforts" in str(p.parent) and p not in briefs and p not in proofs]
        other = [p for p in pdfs if p not in briefs and p not in proofs and p not in studies and p not in work_efforts]
        
        # Build report content
        report_content = self._build_report_content(date, briefs, proofs, studies, work_efforts, other)
    ...
    def _build_report_content(self, date: str, briefs: List[Path], proofs: List[Path], 
                             studies: List[Path], work_efforts: List[Path], other: List[Path]) -> str:
        """Build report content HTML."""
        content = []
        
        content.append(f'<h2>Morning Archive Report - {date}</h2>')
        content.append(f'<p><strong>Date:</strong> {date}</p>')
        content.append(f'<p><strong>Total PDFs Collected:</strong> {len(briefs) + len(proofs) + len(studies) + len(work_efforts) + len(other)}</p>')
        content.append('')
        ...
        if briefs:
            ...
            for pdf in briefs:
                ...
                content.append(f'<li><strong>{pdf.name}</strong> ({size_kb:.1f} KB)<br><code>{rel_path}</code></li>')
            content.append('</ul>')
            content.append('')
        ...
        if work_efforts:
            ...
            if len(work_efforts) > 10:
                content.append(f'<li><em>... and {len(work_efforts) - 10} more work effort documents</em></li>')
            content.append('</ul>')
            content.append('')
        ...
        for cat_name, cat_pdfs in categories:
            if cat_pdfs:
                ...
                total_size += cat_size
                content.append(f'<tr><td>{cat_name}</td><td>{len(cat_pdfs)}</td><td>{cat_size:.1f}</td></tr>')
        
        content.append(f'<tr><td><strong>Total</strong></td><td><strong>{len(briefs) + len(proofs) + len(studies) + len(work_efforts) + len(other)}</strong></td><td><strong>{total_size:.1f}</strong></td></tr>')
        content.append('</table>')
        content.append('')
        
        content.append('<h3>📦 Archive Status</h3>')
        content.append('<p>All documents have been catalogued and archived for future reference.</p>')
        content.append('<p><em>The Archivist keeps things tidy, just in case we need them again someday.</em></p>')
...
def main():
    ...
    parser.add_argument("--report-only", action="store_true", help="Generate report only, don't archive")
    parser.add_argument("--archive-only", action="store_true", help="Archive only, don't generate report")
