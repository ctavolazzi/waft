...
def get_repo_info_via_gh(owner: str, repo: str) -> dict | None:
    ...
    try:
        result = subprocess.run(
            [
                ...
                f"{owner}/{repo}",
                "--json",
                "name,description,url,stargazerCount,updatedAt,isArchived,primaryLanguage,topics",
            ],
            capture_output=True,
            text=True,
            check=True,
...
def get_readme_via_gh(owner: str, repo: str) -> str | None:
...
def analyze_repository(repo_info: dict) -> dict:
    ...
    if repo_data:
        ...
        print("  ⚠️  Could not retrieve repository info")
    ...
    if readme:
        ...
        print("  ⚠️  Could not retrieve README")
...
def save_analysis(analysis: dict):
    ...
    work_effort_name = (
        f"WE-260111-{work_effort_id}_{analysis['repo'].replace('_', '-')}_installation_exploration"
    )
...
def main():
    ...
    summary_file = (
        ...
        / "REPOSITORY_ANALYSIS_SUMMARY.md"
    )
