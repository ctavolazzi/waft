...
def generate_observations_pdf(
    observations_file: Path, experiment_name: str, cycle: int, output_dir: Path | None = None
) -> Path:
    """
    ...
    doc = BriefDocument(
        ...
        cover_warning={
            "message": "EXPERIMENTAL FINDINGS - Ready for next iteration cycle",
            "severity": "INFO",
        },
    ...
    try:
        ...
        html_content = markdown.markdown(
            observations, extensions=["fenced_code", "tables", "nl2br", "extra", "codehilite"]
        )
    except ImportError:
        import re
...
def generate_preparation_pdf(
    preparation_file: Path, experiment_name: str, next_cycle: int, output_dir: Path | None = None
) -> Path:
    """
    ...
    try:
        ...
        html_content = markdown.markdown(
            preparation, extensions=["fenced_code", "tables", "nl2br", "extra", "codehilite"]
        )
    except ImportError:
        import re
...
def main():
    parser = argparse.ArgumentParser(description="Record experiment cycle and generate PDF reports")
    parser.add_argument("--experiment", required=True, help="Name of the experiment")
    parser.add_argument("--cycle", type=int, required=True, help="Current cycle/iteration number")
    parser.add_argument(
        "--observations-file", type=Path, required=True, help="Path to observations markdown file"
    )
    parser.add_argument(
        "--preparation-file", type=Path, help="Path to preparation markdown file (optional)"
    )
    parser.add_argument(
        "--open", action="store_true", default=True, help="Open PDFs on desktop (default: True)"
    )
    parser.add_argument(
        "--no-open", dest="open", action="store_false", help="Don't open PDFs on desktop"
    )
    ...
    obs_pdf = generate_observations_pdf(args.observations_file, args.experiment, args.cycle)
    print(f"✅ Observations PDF: {obs_pdf}")

    if args.open:
        open_pdf_on_desktop(obs_pdf)
        print("   Opened on desktop")
    ...
    if args.preparation_file and args.preparation_file.exists():
        ...
        prep_pdf = generate_preparation_pdf(args.preparation_file, args.experiment, next_cycle)
