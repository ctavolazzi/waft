...
def launch_scenario_campaign(project_root: Path, new_party: bool = False):
    ...
    try:
        from waft.core.dnd_scenario import QuestPDFGenerator, ScenarioOrchestrator, ScenarioRealm
...
"""
        ...
        quest_pdf = quest_generator.generate_quest_pdf(
            quest_markdown=quest_markdown, quest_title=quest_title, template="wenyuan-campaign"
        )
...
def main():
    ...
    parser.add_argument(
        "--new-party", action="store_true", help="Create a new party (force new party creation)"
    )
    parser.add_argument("--location", help="Start at specific location (web mode only)")
    parser.add_argument("--quest", help="Start with specific quest active (web mode only)")

    args = parser.parse_args()

    project_root = Path(__file__).parent.parent

    print("🎲 D&D Campaign Launcher")
