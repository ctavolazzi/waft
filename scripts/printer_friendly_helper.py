...
def convert_css_to_printer_friendly(css: str) -> str:
    ...
    color_replacements = {
        ...
        r"border.*:\s*\d+px\s+double\s+#[0-9a-fA-F]{3,6}": lambda m: m.group(0).split("#")[0]
        + "#000",
    ...
    result = re.sub(
        r"color:\s*#[0-9a-fA-F]{3,6}(?=\s*[;})])", "color: #000", result, flags=re.IGNORECASE
    )
...
def convert_html_template_to_printer_friendly(html_template: str) -> str:
    ...
    result = re.sub(r"<style>(.*?)</style>", replace_style, result, flags=re.DOTALL | re.IGNORECASE)
...
def create_printer_friendly_field_guide_template() -> str:
    ...
    template_file = (
        Path(__file__).parent.parent / "examples" / "generate_waft_field_guide_printer_friendly.py"
    )
