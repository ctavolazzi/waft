...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create a serious Mission (Military Brass)"
    )
    parser.add_argument(
        "objective",
        nargs="?",
        help="Mission objective (clear, measurable)"
    )
    parser.add_argument(
        "--name",
        type=str,
        help="Mission name (auto-generated from objective if not provided)"
    )
    parser.add_argument(
        "--classification",
        type=str,
    ...
    parser.add_argument(
        "--briefing",
        type=str,
        help="Mission briefing content"
    )
    parser.add_argument(
        "--success-criteria",
        nargs="+",
        help="Success criteria (space-separated list)"
    )
    parser.add_argument(
        "--difficulty",
        type=int,
        help="Mission difficulty (1-10, auto-calculated if not provided)"
    )
    parser.add_argument(
        "--project-path",
        type=Path,
    ...
    parser.add_argument(
        "--from-plan",
        type=Path,
        help="Create mission from plan file"
    )
    parser.add_argument(
        "--no-pdf",
        action="store_true",
        help="Don't generate mission PDF"
    )
