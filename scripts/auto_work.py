...
try:
    from src.waft.pantheon import (
        Judge, Magistrate, TheReasoner, GitHubGod,
        Fae, MissionControl, TheVillage, Storyteller,
        Librarian, MilitaryBrass, TestRunner
    )
        ...
        if index_file:
            try:
                ...
                if index_file.stat().st_size > MAX_INDEX_FILE_SIZE:
                    logger.warning(f"Index file too large for {we_id}: {index_file.stat().st_size} bytes")
                else:
            ...
            if magistrate:
                try:
                    ...
                    if precedents:
                        ...
                        proven_count = sum(1 for p in precedents[:3] if p.confidence and p.confidence > 0.7)
                        if proven_count > 0:
            ...
            if librarian:
                try:
                    ...
                    if related_records:
                        ...
                        score += 3.0 * len(related_records)  # Small boost for knowledge base presence
                except Exception as e:
...
def select_best_work_effort(
    ...
    for idx, we in enumerate(work_efforts, 1):
        ...
        score = calculate_work_effort_priority(we, project_path, empirica_manager, pantheon_entities)
        ...
        if top_score > 0 and (top_score - second_score) / top_score < 0.1:
            try:
                ...
                if decision_gate == "BRANCH" and second_we.get('id'):
                    logger.info(f"Empirica suggests investigating {second_we.get('id')} - considering override")
        ...
        try:
            ...
            if judge:
                ...
                if judgment.verdict.value == "DISPROVEN" and judgment.confidence > 0.8:
                    logger.warning(f"Judge DISPROVES selection of {selected.get('id')} (confidence: {judgment.confidence:.2f})")
                ...
                try:
                    ...
                    if active_quests:
                        ...
                        for quest in active_quests[:3]:
                            if quest.name.lower() in we_title_lower or we_title_lower in quest.name.lower():
                                logger.info(f"Fae blessing: Selected work effort aligns with quest '{quest.name}'")
                                break
                except Exception as e:
                ...
                try:
                    ...
                    if registry.exists():
                        ...
                        if any(we_id in str(m) for m in monitored):
                            logger.info(f"MissionControl: Work effort {we_id} is part of monitored mission")
                except Exception as e:
...
def get_work_effort_action(work_effort: Dict, project_path: Path) -> Optional[Dict]:
    """
...
def execute_work_effort_action(
    ...
    if not command or len(command) > 500:  # Reasonable limit
        logger.error(f"Invalid command: too long or empty")
        return {
            "success": False,
...
"""
            ...
            logger.info(f"Campfire story told: {story_result.get('story', {}).get('id', 'unknown')}")
...
def main():
    ...
    try:
        result = execute_work_effort_action(
            ...
            empirica_manager, pantheon_entities,
