...
def generate_field_guides(output_dir: Path, printer_friendly: bool = False, level: str = None):
    ...
    if level:
        if level not in levels:
            print(f"Error: Invalid level '{level}'. Must be one of: layman, professional, scientist")
            return False
...
def generate_booklet(output_dir: Path, printer_friendly: bool = False):
    ...
    print(f"Generating complete booklet ({'printer-friendly' if printer_friendly else 'standard'})...")
...
def main():
    ...
    fg_parser.add_argument('--printer-friendly', action='store_true', help='Generate printer-friendly version')
    fg_parser.add_argument('--level', choices=['layman', 'professional', 'scientist'], help='Generate specific level only')
    
    # Booklet command
    booklet_parser = subparsers.add_parser('booklet', help='Generate complete booklet')
    booklet_parser.add_argument('--printer-friendly', action='store_true', help='Generate printer-friendly version')
    ...
    redact_parser.add_argument('--output', type=Path, help='Output PDF path (default: input_redacted.pdf)')
    redact_parser.add_argument('--areas', nargs='+', required=True, help='Areas to redact: "x,y,width,height" or "x,y,width,height:label"')
    
    # All command
    all_parser = subparsers.add_parser('all', help='Generate everything')
    all_parser.add_argument('--printer-friendly', action='store_true', help='Include printer-friendly versions')
