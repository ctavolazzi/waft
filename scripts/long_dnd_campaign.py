...
class ExtendedCampaign:
    ...
    def spawn_party(self) -> List[PartyMember]:
        ...
        for i, config in enumerate(party_configs):
            ...
            being.record_memory(f"Joined adventuring party as {config['class']}", "experience", {
...
Something ancient has awakened. We need brave souls to face whatever darkness lies within."
        ...
        console.print(Panel(scene["content"], title="[bold]The Rusty Tankard[/bold]", border_style="yellow"))
        return scene
    
    def generate_detailed_encounter(self, encounter_name: str, difficulty: str = "medium", description: str = None) -> Dict[str, Any]:
        """Generate a detailed combat encounter with rich narrative."""
        console.print(f"\n[bold red]⚔️  ENCOUNTER: {encounter_name} ⚔️[/bold red]\n")
        
        difficulty_multiplier = {"easy": 0.8, "medium": 1.0, "hard": 1.5, "boss": 3.0, "epic": 5.0}[difficulty]
...
After {rounds} rounds of combat, {encounter_name} falls, defeated by the heroes' resolve.
        ...
        console.print(f"   [green]✓[/green] Encounter complete! Party HP: {sum([m.hp for m in self.party])}/{sum([m.max_hp for m in self.party])}")
        return encounter
    
    def generate_story_chapter(self, chapter_num: int, title: str, encounters: List[Dict[str, Any]], intro_text: str = None) -> Dict[str, Any]:
...
Light floods the chamber. The corruption is gone. You have won.
        ...
        console.print(Panel(f"[bold green]VICTORY![/bold green]\n\n{boss_name} has been defeated!\n\nThe party has saved the realm!", 
                           title="[bold]🎉 CAMPAIGN COMPLETE 🎉[/bold]", border_style="green"))
            ...
            chapters_config = [
                {
                    ...
                    "encounters": [
                        {"name": "Goblin Ambush", "difficulty": "easy", "description": "A group of goblins ambushes the party from the underbrush. Quick thinking and combat skills are required. The goblins are poorly organized but numerous, making this a test of the party's ability to work together under pressure."},
                        {"name": "Wolves of the Darkwood", "difficulty": "medium", "description": "A pack of dire wolves blocks the path. Their eyes glow with an unnatural hunger. These are no ordinary wolves - something has corrupted them, making them larger and more aggressive than normal. The party must fight or find a way to avoid them."},
                        {"name": "Bandit Encounter", "difficulty": "medium", "description": "Highway bandits attempt to rob the party, but they are no match for skilled adventurers. However, the bandits reveal information about strange happenings near the keep, suggesting the darkness is spreading."},
                        {"name": "Mysterious Traveler", "difficulty": "easy", "description": "A mysterious traveler provides information about the keep, but warns of great danger ahead. The traveler speaks of ancient evils and warns that the party may be walking into a trap. They offer cryptic advice before disappearing into the mist."},
                        {"name": "The Abandoned Village", "difficulty": "medium", "description": "The party discovers an abandoned village. The buildings are empty, but signs of a hasty departure are everywhere. Something terrible happened here, and the party must investigate to understand what they're facing."},
                        {"name": "Corrupted Wildlife", "difficulty": "easy", "description": "The party encounters animals that have been corrupted by the darkness. Birds with glowing red eyes, deer with twisted antlers, and other creatures attack with unnatural ferocity. The corruption is spreading."}
                    ]
                },
                {
                    ...
                    "encounters": [
                        {"name": "Spectral Apparitions", "difficulty": "medium", "description": "Ghostly figures appear, remnants of those who died in the forest long ago. These spirits are trapped between worlds, unable to find peace. They attack the party, driven by rage and sorrow. The party must fight them while trying to understand their tragic story."},
                        {"name": "Giant Spiders", "difficulty": "hard", "description": "Massive spiders descend from the canopy, their webs blocking the path. These are no ordinary spiders - they're the size of horses, with venom that can paralyze. The party must cut through the webs and fight their way past these guardians of the forest."},
                        {"name": "Ancient Treant", "difficulty": "hard", "description": "An ancient treant blocks the path, but can be reasoned with if approached correctly. The treant is thousands of years old and has seen the corruption spreading. It tests the party's intentions before allowing them to pass, offering wisdom about the darkness ahead."},
                        {"name": "Forest Guardian", "difficulty": "medium", "description": "A guardian spirit tests the party's worthiness to pass through the sacred grove. The guardian is a powerful fey creature that protects the forest. It challenges the party to prove they are worthy of continuing their quest."},
                        {"name": "The Lost Shrine", "difficulty": "medium", "description": "The party discovers an ancient shrine hidden in the forest. The shrine is dedicated to a forgotten deity, and it offers a moment of rest and healing. However, corrupted creatures have taken up residence, and the party must cleanse the shrine before they can benefit from its power."},
                        {"name": "The Whispering Trees", "difficulty": "easy", "description": "The trees themselves seem to whisper warnings and threats. The party must navigate through a grove where the trees move and attack, their branches like whips. This is a test of the party's ability to work together and protect each other."}
                    ]
                },
                {
                    ...
                    "encounters": [
                        {"name": "Skeleton Warriors", "difficulty": "medium", "description": "Undead warriors rise from the ground, their bones clattering as they advance. These are the remains of the keep's former defenders, now animated by dark magic. They fight with the skill they had in life, making them dangerous opponents."},
                        {"name": "Dark Cultists", "difficulty": "hard", "description": "Cultists performing dark rituals attempt to stop the party from reaching the keep. These cultists are fanatical followers of the Shadow Lord, and they will stop at nothing to prevent the party from reaching their master. They use dark magic and are willing to sacrifice themselves."},
                        {"name": "Shadow Beasts", "difficulty": "hard", "description": "Creatures of pure shadow emerge from the darkness, their forms shifting and unnatural. These are not natural creatures - they are manifestations of the darkness itself. They are difficult to hit and can phase through solid objects, making them extremely dangerous."},
                        {"name": "The Keep's Gate", "difficulty": "medium", "description": "The massive gate is guarded by corrupted sentinels, but the party finds a way through. The gate itself is a massive structure of dark stone, covered in runes that pulse with evil energy. The sentinels are powerful, but the party's determination sees them through."},
                        {"name": "The Moat of Despair", "difficulty": "medium", "description": "Before reaching the gate, the party must cross a moat filled with dark, churning water. The water itself seems alive and hostile, and creatures lurk beneath the surface. The party must find a way across while fighting off attacks from both the water and the shore."},
                        {"name": "The Outer Defenses", "difficulty": "hard", "description": "The keep's outer defenses are still active, with ballistae and other siege weapons firing at the party. They must dodge projectiles while fighting off the defenders. This is a test of the party's ability to work under pressure and coordinate their movements."}
                    ]
                },
                {
                    ...
                    "encounters": [
                        {"name": "Corrupted Guards", "difficulty": "medium", "description": "The keep's former guards now serve the darkness, their eyes glowing with malevolent light. These guards remember their training and fight with military precision, but they are driven by dark magic that makes them relentless. The party must overcome their former allies."},
                        {"name": "Undead Servants", "difficulty": "easy", "description": "Zombified servants shamble through the halls, attacking anything that moves. These are the keep's former staff, now mindless undead. They are slow but numerous, and they can overwhelm the party if they're not careful."},
                        {"name": "The Keep's Lieutenant", "difficulty": "hard", "description": "A powerful lieutenant of the dark lord blocks the way forward, wielding dark magic. This lieutenant was once a paladin of light, but has been corrupted and now serves the Shadow Lord. They are a tragic figure, and the party must defeat them to continue."},
                        {"name": "Ancient Traps", "difficulty": "medium", "description": "The keep is riddled with traps, both ancient and newly placed by the dark forces. The party must use their skills to detect and disarm these traps, or suffer the consequences. This is a test of the party's rogues and their ability to work together to avoid danger."},
                        {"name": "The Armory", "difficulty": "medium", "description": "The party discovers the keep's armory, which contains weapons and armor that could help them. However, the armory is guarded by animated suits of armor that attack anyone who tries to take the equipment. The party must fight their way through to claim these valuable resources."},
                        {"name": "The Barracks", "difficulty": "hard", "description": "The keep's barracks are filled with corrupted soldiers who were once the keep's defenders. These soldiers are organized and fight as a unit, making them extremely dangerous. The party must use strategy and teamwork to overcome this challenge."}
                    ]
                },
                {
                    ...
                    "encounters": [
                        {"name": "Shadow Wraiths", "difficulty": "hard", "description": "Powerful wraiths made of pure shadow attack from all sides. These wraiths are the souls of those who died in the keep, now bound to serve the Shadow Lord. They are difficult to hit and can drain the life force from their victims. The party must work together to defeat them."},
                        {"name": "Corrupted Artifacts", "difficulty": "medium", "description": "Ancient artifacts have been corrupted and now attack the party with dark magic. These artifacts were once holy relics, but the darkness has twisted them into weapons of evil. The party must destroy them to continue, but doing so releases powerful dark magic."},
                        {"name": "The Dark Council", "difficulty": "hard", "description": "A council of dark mages attempts to stop the party before they reach the final chamber. These mages are powerful spellcasters who have willingly embraced the darkness. They work together, casting spells that complement each other and create devastating combinations."},
                        {"name": "The Guardian Chamber", "difficulty": "boss", "description": "A massive guardian creature blocks the path to the final boss, its form shifting between reality and shadow. This guardian is a creation of the Shadow Lord, designed to be the ultimate defense. It is nearly invulnerable and attacks with devastating power. The party must use all their skills and resources to defeat it."},
                        {"name": "The Corrupted Library", "difficulty": "medium", "description": "The party discovers the keep's library, which contains ancient knowledge about the Shadow Lord and how to defeat him. However, the books themselves have been corrupted and attack the party. The party must fight their way through to gain this crucial information."},
                        {"name": "The Hall of Mirrors", "difficulty": "hard", "description": "The party enters a hall filled with mirrors that show twisted reflections of themselves. These reflections are not just images - they are real, and they attack the party. The party must distinguish between reality and illusion while fighting for their lives."}
                    ]
                },
                {
                    "title": "The Depths",
                    ...
                    "encounters": [
                        {"name": "Trap Rooms", "difficulty": "medium", "description": "Elaborate trap rooms test the party's skills and teamwork. These rooms are designed to separate the party and force them to work together to solve puzzles and avoid deadly traps. The party must use all their abilities to survive."},
                        {"name": "Ancient Guardians", "difficulty": "hard", "description": "Ancient guardians, awakened by the darkness, stand as the final defense. These guardians were created long ago to protect the keep, but the darkness has corrupted them. They are powerful and nearly indestructible, but the party must find a way to defeat them."},
                        {"name": "The Shadow Lord's Minions", "difficulty": "boss", "description": "The most powerful minions of the Shadow Lord make their final stand. These are the Shadow Lord's most trusted servants, and they will fight to the death to protect their master. They are extremely powerful and work together as a coordinated unit."},
                        {"name": "The Corrupted Heart", "difficulty": "hard", "description": "The very heart of the keep has been corrupted, and must be cleansed before facing the final boss. The heart is a massive crystal that pulses with dark energy. The party must fight off waves of enemies while trying to cleanse the crystal. This is a race against time."},
                        {"name": "The Descent", "difficulty": "medium", "description": "The party must descend through a series of treacherous passages filled with obstacles and enemies. The descent is long and dangerous, testing the party's endurance and resolve. They must push forward despite the growing darkness."},
                        {"name": "The Final Preparation", "difficulty": "medium", "description": "Before facing the Shadow Lord, the party finds a chamber where they can rest and prepare. However, this chamber is not safe - it is filled with illusions and traps designed to weaken the party before the final battle. The party must be careful even in this moment of respite."}
                    ]
                }
            ]
            
            task3 = progress.add_task("[cyan]Generating campaign chapters...", total=len(chapters_config))
...
"""
        ...
        try:
            ...
            console.print(f"[green]   Estimated pages: ~{len(markdown_content.split()) // 250}[/green]")
            ...
            with open(log_path, 'w') as f:
                json.dump({
                    ...
                    "chapters": [{"title": c["title"], "encounters": c.get("encounters", [])} for c in self.chapters]
                }, f, indent=2)
