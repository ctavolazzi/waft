...
def generate_case_pdf(case_file_path: Path, output_path: Optional[Path] = None) -> Path:
    """Generate PDF binder from case file."""
    ...
    title = generate_headline_title(claim, verdict) if claim else f"Proof Case {case_id}" if case_id else "Proof Case"
    ...
    print(f"📄 Generating PDF binder from case file...")
    ...
    if output_path is None:
        ...
        output_path = case_file_path.parent / f"PROOF_CASE_{safe_title}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
