    ...
    try:
        ...
        if not hasattr(being, 'decision_fatigue') or being.decision_fatigue is None:
            ...
            if hasattr(being, 'personality_type'):
                modifiers = {"analytical": 5.0, "systematic": 5.0, "creative": -2.0, "intuitive": -2.0, "balanced": 0.0}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Migrate existing beings to include lifecycle attributes")
    parser.add_argument("--project-path", type=Path, default=Path.cwd(), help="Project root path")
    parser.add_argument("--dry-run", action="store_true", help="Don't actually migrate, just show what would be done")
