...
def gather_dossier_data(project_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        result = subprocess.run(
            ["git", "log", "--oneline", "-5"], capture_output=True, text=True, cwd=project_path
        )
    ...
    if work_efforts_dir.exists():
        ...
        recent_we = sorted(
            work_efforts_dir.glob("*.md"), key=lambda p: p.stat().st_mtime, reverse=True
        )[:10]
    ...
    if briefs_dir.exists():
        recent_briefs = sorted(
            briefs_dir.glob("*.pdf"), key=lambda p: p.stat().st_mtime, reverse=True
        )[:5]
...
def create_dossier(
    ...
    doc = BriefDocument(
        ...
        or {"role": "AUTHORIZED BY", "name": "Site-Delta-9", "date": data["date"]},
        ...
        chat_context={
            ...
            "key_decisions": [
                "Focus on effort cost and will to act",
