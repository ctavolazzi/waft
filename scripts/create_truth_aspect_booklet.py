    ...
    if booklet_path:
        ...
        if compile_result:
            ...
            if pdf_path.exists():
                ...
                print(f"   ⚠️  PDF not found after compilation")
                return 1
        else:
            print(f"   ⚠️  LaTeX compilation had issues (check logs)")
            return 1
    else:
        print(f"   ❌ Failed to create booklet")
        return 1
...
def create_latex_booklet(
    ...
    if not dnd_template_path.exists():
        # Clone DND template
        print(f"   📥 Cloning DND LaTeX template...")
        try:
            subprocess.run(
                ["git", "clone", "https://github.com/rpgtex/DND-5e-LaTeX-Template.git", str(dnd_template_path)],
                check=True,
                capture_output=True
            )
    ...
    if not dnd_sty.exists() or not dndbook_cls.exists():
        print(f"   ⚠️  DND template files not found")
        return None
...
def compile_latex(booklet_path: Path) -> bool:
    ...
    try:
        result = subprocess.run(
            ["pdflatex", "-interaction=nonstopmode", "-output-directory", str(booklet_path), str(tex_file)],
            cwd=booklet_path,
            capture_output=True,
            text=True
        )
        ...
        print(f"   ⚠️  pdflatex not found - install LaTeX (e.g., MacTeX, TeX Live)")
        return False
