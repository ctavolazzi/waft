...
class PDFOrganizer:
    ...
    def find_all_pdfs(self, exclude_patterns: Optional[List[str]] = None) -> List[Path]:
        """
...
"""
    ...
    generator = PDFGenerator.from_content(
        content=content,
        title="PDF Organization Process Documentation",
        style="clinical_standard"
    )
    
    # Save to reports directory
    output_path = get_storage_path(
        Path("_pantheon/library/reports/pdf_organization_report.pdf"),
        organizer.project_path
    )
...
def main():
    ...
    parser.add_argument(
        "--no-document",
        action="store_true",
        help="Skip scientific documentation"
    )
