...
def generate_pdf_examples():
    ...
    try:
        ...
        newsletter_tex = (
            project_root
            / "_work_efforts"
            / "WE-260116-7e6g_latex_newsletter_template_improvements"
            / "newsletter_template_improved.tex"
        )
        if newsletter_tex.exists():
            compiler = LaTeXCompiler(compiler="pdflatex")
            pdf_path = compiler.compile_file(
                newsletter_tex, output_dir / "newsletter_example.pdf", runs=2
            )
            console.print(f"✅ [green]PDF gerado:[/green] {pdf_path}")
    except Exception as e:
...
<body>
    <div class="container">
        ...
        <div class="grid">
            ...
            sum(1 for w in work_efforts if w["status"] == "completed")
        }</div>
            ...
            "".join(
                f'''
                    ...
                    '''
                for w in work_efforts[:10]
            )
            if work_efforts
            else '<p style="color: #666;">No work efforts from today</p>'
        }
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div style="margin-top: 20px;">
                    ...
                    {
            "".join(
                f'<div class="template-item"><strong>{k}:</strong> {v}</div>'
                for k, v in catalog_summary.get("by_type", {}).items()
            )
        }
                </div>
            </div>
