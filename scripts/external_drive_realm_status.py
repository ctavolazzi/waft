...
"""
    ...
    principles_table.add_row("Core Content → Local", "✅" if principles.get("core_content_local") else "❌")
    principles_table.add_row("Augmented Content → External", "✅" if principles.get("augmented_content_external") else "❌")
    principles_table.add_row("Fallback to Local", "✅" if principles.get("fallback_to_local") else "❌")
...
def register(
    ...
    project_name: str = typer.Option(None, "--project", "-p", help="Project name (auto-detected if not provided)")
):
    """
...
"""
        console.print(Panel(panel_content.strip(), title=f"Realm: {r['realm_name']}", border_style="cyan"))
...
@app.command()
def content(
    realm_name: str = typer.Argument(..., help="Realm name")
):
    """
