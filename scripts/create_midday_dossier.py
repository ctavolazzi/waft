                    ...
                    if index_file.exists():
                        try:
                            ...
                            for status_key, status_patterns in WORK_EFFORT_STATUS.items():
                                if any(f"status: {pattern}" in content_lower for pattern in status_patterns):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                if date_marker in line:
                    ...
                    if line.startswith(DEVLOG_MARKERS["date_prefix"]) and not line.startswith(date_marker):
                        break
                    current_entry.append(line)
                    if (DEVLOG_MARKERS["summary"] in line or 
                        DEVLOG_MARKERS["accomplishments"] in line):
...
def generate_abstract(status: Dict[str, Any], afternoon_focus: Optional[str] = None) -> str:
    ...
    if git_changed > 0:
        abstract_parts.append(
            ABSTRACT_TEMPLATES["branch_with_changes"].format(
                branch=html_module.escape(git_branch),
                count=git_changed
            )
        )
    else:
    ...
    if active_count > 0:
        abstract_parts.append(
            ABSTRACT_TEMPLATES["active_work_count"].format(count=active_count)
        )
    else:
...
def generate_summary(status: Dict[str, Any], afternoon_focus: Optional[str] = None) -> str:
    ...
    if git_changed > 0:
        summary_parts.append(
            ABSTRACT_TEMPLATES["files_modified"].format(count=git_changed)
        )
...
def main():
    ...
    warning_message = f"{REPORT_CONFIG['subtitle_prefix'].upper()} - {status['git_changed']} files changed"
    ...
    if status.get('active_work_efforts_count', 0) > 0:
        chat_context['recent_topics'].append(f"{status['active_work_efforts_count']} active work efforts")
    ...
    doc = BriefDocument(
        ...
        cover_warning={
            ...
            "severity": REPORT_CONFIG["warning_severity"]
        },
