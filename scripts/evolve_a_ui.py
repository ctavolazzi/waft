...
def infer_chat_context(work_efforts: List[Dict], recent_activity: Dict) -> Dict[str, Any]:
    ...
    if work_efforts:
        ...
        if primary:
            ...
            if any(word in title_lower or word in desc_lower for word in ["bug", "fix", "error", "issue"]):
                context["work_type"] = "bug_fix"
            elif any(word in title_lower or word in desc_lower for word in ["feature", "implement", "add", "create"]):
                context["work_type"] = "feature"
            elif any(word in title_lower or word in desc_lower for word in ["research", "study", "analyze", "investigate"]):
                context["work_type"] = "research"
            elif any(word in title_lower or word in desc_lower for word in ["refactor", "cleanup", "improve"]):
...
def generate_ui_html(context: Dict[str, Any], work_efforts: List[Dict], recent_activity: Dict, design_insights: Optional[Dict] = None) -> str:
