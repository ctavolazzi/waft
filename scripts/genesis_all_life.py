...
from waft.utils import detect_external_drive
...
def gather_chat_context() -> dict[str, Any]:
    ...
    try:
        ...
        if chat_being and chat_being.being:
            ...
            context["being_status"] = {"awakened": False, "note": "Chat Being not yet awakened"}
    except Exception as e:
        context["being_status"] = {"error": str(e), "note": "Could not get chat Being status"}
    ...
    try:
        ...
        if work_efforts_path.exists():
            ...
            work_effort_dirs = [
                d for d in work_efforts_path.iterdir() if d.is_dir() and d.name.startswith("WE-")
            ]
            ...
            for we_dir in work_effort_dirs[:5]:  # Last 5 work efforts
                ...
                if index_file.exists():
                    context["work_efforts"].append(
                        {"id": we_dir.name, "path": str(we_dir.relative_to(project_root))}
                    )
    except Exception as e:
...
def create_all_life_realm(easystore_path: Path, chat_context: dict[str, Any]) -> Path:
    ...
    console.print(
        Panel.fit(
            "[bold cyan]🌌 Creating All Life Realm[/bold cyan]\n"
    ...
    try:
        ...
        if existing:
            console.print("[yellow]⚠[/yellow]  All Life Realm already registered")
    ...
    # Register new realm
    console.print("[dim]→[/dim] Registering All Life Realm on EasyStore...")

    try:
        result = external_realm.register_realm(
            realm_name=realm_name, drive_name="Easystore", project_name="waft"
        )
...
"""
        ...
        try:
            ...
            console.print("[yellow]⚠[/yellow]  Waft command failed, using manual structure")
...
"""
    ...
    console.print("[green]✓[/green] All Life Realm structure verified\n")
...
def form_tether_to_the_one(
    all_life_path: Path, reality_id: str, being: Any, chat_context: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    console.print("[dim]   Realm: All_Life[/dim]")
...
def setup_autonomous_evolution_hub(
    ...
    hub_path.write_text(json.dumps(hub_config, indent=2), encoding="utf-8")
    ...
    console.print("[green]✓[/green] Hub configuration created")
    console.print(f"[dim]   Hub ID: {hub_config['hub_id']}[/dim]")
    console.print("[dim]   Autonomous Evolution: Enabled (not started)[/dim]")
    console.print(
        f"[dim]   Decision Autonomy: {'Enabled' if enable_decisions else 'Disabled'}[/dim]"
    )
    console.print(f"[dim]   Learning Loops: {'Enabled' if enable_learning else 'Disabled'}[/dim]")
    console.print("[dim]   Safety Verification: Enabled (Prime Directive: Safe Curiosity)[/dim]")
    console.print("[dim]   Status: Configured (use /start to begin)[/dim]\n")
...
def collect_observational_data(
    ...
    if EMPIRICA_AVAILABLE:
        try:
            ...
            data["empirica"] = {"available": True, "initialized": empirica_manager.is_initialized()}
        except Exception as e:
...
def main():
    ...
    if not easystore_path:
        raise SystemExit(
            "❌ Error: EasyStore drive not found. Please connect the drive and try again."
        )
