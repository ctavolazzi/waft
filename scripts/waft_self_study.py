...
class WAFTSelfStudy:
    ...
    def run_study(self, study_area: str | None = None, all_areas: bool = False) -> dict[str, Any]:
        ...
        if all_areas:
            ...
            if study_area not in study_areas:
                raise ValueError(
                    f"Unknown study area: {study_area}. Available: {list(study_areas.keys())}"
                )
            areas_to_study = [study_area]
        else:
            areas_to_study = list(study_areas.keys())  # Default: all areas

        results = {"timestamp": datetime.now().isoformat(), "studies": {}, "overall_health": {}}
        ...
        # Save study results
        results_file = (
            self.study_history_path
            / f"study_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def _run_area_specific_checks(
        self, area: str, proof_builder: ProofCaseBuilder
    ) -> dict[str, Any]:
    ...
    def _check_system_health(self, proof_builder: ProofCaseBuilder) -> dict[str, Any]:
        ...
        try:
            git_status = subprocess.run(
                ["git", "status", "--short"], capture_output=True, text=True, cwd=self.project_path
            ).stdout.strip()
            ...
            assumptions.append(
                {
                    ...
                    "risk": "low",
                    "status": "PROVEN"
                    if not git_status or "??" not in git_status
                    else "INCONCLUSIVE",
                    "confidence": 0.9,
                    "evidence": [
                        {
            ...
            assumptions.append(
                {
                    ...
                    "status": "INCONCLUSIVE",
                    "confidence": 0.0,
                    "evidence": [{"type": "error", "description": f"Git check failed: {e}"}],
                }
            )
            ...
            assumptions.append(
                {
                    ...
                    "status": "INCONCLUSIVE",
                    "confidence": 0.0,
                    "evidence": [{"type": "error", "description": f"Empirica check failed: {e}"}],
                }
            )
...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="Study all areas")
    parser.add_argument("--history", type=int, metavar="N", help="Show last N studies")
    parser.add_argument(
        "--compare", nargs=2, metavar=("STUDY1", "STUDY2"), help="Compare two study results"
    )
