...
class DSStore:
    ...
    def __init__(self, store):
        ...
        with self._get_block(self._superblk) as s:
            self._rootnode, self._levels, self._records, self._nodes, self._page_size = s.read(
                b">IIIII"
            )
    ...
    def open(cls, file_or_name, mode="r+", initial_entries=None):
        ...
        if mode == "w" or mode == "w+":
            ...
            if not initial_entries:
                ...
                with store.get_block(superblk) as s:
                    s.write(
                        b">IIIII", root, len(levels), len(initial_entries), node_count, page_size
                    )
    ...
    def _split(self, node, entry, right_ptr=0):
        ...
        with self._get_block(node) as block, self._get_block(new_right) as right_block:
            ...
            pivot = self._split2([block, right_block], entries, pointers, before, bool(next_node))[
                0
            ]
