...
class Alias:
    ...
    def _from_fd(cls, b):
        ...
        if version == 2:
            ...
            kind, voldate_hr, fstype, disktype, folder_cnid, cnid, crdate_hr, volattrs, reserved = (
        ...
        alias.volume = VolumeInfo(
            volname.decode().replace("/", ":"), voldate, fstype, disktype, volattrs, volfsid
        )
    ...
    def _to_fd(self, b):
        ...
        if self.volume.network_mount_info:
            b.write(
                struct.pack(b">hh", TAG_NETWORK_MOUNT_INFO, len(self.volume.network_mount_info))
            )
        ...
        if self.target.user_home_prefix_len is not None:
            b.write(
                struct.pack(
                    b">hhh", TAG_USER_HOME_LENGTH_PREFIX, 2, self.target.user_home_prefix_len
                )
            )
