...
class Bookmark:
    ...
    def _encode_item(cls, item, offset):
        if item is True:
            ...
            if item.base:
                ...
                result = b"".join(
                    [
                        struct.pack(b"<IIII", 8, BMK_URL | BMK_URL_ST_RELATIVE, baseoff, reloff),
                        baseenc,
                        relenc,
                    ]
                )
    ...
    def to_bytes(self):
        ...
        for ndx, toc in enumerate(tocs):
            ...
            result.append(
                struct.pack(b"<IIIII", len(data) - 8, 0xFFFFFFFE, tid, next_offset, len(data) // 12)
            )
            result.append(data)
        ...
        header = struct.pack(
            b"<4sIIIQQQQI", b"book", offset + 48, 0x10040000, 48, 0, 0, 0, 0, first_toc_offset
        )
