...
from src.waft.evolution.styling_genome import StylingGene, StylingGenome
...
class TestResult:
    ...
    error: str | None = None
    idea_gene: IdeaGene | None = None
    evolution_event: EvolutionaryEvent | None = None
...
class TestSuite:
            ...
            json.dump(
                {
                    ...
                    "idea_scientific_name": result.idea_gene.scientific_name
                    if result.idea_gene
                    else None,
                    "event_genome_id": result.evolution_event.genome_id
                    if result.evolution_event
                    else None,
        ...
        try:
            ...
            if not test_pdf.exists():
                ...
                try:
                    ...
                    for i, color in enumerate(colors):
                        ...
                        y = 500
                        draw.rectangle(
                            [x, y, x + 400, y + 400], fill=color, outline="#2c3e50", width=8
                        )
                        label = f"Section {i + 1}"
                        if font_medium:
                            draw.text(
            ...
            if self.metrics_db:
                ...
                duration = time.time() - (
                    ...
                    if hasattr(result.timestamp, "timestamp")
                    else time.time()
                )
    ...
    def phase2_png_to_pdf(self) -> list[TestResult]:
        ...
        try:
            ...
            phase1_pngs = list(
                (self.test_results_dir / "pdf_to_png" / "phase1_test_001").glob("*.png")
            )

            if not phase1_pngs:
                print("  ⚠️  No PNGs from Phase 1, creating test PNGs...")
                ...
                try:
                    ...
                    if font_medium:
                        draw.text(
                            (1275, 250), subtitle, fill="#ecf0f1", font=font_medium, anchor="mm"
                        )
                    else:
                    ...
                    for i, color in enumerate(colors):
                        ...
                        y = 500
                        draw.rectangle(
                            [x, y, x + 350, y + 350], fill=color, outline="#34495e", width=5
                        )
                        if font_small:
                            draw.text(
                                (x + 175, y + 175),
                    ...
                    if font_small:
                        draw.text(
                            (1275, 3150), footer_text, fill="white", font=font_small, anchor="mm"
                        )
                    else:
                    ...
                    print("  ⚠️  PIL not available, skipping PNG creation")
                    result = TestResult(
                        test_id=test_id,
                        phase=2,
                        test_name=test_name,
            ...
            pdf_path = convert_images_to_pdf(
                phase1_pngs, output_pdf, page_size=(8.5, 11.0), dpi=300
            )
...
"""
            ...
            print(
                f"  → Ideas: {len(distilled.ideas)}, Pages: {metrics['page_count']}, Fitness: {metrics['fitness']:.3f}"
            )
...
"""
            ...
            if pdf_path_actual and Path(pdf_path_actual).exists():
                png_paths = convert_pdf_to_images(
                    Path(pdf_path_actual), output_dir=png_dir, dpi=300
                )
            ...
            metrics = {
                ...
                "pdf_generated": pdf_path_actual is not None and Path(pdf_path_actual).exists()
                if pdf_path_actual
                else False,
            ...
            success = (
                metrics["distill_success"]
                and metrics["generate_success"]
                and (
                    metrics["convert_success"] or not metrics["pdf_generated"]
                )  # PNG optional if no PDF
            )
            ...
            print(f"  {'✓' if success else '✗'} Success: {success}")
            print(
                f"  → Distill: {metrics['distill_success']}, Generate: {metrics['generate_success']}, PDF: {metrics['pdf_generated']}, Convert: {metrics['convert_success']}"
            )
            ...
            if phase_results:
                if self.metrics_db:
                    ...
                    phase_stats = {
                        ...
                        "success_rate": phase_success / len(phase_results)
                        if phase_results
                        else 0.0,
                    }
                if self.formatter:
            ...
            json.dump(
                {
                    ...
                    "phase_stats": {
                        str(p): (self.metrics_db.get_phase_stats(p) if self.metrics_db else {})
                        for p in [1, 2, 3, 4]
                    },
...
def main():
    ...
    parser.add_argument(
        "--research-dir", type=Path, default=Path(__file__).parent, help="Research directory path"
    )
