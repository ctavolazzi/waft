...
class ImageFetcher:
    ...
    def _get_cached_image(self, cache_key: str) -> Path | None:
    ...
    def _download_from_pexels(self, query: str, width: int, height: int) -> Path | None:
        """
    ...
    def _download_from_unsplash(self, query: str, width: int, height: int) -> Path | None:
        """
    ...
    def get_multiple_images(self, queries: list, width: int = 800, height: int = 600) -> list:
        """
        Get multiple images.
...
def create_test_image_with_photo(
    ...
    draw.rectangle(
        [100, content_y, width - 100, height - 200], fill="white", outline="#ddd", width=3
    )
