...
from waft.metrics import Phase, Quest, PlayerStats, prioritize_phases
...
def print_quest_analysis(quest: Quest):
    ...
    if evolution is not None:
        ...
        print(f"  🎉 UNLOCKS: The Architect evolution!")
    ...
    for i, phase in enumerate(quest.phases):
        ...
        print(f"\n{phase.name}")
        print(f"  Scint: {phase.scint_cost} → {phase.scint_earned} (net: {phase.net_scint():+d}, ROI: {phase.roi():.2f}x)")
        print(f"  Karma: {phase.karma_impact:+d} ☯️")
        print(f"  Integrity Risk: {phase.integrity_risk} 💚 ({phase.integrity.risk_level.value})")
        print(f"  Cognitive Load: {phase.cognitive_load} 🧠 ({phase.cognitive.complexity_level.value})")
        ...
        if i == evolution:
            print(f"  🎉 EVOLUTION TRIGGERED! (The Architect)")
