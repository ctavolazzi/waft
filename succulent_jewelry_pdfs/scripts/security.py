...
def sanitize_content(content: str, max_size: int = 10 * 1024 * 1024) -> str:
    ...
    if len(content_bytes) > max_size:
        raise ValueError(f"Content exceeds maximum size: {len(content_bytes)} bytes (max: {max_size})")
...
def validate_image_path(image_path: str, project_root: Path) -> Optional[Path]:
    """
    ...
    try:
        ...
        if validated.exists() and validated.suffix.lower() in ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg']:
            return validated
...
def validate_metadata(title: str, topic: Optional[str] = None, allowed_topics: Optional[list] = None) -> dict:
    """
