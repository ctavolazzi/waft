...
def generate_product_description(title: str, topic: Optional[str] = None, config: Optional[dict] = None) -> str:
    ...
    description = template.format(
        title=title,
        topic=topic or 'succulent jewelry',
...
def generate_product_metadata(pdf_path: Path, title: Optional[str] = None, topic: Optional[str] = None) -> Dict:
...
def prepare_gumroad_products(pdf_dir: Path, output_file: Optional[Path] = None) -> Dict:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Prepare PDFs for Gumroad upload'
    )
    parser.add_argument(
        '--pdf-dir',
        type=Path,
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"Gumroad Preparation Complete")
        print(f"{'='*60}")
