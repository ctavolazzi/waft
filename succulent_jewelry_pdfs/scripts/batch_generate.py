...
def batch_generate(manifest_path: Path, output_dir: Path | None = None) -> dict:
    """
    ...
    # Process each guide
    results = {"total": len(guides), "success": 0, "failed": 0, "results": []}
        ...
        try:
            ...
            if success:
                ...
                results["results"].append(
                    {"title": title, "status": "success", "output": str(output_path)}
                )
            else:
                results["failed"] += 1
                results["results"].append(
                    {"title": title, "status": "failed", "error": "Generation failed"}
                )
            ...
            results["failed"] += 1
            results["results"].append(
                {"title": guide_data.get("title", "Unknown"), "status": "error", "error": str(e)}
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Batch generate guide PDFs from manifest file")
    parser.add_argument("--manifest", type=Path, required=True, help="Path to manifest file (JSON)")
    parser.add_argument(
