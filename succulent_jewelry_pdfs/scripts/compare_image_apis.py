...
from scripts.image_api import PixabayAPI, PexelsAPI, get_placeholder_image
...
def test_query(
    ...
    try:
        pixabay = PixabayAPI()
        pixabay_result = pixabay.search_images(query, per_page=3, min_width=width, min_height=height)
...
def main():
    ...
    total_pixabay = sum(r["pixabay"].get("total_hits", 0) for r in results if r["pixabay"].get("status") == "success")
    total_pexels = sum(r["pexels"].get("total_results", 0) for r in results if r["pexels"].get("status") == "success")
