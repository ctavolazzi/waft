...
class PixabayAPI(ImageAPI):
    ...
    def __init__(self, api_key: str | None = None):
        ...
        self.api_key = (
            api_key
            or _ENV_VARS.get("PIXABAY_API_KEY")
            or os.getenv("PIXABAY_API_KEY")
            or "29486486-de7f8c25dff5fd83f7b7b41a0"
        )
    ...
    def get_image_url(
        ...
        **kwargs,
        ...
        result = self.search_images(
            query=query, per_page=1, min_width=width, min_height=height, **kwargs
        )
...
class PexelsAPI(ImageAPI):
    ...
    def __init__(self, api_key: str | None = None):
        ...
        self.api_key = api_key or _ENV_VARS.get("PEXELS_API_KEY") or os.getenv("PEXELS_API_KEY")
        if not self.api_key:
            logger.warning(
    ...
    def get_image_url(
        self, width: int = 800, height: int = 600, query: str = "nature", **kwargs
    ) -> str:
        """
...
def get_placeholder_image(
    width: int = 800, height: int = 600, provider: str = "picsum", fallback: bool = True, **kwargs
) -> str:
    """
    ...
    if provider.lower() == "pixabay":
        ...
        if fallback and url.startswith("https://picsum"):
            try:
                ...
                pixabay_url = pixabay_api.get_image_url(
                    width, height, query=query, size="large", **kwargs
                )
