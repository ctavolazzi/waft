...
def experiment_session_id_uniqueness():
    ...
    print(f"  Controlled variables: tmpdir, LLM config, max_iterations=1")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(N):
            ...
            answer, protocol = guide.solve(
                problem_statement=f"Experiment 1 session {i}",
                max_iterations=1
            )
            t1 = time.time()
        ...
        if uniqueness_rate == 100.0:
            print(f"  ✅ HYPOTHESIS ACCEPTED: 100% uniqueness achieved")
            print(f"  ❌ NULL HYPOTHESIS REJECTED: No collisions detected")
            return True, {
            ...
            print(f"  ✅ NULL HYPOTHESIS ACCEPTED: Collisions occurred")
            return False, {
                'uniqueness_rate': uniqueness_rate,
                'total': len(session_ids),
                'unique': unique_count,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for iteration_count in iteration_counts:
            ...
            for trial in range(trials_per_count):
                start = time.time()
                answer, protocol = guide.solve(
                    problem_statement=f"Scaling test",
            ...
            print(f"  {iteration_count:2d} iterations: {mean_time*1000:6.2f}ms ± {stdev_time*1000:5.2f}ms ({time_per_iter*1000:.3f}ms/iter)")
        ...
        r = numerator / (denominator_x * denominator_y) ** 0.5 if denominator_x > 0 and denominator_y > 0 else 0
        r_squared = r ** 2
        ...
        if r_squared > 0.95:
            ...
            print(f"  ❌ NULL HYPOTHESIS REJECTED: Scaling is linear")
            return True, {
                'r_squared': r_squared,
                'slope': slope,
            ...
            print(f"  ✅ NULL HYPOTHESIS ACCEPTED: Non-linear scaling detected")
            return False, {
                'r_squared': r_squared,
                'slope': slope
            }
...
def experiment_concurrent_safety():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def worker(thread_id):
            ...
            try:
                ...
                for i in range(sessions_per_thread):
                    try:
                        answer, protocol = guide.solve(
                            problem_statement=f"Thread {thread_id} session {i}",
                            max_iterations=1
                        )
        ...
        if error_rate == 0 and corruption_rate == 0 and uniqueness_rate == 100.0:
            print(f"  ✅ HYPOTHESIS ACCEPTED: Zero errors, zero corruption, 100% uniqueness")
            print(f"  ❌ NULL HYPOTHESIS REJECTED: Concurrent access is safe")
            return True, {
            ...
            print(f"  ❌ HYPOTHESIS REJECTED: Detected issues in concurrent access")
            print(f"  ✅ NULL HYPOTHESIS ACCEPTED: Race conditions exist")
            return False, {
                'error_rate': error_rate,
                'corruption_rate': corruption_rate,
                'uniqueness_rate': uniqueness_rate,
