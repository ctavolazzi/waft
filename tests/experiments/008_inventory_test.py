...
async def run_experiment():
    ...
    biome = Biome(biome_id="biome_008", project_path=project_root, abiotic_factors=abiotic)
    ...
    scint_shard = Item.create(
        name="Scint-Shard", weight=1, properties={"type": "energy_crystal", "power": 10}
    )
    ...
    grabbed = rishi.grab(
        scint_shard.item_id, target_slot="appendage", dish=dish, position=item_position
    )
    assert grabbed, "Item should be grabbed"

    assert len(rishi.state.appendage) == 1, "Appendage should contain 1 item"
    assert rishi.state.appendage[0].get("item_id") == scint_shard.item_id, (
        "Appendage should contain Scint-Shard"
    )
    ...
    print("✓ Item grabbed into Appendage")
    print(f"  Appendage: {len(rishi.state.appendage)} items")
    print(f"  Pocket: {len(rishi.state.pocket)} items")
    ...
    assert rishi.state.pocket[0].get("item_id") == scint_shard.item_id, (
        "Pocket should contain Scint-Shard"
    )

    print("✓ Item stowed to Pocket")
    print(f"  Appendage: {len(rishi.state.appendage)} items")
    print(f"  Pocket: {len(rishi.state.pocket)} items")
    ...
    print("✓ Item retrieved to Appendage")
    print(f"  Appendage: {len(rishi.state.appendage)} items")
    print(f"  Pocket: {len(rishi.state.pocket)} items")
    print()
    ...
    print("✓ Capacity constraints verified")
    print("  Pocket capacity: 3/3 (FULL)")
    print("  Attempted to grab 4th item: FAILED (as expected)")
    print()
    ...
    if rishi.state.appendage:
        for item_dict in rishi.state.appendage:
            print(
                f"    - {item_dict.get('name', 'Unknown')} (ID: {item_dict.get('item_id', '')[:8]}...)"
            )
    print(f"  Pocket: {len(rishi.state.pocket)} items")
    ...
    print("RESULTS:")
    print(f"  ✓ Organism birthed: {rishi.scientific_name}")
    print("  ✓ Item grabbed from lattice to Appendage")
    print("  ✓ Item stowed from Appendage to Pocket")
    print("  ✓ Item retrieved from Pocket to Appendage")
    print("  ✓ Capacity constraints enforced (Appendage: 1, Pocket: 3)")
    print()
