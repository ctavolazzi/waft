...
from src.waft.core.agent import AgentConfig, BaseAgent, Modification
...
async def run_experiment():
    ...
    assert genesis.lineage_path[0] == genesis.genome_id, (
        "Genesis lineage path should start with its genome_id"
    )
    assert len(genesis.flight_recorder) >= 1, (
        "Genesis should have at least one event (genesis spawn)"
    )
    ...
    assert child.generation == genesis.generation + 1, (
        f"Child generation should be {genesis.generation + 1}"
    )
    assert child.parent_id == genesis.genome_id, "Child parent_id must match parent's genome_id"
    assert len(child.lineage_path) == len(genesis.lineage_path) + 1, (
        "Child lineage should extend parent's"
    )
    assert child.lineage_path[0] == genesis.lineage_path[0], (
        "Child lineage should start with genesis"
    )
    assert child.lineage_path[-1] == child.genome_id, (
        "Child lineage should end with its own genome_id"
    )
    ...
    print(
        f"  ✓ Flight Recorder active: {len(genesis.flight_recorder) + len(child.flight_recorder)} events"
    )
