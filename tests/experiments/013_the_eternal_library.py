...
async def run_experiment():
    ...
    biome = Biome(
        biome_id="biome_013_eternal_library", project_path=project_root, abiotic_factors=abiotic
    )
    ...
    for i, pos in enumerate(positions):
        ...
        if added:
            ...
            print(
                f"  Symbol: {organism.state.anatomical_symbol} ({organism.state.anatomical_archetype})"
            )
        else:
    ...
    try:
        display = PygameBiomeEngine(
            width=1200, height=800, cell_size=20, title="WAFT Biome Engine - Experiment 013"
        )
    ...
    if first_birth:
        ...
        print(
            f"Symbol: {first_birth.state.anatomical_symbol} ({first_birth.state.anatomical_archetype})"
        )
