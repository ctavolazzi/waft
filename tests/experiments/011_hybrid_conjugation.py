...
from src.waft.core.agent import AgentConfig, BaseAgent
from src.waft.core.agent.anatomy import AnatomicalArchetype
from src.waft.core.hub import TheReaper, TheSlicer
...
async def run_experiment():
    ...
    biome = Biome(biome_id="biome_011", project_path=project_root, abiotic_factors=abiotic)
    ...
    print(
        f"  Appendage Capacity: {AnatomicalArchetype.get_appendage_capacity(parent_a.state.anatomical_symbol)}"
    )
    print(
        f"  Pocket Capacity: {AnatomicalArchetype.get_pocket_capacity(parent_a.state.anatomical_symbol)}"
    )
    ...
    print(
        f"  Appendage Capacity: {AnatomicalArchetype.get_appendage_capacity(parent_b.state.anatomical_symbol)}"
    )
    print(
        f"  Pocket Capacity: {AnatomicalArchetype.get_pocket_capacity(parent_b.state.anatomical_symbol)}"
    )
    ...
    for pulse in range(2, 8):  # Pulses 2-7 (seed created at pulse 1)
        ...
        if seed_id in seed_parent.state.gestation_pulses:
            ...
            print("  Gestation: Seed removed or completed")
    ...
    # Verify lineage
    assert child.parent_id == parent_a.genome_id, (
        "Child should reference parent_a as primary parent"
    )
    ...
    assert child_pos != position_a and child_pos != position_b, (
        "Child should be in different position"
    )

    print("✓ Child Genome: Unique (derived from both parents)")
    ...
    print(
        f"  Culture: Hybrid (from {parent_a.scientific_name.split()[0]} + {parent_b.scientific_name.split()[0]})"
    )
    print()
    ...
    print()
    print("RESULTS:")
    print("  ✓ Anatomical archetypes assigned deterministically")
    print("  ✓ Conjugation successful (proximity + metabolic surplus verified)")
    print("  ✓ Developing Seed created and gestated for 5 pulses")
    print("  ✓ Child spawned with hybrid genome and name")
    print("  ✓ Symbol inheritance functional")
    print()
    print("FAMILY CERTIFICATE:")
    print(f"  Parent A: {parent_a.state.anatomical_symbol} {parent_a.scientific_name}")
    print(f"  Parent B: {parent_b.state.anatomical_symbol} {parent_b.scientific_name}")
...
if __name__ == "__main__":
    ...
    print("\nExperiment Results:")
    print(
        f"  Parent A: {results['parent_a'].state.anatomical_symbol} {results['parent_a'].scientific_name}"
    )
    print(
        f"  Parent B: {results['parent_b'].state.anatomical_symbol} {results['parent_b'].scientific_name}"
    )
