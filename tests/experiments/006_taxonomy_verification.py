...
async def run_experiment():
    ...
    biome = Biome(biome_id="biome_006", project_path=project_root, abiotic_factors=abiotic)
    ...
    for organism in organisms:
        ...
        assert name1 == name2 == name3, (
            f"Name should be deterministic for {organism.state.agent_id}"
        )
    ...
    for i, organism in enumerate(organisms):
        ...
        assert density > 0, "Organism should sense population density > 0"
        assert (
            organism.state.working_memory.get("social_context", {}).get("total_population") == 3
    ...
    genesis_events = [
        e
        for e in events
        if e.get("event_type") == "spawn"
        and e.get("payload", {}).get("event") == "genesis"
        and e.get("agent_id") in organism_ids
    ]
    ...
    for organism in organisms:
        ...
        event = next(
            (e for e in genesis_events if e.get("agent_id") == organism.state.agent_id), None
        )

        if event:
            scientific_name = (
                event.get("scientific_name")
                or event.get("payload", {}).get("scientific_name")
                or LineagePoet.generate_name(event.get("genome_id", ""))
            )
    ...
    print()
    print("RESULTS:")
    print("  ✓ 3 Genesis organisms birthed")
    print("  ✓ All have distinct scientific names")
    print("  ✓ Names are deterministic (same genome = same name)")
    print("  ✓ Quorum sensing functional (neighbor_count, population_density)")
