...
async def run_experiment():
    ...
    biome = Biome(biome_id="biome_007", project_path=project_root, abiotic_factors=abiotic)
    ...
    for i, name in enumerate(scientific_names, 1):
        ...
        if len(title_words) > 1:
            ...
            assert title_words[1][0].isupper(), (
                f"Title word should be capitalized: {title_words[1]}"
            )

        print(f"✓ Name {i}: {name}")
        print("  Format: Valid (Genus Species, Title)")
        print("  Capitalization: Correct")
    ...
    print()
    print("RESULTS:")
    print("  ✓ 10 Genesis organisms birthed")
    print("  ✓ All names properly capitalized (Genus Species, Title)")
