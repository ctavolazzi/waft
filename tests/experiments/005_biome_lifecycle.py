...
class TestOrganism(BaseAgent):
    ...
    async def act(self, decision):
        ...
        if decision.get("action") == "breach_membrane":
            ...
            return {"result": "breach_attempted", "action": decision}
...
async def run_experiment():
    ...
    biome = Biome(biome_id="biome_001", project_path=project_root, abiotic_factors=abiotic)
    ...
    print("✓ Genesis Organism Birthed")
    ...
    if slice_result_breach.get("actions", {}).get("act", {}).get("result") == "breach_attempted":
        ...
        assert breach_organism.state.agent_id not in dish.organisms, (
            "Breach organism should be removed"
        )
