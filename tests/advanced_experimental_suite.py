...
def experiment_endurance_testing():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for batch_num in range(num_batches):
            ...
            print(f"  Batch {batch_num + 1:2d}: {mean_time*1000:.3f}ms ± {stdev_time*1000:.3f}ms "
        ...
        print(f"  Mean throughput: {statistics.mean([r['throughput'] for r in batch_results]):.1f} sess/s")

        print(f"\nTREND ANALYSIS:")
        if is_stable:
            print(f"  ✅ STABLE: Performance change within ±10% threshold")
        else:
            if percent_change > 0:
        ...
        print(f"\nHYPOTHESIS TEST:")
        if is_stable:
            print(f"  ✅ HYPOTHESIS ACCEPTED: No significant performance degradation")
            return True, {
...
def experiment_memory_profiling():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(num_sessions):
            ...
            if (i + 1) % sample_interval == 0:
                ...
                print(f"  Session {i+1:3d}: {current_mb:.2f} MB (Δ{current_mb - initial_memory:+.2f} MB)")
    ...
    if len(memory_samples) >= 2:
        ...
        if len(warmup_samples) >= 2:
            ...
            print(f"  Memory per session (after warmup): {memory_per_session*1000:.3f} KB/session")
            ...
            print(f"\nHYPOTHESIS TEST:")
            if not is_leaking:
                print(f"  ✅ HYPOTHESIS ACCEPTED: Memory usage is stable")
                return True, {
                ...
                print(f"  ❌ HYPOTHESIS REJECTED: Memory leak detected")
                return False, {
                    'memory_per_session_kb': memory_per_session * 1000,
                    'leak_rate': 'linear'
                }
...
def experiment_breaking_point():
            ...
            def worker(tid):
                try:
                    ...
                    for i in range(sessions_per_thread):
                        answer, protocol = guide.solve(
                            problem_statement=f"Load test T{tid} S{i}",
                            max_iterations=1
                        )
                    return True, None
    ...
    for num_threads in test_loads:
        ...
        print(f"    Success rate: {result['success_rate']:.1f}% ({result['successes']}/{result['threads']})")
    ...
    if max_successful >= 50:
        ...
        return False, {
            'max_threads': max_successful,
            'breaking_point': next((r['threads'] for r in results if r['success_rate'] < 100.0), None)
        }
...
def experiment_statistical_distributions():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(num_samples):
            t0 = time.time()
            answer, protocol = guide.solve(
                problem_statement=f"Distribution test {i}",
                max_iterations=1
            )
            t1 = time.time()
        ...
        print(f"\nINTERPRETATION:")
        if abs(skewness) < 0.5:
            print(f"  Skewness: Approximately symmetric")
        elif abs(skewness) < 1:
            print(f"  Skewness: Moderately skewed")
        else:
            print(f"  Skewness: Highly skewed")

        if abs(kurtosis) < 0.5:
            print(f"  Kurtosis: Approximately normal (mesokurtic)")
        elif kurtosis > 0.5:
            print(f"  Kurtosis: Heavy-tailed (leptokurtic)")
        else:
            print(f"  Kurtosis: Light-tailed (platykurtic)")
        ...
        print(f"\nHYPOTHESIS TEST:")
        if is_normal:
            print(f"  ✅ HYPOTHESIS ACCEPTED: Approximately normal distribution")
            return True, {
                'mean': mean_time,
                'median': median_time,
                'stdev': stdev_time,
            ...
            print(f"  ❌ HYPOTHESIS REJECTED: Distribution is not normal")
            return False, {
                'skewness': skewness,
                'kurtosis': kurtosis,
        ...
        for k, v in results.items():
            if v['hypothesis_accepted'] is not None:
                serializable[k] = {
                    ...
                    'data': {
                        dk: (float(dv) if isinstance(dv, (int, float)) and not isinstance(dv, bool) else
                             [float(x) if isinstance(x, (int, float)) else x for x in dv] if isinstance(dv, list) else
                             str(dv))
