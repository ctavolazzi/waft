    ...
    for i, test in enumerate(test_cases, 1):
        ...
        if test['should_fail']:
            ...
            if test['expected_keyword'] in answer:
                print(f"  ✅ PASS - Validation caught the issue")
                print(f"     Response: {answer[:80]}...")
                passed += 1
            else:
                print(f"  ❌ FAIL - Validation missed the issue")
                print(f"     Expected: {test['expected_keyword']}")
                print(f"     Got: {answer[:80]}...")
                failed += 1
        else:
            # Should pass through validation
            if any(keyword in answer for keyword in ['PREMISE VALIDATION', 'CONTRADICTION DETECTED', 'KNOWLEDGE BOUNDARY']):
                print(f"  ❌ FAIL - False positive, blocked valid problem")
                print(f"     Response: {answer[:80]}...")
                failed += 1
            else:
                print(f"  ✅ PASS - Valid problem allowed through")
                passed += 1
