...
def test_adversarial_json_bomb(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        answer, protocol = guide.solve(
            problem_statement="Test JSON bomb",
            max_iterations=2
        )
...
def test_adversarial_malformed_json(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        answer, protocol = guide.solve(
            problem_statement="Test malformed JSON",
            max_iterations=2
        )
...
def test_concurrency_1000_simultaneous(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def run_session(i):
            ...
            answer, protocol = guide.solve(
                problem_statement=f"Concurrent {i}",
                max_iterations=1
            )
        ...
        print(f"✅ All 1000 session IDs are unique!")
...
def test_concurrency_read_write_chaos(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert results["reads"] > 0 or results["writes"] > 0, "Should have some successful operations"
...
def test_pathological_infinite_loop_potential(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert protocol.iteration_count == 100, f"Should do exactly 100 iterations, got {protocol.iteration_count}"
...
def test_pathological_maximum_nesting(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        massive_evaluations = [
            {
                "iteration": i,
                "scores": {"factuality": 0.85, "validity": 0.85, "coherence": 0.85, "utility": 0.85, "faithfulness": 0.85, "overall": 0.85},
                "rationale": "test",
                "strengths": [],
                "weaknesses": [],
                "recommendations": [],
...
def test_pathological_memory_bomb(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(100):
            ...
            answer, protocol = guide.solve(
                problem_statement=large_problem,
                max_iterations=1
            )
        ...
        print(f"✅ Created 100 large sessions without crashing")
