...
def test_float_precision_negative_zero(runner: TestRunner):
    ...
    scores = EvaluationScores(
        factuality=-0.0, validity=0.0, coherence=-0.0, utility=0.0, faithfulness=-0.0, overall=0.0
    )

    assert scores.factuality == 0.0, "Should handle -0.0"
    print("Handled negative zero correctly")
...
def test_float_precision_quality_threshold_edge(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        guide = TheGuide(
            project_path=Path(tmpdir), client_llm=PreciseLLM(), guide_llm_config={"model": "test"}
        )
        ...
        answer, protocol = guide.solve(
            problem_statement="Float precision test", max_iterations=5, quality_threshold=0.85
        )

        # Should terminate early (score >= threshold)
        print(f"Iterations: {protocol.iteration_count}, Quality: {protocol.quality_score:.10f}")
        assert protocol.iteration_count <= 2, "Should terminate early with exact threshold match"
...
def test_time_clock_skew_microseconds(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        session_ids = []
        for i in range(1000):
            answer, protocol = guide.solve(problem_statement=f"Speed test {i}", max_iterations=1)
        ...
        print("✅ 1000 rapid sessions, all unique IDs")
...
def test_encoding_every_encoding(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for test_str in test_strings:
            answer, protocol = guide.solve(problem_statement=test_str, max_iterations=1)
            # Should be able to retrieve
            retrieved = guide.get_protocol(protocol.session_id)
            assert retrieved is not None, (
                f"Should retrieve protocol with encoding: {repr(test_str)}"
            )
...
def test_encoding_latin1_vs_utf8(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        answer, protocol = guide.solve(problem_statement=latin1_chars, max_iterations=1)
...
def test_thread_safety_shared_guide_instance(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def run_session(i):
            try:
                answer, protocol = guide.solve(problem_statement=f"Thread {i}", max_iterations=1)
...
def test_thread_safety_index_corruption(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def writer(i):
            ...
            answer, protocol = guide.solve(problem_statement=f"Index test {i}", max_iterations=1)
...
def test_filesystem_readonly_directory(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            answer2, protocol2 = guide.solve(problem_statement="Test 2", max_iterations=1)
...
def test_resource_exhaustion_file_descriptors(runner: TestRunner):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(100):
            answer, protocol = guide.solve(problem_statement=f"FD test {i}", max_iterations=1)
...
def run_all_tests():
    ...
    runner.run_test(
        "Quality Threshold Float Precision", test_float_precision_quality_threshold_edge, runner
    )
