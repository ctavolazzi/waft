...
def test_cors_headers_present(client):
    ...
    response = client.get("/api/decision/health", headers={"Origin": "http://localhost:5173"})
...
def test_cors_preflight_request(client):
    ...
    assert (
        "localhost:5173" in allow_origin or "localhost:3000" in allow_origin or allow_origin == "*"
    )


def test_cors_allows_react_dev_server(client):
    """Test that React dev server (localhost:3000) is allowed."""
    response = client.get("/api/decision/health", headers={"Origin": "http://localhost:3000"})
...
def test_cors_credentials_allowed(client):
    ...
    assert (
        "true" in str(allow_credentials).lower() or response.status_code == 200
    )  # If middleware is working, this is fine
...
def test_actual_post_request_with_cors(client):
    ...
    request_data = {
        ...
        "scores": {"Option A": {"Cost": 10, "Quality": 5}, "Option B": {"Cost": 5, "Quality": 10}},
    }

    response = client.post(
        "/api/decision/analyze", json=request_data, headers={"Origin": "http://localhost:5173"}
    )
