...
def test_missing_keys():
    ...
    # Missing 'criteria'
    data = {"alternatives": ["Option A"], "scores": {"Option A": {"Cost": 10}}}

    with pytest.raises(ValueError, match="Missing required keys"):
        InputTransformer.transform_input(data)

    # Missing 'scores'
    data = {"alternatives": ["Option A"], "criteria": {"Cost": 1.0}}
...
def test_invalid_number_strings():
    ...
    data = {
        "alternatives": ["Option A"],
        "criteria": {"Cost": 1.0},
        "scores": {"Option A": {"Cost": "not-a-number"}},
    }
...
def test_alternative_dict_format():
    ...
    data = {
        ...
        "criteria": {"Cost": 1.0},
        "scores": {"Option A": {"Cost": 10}, "Option B": {"Cost": 5}},
    }
