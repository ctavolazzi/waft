...
class PDFBinderOrganizer:
    ...
    def extract_metadata(self, pdf_paths: List[Path]) -> List[PDFMetadata]:
        ...
        with Progress(
            ...
            for pdf_path in pdf_paths:
                try:
                    ...
                    console.print(f"  [yellow]⚠️[/yellow]  Error extracting metadata from {pdf_path.name}: {e}")
        ...
        console.print(f"  [green]✅[/green] Extracted metadata from [bold]{len(metadata_list)}[/bold] PDFs\n")
    ...
    def organize_into_booklets(self, pdfs: List[PDFMetadata]) -> List[BookletInfo]:
        ...
        for pdf in pdfs:
            ...
            if current_pages + pdf.page_count > MAX_PAGES_PER_BOOKLET and current_booklet.pdfs:
                ...
                current_booklet.description = f"Contains {len(current_booklet.pdfs)} PDFs, {current_pages} pages"
    ...
    def create_full_binder(self, pdfs: List[PDFMetadata]) -> Path:
        ...
        console.print(f"  [cyan]📄[/cyan] Generating full binder...")
        binder.generate(full_binder_path, include_dividers=True)
        
        size_mb = full_binder_path.stat().st_size / (1024 * 1024)
        console.print(f"  [green]✅[/green] Full binder created: [bold]{full_binder_path.name}[/bold]")
    ...
    def organize(self) -> Dict[str, Any]:
        ...
        summary_table.add_row("PDFs Found", str(len(pdfs)), f"{sum(p.page_count for p in pdfs)} pages")
        summary_table.add_row("Booklets Created", str(len(booklets)), f"Max {MAX_PAGES_PER_BOOKLET} pages each")
