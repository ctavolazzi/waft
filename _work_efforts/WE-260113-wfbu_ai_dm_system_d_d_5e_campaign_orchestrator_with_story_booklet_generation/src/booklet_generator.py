...
class BookletGenerator:
    ...
    def __init__(self, config: BookletConfig):
        ...
        self.config = config
        self.data_structure: DataStructure | None = None

    def generate_from_data(self, data: Any, output_path: Path | None = None) -> Path:
        """
    ...
    def _analyze_structure(self, data: Any) -> dict[str, Any]:
        ...
        if isinstance(data, dict):
            ...
            structure["nested"] = any(isinstance(v, (dict, list)) for v in data.values())

        elif isinstance(data, list):
            structure["item_count"] = len(data)
            if data:
                structure["item_type"] = type(data[0]).__name__
                structure["uniform"] = all(type(item) == type(data[0]) for item in data)
    ...
    def _extract_schema(self, data: Any) -> dict[str, Any]:
        ...
        if isinstance(data, dict):
            ...
            schema["type"] = "array"
            if data:
                schema["items"] = {"type": type(data[0]).__name__}
    ...
    def _calculate_statistics(self, data: Any) -> dict[str, Any]:
        ...
        if isinstance(data, dict):
            ...
            stats["has_nested"] = any(isinstance(v, (dict, list)) for v in data.values())
            stats["value_types"] = {}
    ...
    def _generate_examples(self, data: Any) -> list[dict[str, Any]]:
        ...
        if isinstance(data, dict):
            ...
            examples.append(
                {"title": "Accessing Items", "code": "data[0]", "description": "Access first item"}
            )
...
def generate_booklet(
    ...
    config = BookletConfig(
        title=title, author=author, include_apis=include_apis, include_examples=include_examples
    )
