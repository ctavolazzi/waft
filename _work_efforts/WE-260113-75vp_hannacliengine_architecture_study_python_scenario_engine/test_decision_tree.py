...
def test_feature_extraction():
    ...
    state = ScenarioState(
        ...
        containers={"inventory": ["rusty_key", "coin_purse"], "clues": ["mysterious_note"]},
...
def test_scenario_engine_integration():
    ...
    print(
        f"Decision tree trained: {engine.decision_tree.is_trained if engine.decision_tree else False}"
    )
        ...
        if engine.events:
            ...
            if last_event.sequence_id:
                ...
                if recommendation:
                    choice, confidence = recommendation
                    print(
                        f"Recommendation for {last_event.sequence_id}: {choice} (confidence: {confidence:.2f})"
                    )
        ...
        if event.choices_available and event.choice_made:
            ...
            if recommendation:
                ...
                if predicted_choice == actual_choice:
                    correct += 1
                    print(
                        f"✓ Correct: {event.sequence_id} -> {predicted_choice} (confidence: {confidence:.2f})"
                    )
                else:
                    print(
                        f"✗ Wrong: {event.sequence_id} -> predicted {predicted_choice}, actual {actual_choice}"
                    )
