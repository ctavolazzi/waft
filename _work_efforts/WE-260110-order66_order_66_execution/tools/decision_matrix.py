...
def evaluate_options(
    ...
    for option in options:
        ...
        if len(option_scores) != len(criteria):
            raise ValueError(f"Option '{option}' has {len(option_scores)} scores, expected {len(criteria)}")
...
def interactive_mode():
    ...
    # Get weights
    weights_input = input(f"Enter weights for {len(criteria)} criteria (comma-separated, must sum to 1.0): ").strip()
...
def main():
    ...
    parser.add_argument(
        "--criteria",
        type=str,
        help="Comma-separated list of criteria"
    )
    parser.add_argument(
        "--weights",
        type=str,
        help="Comma-separated list of weights (must sum to 1.0)"
    )
    parser.add_argument(
        "--options",
        type=str,
    ...
    parser.add_argument(
        "--scores",
        type=str,
        help="JSON object mapping options to score arrays"
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
    ...
    if not all([args.criteria, args.weights, args.options, args.scores]):
        print("Error: --criteria, --weights, --options, and --scores are required (or use --interactive)")
        parser.print_help()
        return
