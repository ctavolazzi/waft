    ...
    def start_electron_window(self):
        ...
        try:
            ...
            try:
                ...
                console.print("[yellow]💡 Tip: Install Electron globally for better experience: npm install -g electron[/yellow]")
            ...
            console.print("[yellow]   Open it manually or run: open output/campaign_display.html[/yellow]")
...
<body>
    <div class="container">
        ...
        {f'<div class="victory"><h2>🎉 VICTORY! 🎉</h2><p>The Shadow Lord Malachar has been defeated!<br>The realm is saved!</p></div>' if campaign_state.get('victory') else ''}
    </div>
...
class SelfPlayingCampaignElectron:
    ...
    def spawn_party(self) -> List[PartyMember]:
        ...
        for config in party_configs:
            ...
            self.campaign_log.append(f"✨ {config['name']} ({config['race']} {config['class']}) joined the party!")
    ...
    def update_display(self):
        ...
        self.display.update_html({
            ...
            "message": "🎲 Adventure in Progress..." if not self.final_boss_defeated else "🎉 VICTORY!",
            "party": party_data,
    ...
    def run_complete_campaign(self):
        ...
        for i, chapter_config in enumerate(chapters_config, 1):
            ...
            for encounter_name in chapter_config["encounters"]:
                difficulty = "boss" if i >= 4 and encounter_name == chapter_config["encounters"][-1] else "medium"
        ...
        console.print("[yellow]💡 The window will stay open. Close it when done viewing.[/yellow]\n")
    ...
    def generate_campaign_pdf(self):
        ...
        party_summary = "\n".join([
            f"- **{m.name}**: {m.race} {m.class_type} (Level {m.level})"
            for m in self.party
        ])
...
def main():
    ...
    console.print("[blue]💡 You can also open it manually: open output/campaign_display.html[/blue]\n")
