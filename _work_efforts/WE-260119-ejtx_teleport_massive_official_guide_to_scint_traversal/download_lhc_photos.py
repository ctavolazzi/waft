...
def extract_image_from_cern_html(html_content: str, base_url: str) -> Optional[str]:
...
def extract_wikimedia_image_url(page_url: str) -> Optional[str]:
...
def download_cern_image(url: str, output_path: Path) -> bool:
    ...
    try:
        ...
        print(f"  📄 URL is HTML page, extracting image...")
        ...
        if image_url:
            ...
            print(f"  ⚠️  Could not find image in HTML page")
            return False
...
def main():
    ...
    for person_id, info in PHOTO_SOURCES.items():
        ...
        if output_path.exists():
            ...
            results[person_id] = {"status": "exists", "path": str(output_path.relative_to(OUTPUT_DIR.parent))}
            continue
        ...
        for url in info['urls']:
            ...
            if success:
                results[person_id] = {"status": "downloaded", "path": str(output_path.relative_to(OUTPUT_DIR.parent))}
                break
