...
class GeminiPDFAdapter:
    ...
    async def enhance_character_description(
        ...
        if use_gemini and self.enabled:
            try:
                ...
                logger.info(f"✅ Generated Gemini-enhanced description for {character.get('name', 'character')}")
                return description
            except Exception as e:
    ...
    async def generate_story_chapter(
        ...
        if use_gemini and self.enabled and context:
            try:
                ...
                logger.info(f"✅ Generated Gemini-enhanced story chapter: {chapter_data.get('title', 'Unknown')}")
                return narrative
            except Exception as e:
    ...
    def enhance_campaign_content(
        ...
        if 'campaign' in enhanced:
            enhanced['campaign']['narrative'] = self.engine.generate_campaign_narrative(
                enhanced['campaign']
            ) if (use_gemini and self.enabled) else self.engine._fallback_campaign_narrative(
                enhanced['campaign']
            )
        ...
        if 'characters' in enhanced:
            for character in enhanced['characters']:
                character['description'] = self.engine.generate_character_description(
                    character
                ) if (use_gemini and self.enabled) else self.engine._fallback_character_description(
                    character
                )
