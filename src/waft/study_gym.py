    ...
    def _get_templates() -> dict[str, dict[str, Any]]:
        ...
        return {
            "page_constraint": {
                ...
                "constraints": {"exact_pages": "{target_pages}", "printer_friendly": True},
            ...
            "content_fitting": {
                ...
                "constraints": {"max_pages": "{max_pages}"},
                "content_template": "{content}",
            ...
            "multi_document": {
                ...
                "constraints": {"per_document_pages": "{target_pages}"},
                "content_template": "{content}",
            ...
            "printer_friendly": {
                ...
                "constraints": {"printer_friendly": True, "exact_pages": "{target_pages}"},
                "content_template": "{content}",
...
def run_study_session(
    challenge_config: dict[str, Any], output_dir: Path | None = None
) -> StudySession:
    """
...
def _attempt_challenge(challenge_config: dict[str, Any], gym: StudyGym) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": str(e), "constraints_met": False}
...
def _form_initial_hypothesis(
    observations: list[Observation], challenge_config: dict[str, Any], gym: StudyGym
) -> Hypothesis:
    ...
    if isinstance(result, dict):
        if result.get("success"):
            ...
            if not constraints_met:
                statement = (
                    "Adjusting CSS (font size, margins, spacing) will help meet page constraints"
                )
...
def _test_hypothesis(
    hypothesis: Hypothesis, challenge_config: dict[str, Any], gym: StudyGym
) -> dict[str, Any]:
    """Test a hypothesis by attempting the challenge again with adjustments."""
    # Try to implement the test plan
    test_result = {"tested": True, "confirmed": False, "refuted": False, "notes": ""}
...
def _analyze_results(session: StudySession, gym: StudyGym) -> list[str]:
    ...
    if successful_attempts:
        ...
        # Analyze page counts
        page_counts = [
            obs.result.get("pages", 0) for obs in successful_attempts if "pages" in obs.result
        ]
        if page_counts:
...
def _form_conclusions(session: StudySession, gym: StudyGym) -> list[str]:
    ...
    successful = [
        ...
        and obs.result.get("success")
        and obs.result.get("constraints_met")
    ]
