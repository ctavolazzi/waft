...
from waft.core.science.tam_psyche import TamPsyche
...
class TamPsycheWithDivergence(TamPsyche):
    ...
    cycle_number: int = Field(
        default=1, description="Current cycle number (increments on each rebirth)"
    )
    last_divergence_check: datetime | None = Field(
        default=None, description="Timestamp of last divergence check"
    )
        ...
        base_state.update(
            {
                "divergence": self.divergence,
                "cycle_number": self.cycle_number,
                "last_divergence_check": self.last_divergence_check.isoformat()
                if self.last_divergence_check
                else None,
            }
        )
        return base_state
