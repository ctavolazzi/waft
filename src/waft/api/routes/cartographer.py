...
def extract_bob_data(project_path: Path) -> Dict[str, Any]:
    ...
    if not pyrite_path.exists():
        return {
            "found": False,
            "message": "No _pyrite/active directory found"
        }
    ...
    if not bob_files:
        return {
            "found": False,
            "message": "No Bob the Cartographer files found"
        }
    ...
    api_module_path = project_path / "src" / "waft" / "core" / "cartographer" / "bob_cartographer_api.py"
