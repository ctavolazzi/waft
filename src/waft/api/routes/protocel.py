...
from waft.protocel import ProtoCelSystem
...
class CreateProtoCelRequest(BaseModel):
    name: str | None = Field(default=None, description="Optional name for the ProtoCel")
...
class InteractWithBeingRequest(BaseModel):
    ...
    data: dict[str, Any] | None = Field(default=None, description="Optional interaction data")
...
async def create_protocel(request: CreateProtoCelRequest) -> dict[str, Any]:
    ...
    try:
        ...
        protocel = system.create_protocel(name=request.name, description=request.description)
...
async def list_protocels() -> dict[str, Any]:
    ...
    try:
        ...
        return {"protocels": protocel_ids, "count": len(protocel_ids)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def observe_being(protocel_id: str, request: ObserveBeingRequest) -> dict[str, Any]:
    """
        ...
        if request.fetch_data:
            ...
            try:
                ...
                being_data = {
                    ...
                    if hasattr(being.state, "value")
                    else str(being.state),
                    "skills": being.skills,
...
async def interact_with_being(
    protocel_id: str, request: InteractWithBeingRequest
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        result = protocel.interact_with_being(
            request.being_id, request.interaction_type, request.data
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
