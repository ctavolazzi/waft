...
async def get_health():
    ...
    """
    return {"status": "ok", "version": "1.0", "core": "diamond_plated", "engine": "iron_core"}
...
def _run_sensitivity_analysis(
    matrix: Any, calculator: DecisionMatrixCalculator, original_rankings: list
) -> dict[str, Any]:
    """
    ...
    """
    from ...core.decision_matrix import Criterion, DecisionMatrix
    ...
    if other_total > 0:
        ...
        for crit in matrix.criteria:
            if crit.name == crit_name:
                crit_objects.append(Criterion(crit.name, reduced_weight, crit.description))
            else:
                # Proportional distribution
                adjusted_weight = crit.weight * (remaining_weight / other_total)
                crit_objects.append(Criterion(crit.name, adjusted_weight, crit.description))

        # Create adjusted matrix and calculate
        adjusted_matrix = DecisionMatrix(
            matrix.alternatives, crit_objects, matrix.scores, methodology="WSM"
        )
