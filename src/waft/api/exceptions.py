...
class WorkEffortNotFoundError(HTTPException):
    ...
    def __init__(self, work_effort_id: str):
        super().__init__(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Work effort not found: {work_effort_id}"
        )
...
class ValidationError(HTTPException):
    ...
    def __init__(self, message: str, detail: dict = None):
        super().__init__(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=detail or {"message": message}
        )
