...
class HourglassTorus:
    ...
    def _load(self):
        ...
        if self.torus_index_file.exists():
            with open(self.torus_index_file) as f:
                index = json.load(f)
        ...
        if self.current_cycle_file.exists():
            with open(self.current_cycle_file) as f:
    ...
    def complete_cycle(self) -> dict[str, Any]:
        ...
        cycle_file = (
            self.cycles_path
            / f"generation_{self.current_generation}_cycle_{self.current_cycle}.json"
        )
    ...
    def get_cycle(self, generation: int, cycle: int) -> dict[str, Any] | None:
        """
    ...
    def get_generation(self, generation: int) -> dict[str, Any] | None:
        """
