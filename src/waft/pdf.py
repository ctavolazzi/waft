...
class PDF:
    ...
    def __init__(self, config: PDFConfig):
        ...
        self._backend: Any | None = None  # Store backend instance
    ...
    def from_blocks(
        ...
        **foundation_config,
        ...
        """
        config = PDFConfig(
            title=title, output_path=output_path, use_foundation_v2=use_foundation_v2, blocks=blocks
        )
        pdf = cls(config)
    ...
    def from_markdown(
        ...
        """
        config = PDFConfig(
            title=title or "Document", content=markdown, output_path=output_path, style=style
        )
        pdf = cls(config)
    ...
    @classmethod
    def from_html(
        cls, html: str, title: str | None = None, output_path: Path | None = None
    ) -> "PDF":
        """
        ...
        Returns:
            PDF instance
        """
        config = PDFConfig(title=title or "Document", content=html, output_path=output_path)
        pdf = cls(config)
        pdf._backend = "html"
        return pdf
    ...
    def two_page(
        ...
        **kwargs,
        ...
        """
        config = PDFConfig(
            title=title, content=content, output_path=output_path, style=style, target_pages=2
        )
        pdf = cls(config)
    ...
    def latex(
        ...
        **kwargs,
        ...
        """
        config = PDFConfig(
            title=title, content=content, output_path=output_path, compile_latex=compile_pdf
        )
        pdf = cls(config)
        pdf._backend = "latex"
        return pdf
    ...
    @classmethod
    def from_pdf(cls, pdf_path: str | Path, output_path: Path | None = None) -> "PDF":
        """
    ...
    def save(self, output_path: Path | None = None, open_pdf: bool = False, **kwargs) -> Path:
        """
    ...
    def _save_template(self, output_path: Path | None, open_pdf: bool, **kwargs) -> Path:
        """Save using template system."""
        if not output_path:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            safe_title = "".join(
                c if c.isalnum() or c in (" ", "-", "_") else "" for c in self.config.title
            )
    ...
    def _save_evolution(self, output_path: Path | None, open_pdf: bool, **kwargs) -> Path:
        ...
        pdf_path = generator.save(output_path=output_path, open_pdf=open_pdf, **kwargs)

        self._generated_path = pdf_path
        return pdf_path

    def _save_foundation(self, output_path: Path | None, open_pdf: bool, **kwargs) -> Path:
        ...
        if self.config.use_foundation_v2:
            ...
            foundation_config = FoundationConfig(
                title=self.config.title, page_margins=(25.4, 25.4, 25.4, 25.4)
            )
    ...
    def _save_markdown(self, output_path: Path | None, open_pdf: bool, **kwargs) -> Path:
    ...
    def _save_html(self, output_path: Path | None, open_pdf: bool, **kwargs) -> Path:
        ...
        pdf_path = golden_triangle.html_to_pdf(
            html=self.config.content or "", output_path=output_path
        )
    ...
    def _save_scientific(self, output_path: Path | None, open_pdf: bool, **kwargs) -> Path:
    ...
    def _save_two_page(self, output_path: Path | None, open_pdf: bool, **kwargs) -> Path:
        ...
        from .evolution.styling_genome import (
            ColorGene,
            FontGene,
            LayoutGene,
            MarginGene,
        ...
        result = generator.generate(
            distilled_chat=distilled_chat, styling_genome=styling_genome, output_path=output_path
        )
    ...
    def _save_latex(self, output_path: Path | None, open_pdf: bool, **kwargs) -> Path:
        ...
        # Generate LaTeX
        generator = LaTeXGenerator(distilled_chat=distilled_chat, styling_genome=styling_genome)
