...
from .templates.worldbuild import generate_worldbuild_document
...
class WorldbuildDocument:
    ...
    def add_keyvalue_block(self, data: dict[str, str], label: str | None = None):
        """Add a key-value block (metadata, parameters, etc.)."""
        self.blocks.append({"type": "keyvalue", "data": data, "label": label})
        return self
    ...
    def add_warning_block(self, message: str, severity: str = "WARNING"):
        ...
        """
        self.blocks.append({"type": "warning", "message": message, "severity": severity.upper()})
        return self

    def add_signature_block(self, role: str, name: str, date: str | None = None):
        """Add a signature block."""
        self.blocks.append(
            {"type": "signature", "role": role, "name": name, "date": date or self.date}
        )
        return self

    def add_section_header(self, text: str, level: int = 2):
        """Add a section header."""
        self.blocks.append({"type": "section", "text": text, "level": level})
        return self

    def add_summary_box(self, title: str, content: str):
        """Add a summary box."""
        self.blocks.append({"type": "summary", "title": title, "content": content})
        return self
    ...
    def add_table(self, headers: list[str], rows: list[list[str]], caption: str | None = None):
        """Add a table."""
        self.blocks.append({"type": "table", "headers": headers, "rows": rows, "caption": caption})
        return self

    def add_log_block(self, entries: list[str]):
        """Add a log block (terminal-style output)."""
        self.blocks.append({"type": "log", "entries": entries})
        return self

    def add_markdown(self, markdown: str):
        """Add markdown content (converted to HTML)."""
        self.blocks.append({"type": "markdown", "content": markdown})
        return self
    ...
    def _render_warning_block(self, block: dict[str, Any]) -> str:
        ...
        severity_class = (
            "caution" if severity == "caution" else ("critical" if severity == "critical" else "")
        )
    ...
    def generate(self, output_path: Path | None = None) -> Path:
        ...
        if output_path is None:
            ...
            output_path = Path(
                f"_work_efforts/worldbuild/{safe_title}_{datetime.now().strftime('%Y%m%d')}.pdf"
            )
