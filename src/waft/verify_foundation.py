...
def generate_calibration_report(output_path: Path | None = None) -> Path:
    """
    ...
    engine.add(
        TextBlock("This page contains every single block type to verify they render correctly.")
    )
    ...
    # Signature Block
    engine.add(
        SignatureBlock(role="CALIBRATION TESTER", name="Test System", timestamp=datetime.now())
    )
    ...
    for i in range(1, 101):
        ...
        log_entries.append(
            f"{timestamp} Entry {i:03d}: This is log entry number {i} for pagination testing"
        )
    ...
    engine.add(
        SignatureBlock(
            role="CALIBRATION SYSTEM", name="Automated Test Generator", timestamp=datetime.now()
        )
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("   ✅ All stress tests included:")
        print("      - Kitchen Sink page (all block types)")
        print("      - Redaction torture test (50 occurrences)")
        print("      - Pagination stress test (100 log entries)")
        print("      - Selectability verification")
