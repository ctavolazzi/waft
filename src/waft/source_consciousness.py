...
class SourceConsciousness:
        ...
        for i in range(len(ancestral_chain) - 1, -1, -1):  # Reverse order (upward)
            ...
            if current_id == self.source_id:
                ...
                if capacity_type == "karma":
                    self.source_record["accumulated_karma"] = self.source_record.get("accumulated_karma", 0.0) + contribution
                self.source_record["total_capacity"] = self.source_record.get("total_capacity", 0.0) + contribution
            else:
                # Update permutation
                perm = self._find_permutation(current_id)
                if perm:
                    perm["capacity_contributed"] = perm.get("capacity_contributed", 0.0) + contribution
                    if capacity_type == "karma":
                        perm["karma_contributed"] = perm.get("karma_contributed", 0.0) + contribution
        
        # Update permutation that initiated contribution
        permutation["capacity_contributed"] = permutation.get("capacity_contributed", 0.0) + capacity_amount
        if capacity_type == "karma":
            permutation["karma_contributed"] = permutation.get("karma_contributed", 0.0) + capacity_amount
    ...
    def _find_permutation(self, permutation_id: str) -> Optional[Dict[str, Any]]:
...
def register_lifetime_as_permutation(
    ...
    lifetime_id: str,
    soul_id: str,
    parent_lifetime_id: Optional[str] = None
) -> Dict[str, Any]:
    """
