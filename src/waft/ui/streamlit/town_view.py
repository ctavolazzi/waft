...
def render_town_view(
    being_system, empirica_manager, gamification_manager, tavern_keeper, project_path: Path
):
    ...
    with col2:
        if being_system:
            ...
            being_count = (
                len([d for d in beings_path.iterdir() if d.is_dir()]) if beings_path.exists() else 0
            )
            ...
            if beings_path.exists():
                ...
                if beings:
                    for being_id in beings[:5]:  # Show 5 most recent
                        st.markdown(
                            f'<div class="town-building">👤 {being_id[:20]}...</div>',
                            unsafe_allow_html=True,
                        )
                else:
            ...
            if work_efforts:
                ...
                for effort in work_efforts[:6]:  # Show 6 most recent
                    with st.expander(f"🏗️ {effort['id']}"):
                        ...
                        if st.button("View Details", key=f"view_{effort['id']}"):
    ...
    with right_col:
        ...
        try:
            ...
            if "town_world" in st.session_state and st.session_state.town_world:
                ...
                st.metric(
                    "Conversations",
                    len([a for a in town.agents.values() if a.current_conversation]),
                )
    ...
    with col1:
        ...
        if being_system:
            ...
            if beings_path.exists():
                beings = sorted(
                    [d.name for d in beings_path.iterdir() if d.is_dir()], reverse=True
                )[:3]
    ...
    with col2:
        ...
        if work_efforts_path.exists():
            work_efforts = sorted(
                [
                    d.name
                    for d in work_efforts_path.iterdir()
                    if d.is_dir() and d.name.startswith("WE-")
                ],
                reverse=True,
            )[:3]
