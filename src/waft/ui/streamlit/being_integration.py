...
from .utils import display_error, display_success, load_json_file
...
def render_being_system_page(being_system: BeingSystem | None, project_path: Path):
    ...
    with st.form("spawn_being_form"):
        ...
        if submitted:
            try:
                ...
                try:
                    initial_skills = (
                        validate_skills_json(initial_skills_json) if initial_skills_json else {}
                    )
                ...
                if not re.match(r"^[a-zA-Z0-9_-]+$", reality_id):
                    display_error(
                        "Reality ID contains invalid characters. Use only letters, numbers, underscores, and hyphens.",
                        "Validation Error",
                    )
                    return
                ...
                being = being_system.spawn_being(
                    reality_id=reality_id, parent_being_id=parent_id, initial_skills=initial_skills
                )
...
def render_being_details(being_system: BeingSystem, being_id: str):
    ...
    try:
        ...
        chronicle_path = (
            project_path / "_hidden" / ".truth" / "beings" / being_id / "chronicle.jsonl"
        )
...
def render_recent_beings(being_system: BeingSystem | None):
...
def render_spawn_being_modal(being_system: BeingSystem | None):
