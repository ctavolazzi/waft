...
try:
    from waft.ai_town import TownAgent, TownVotingSystem, TownWorld
...
def render_town_page(project_path: Path):
    ...
    if st.session_state.town_world is None:
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(
            ["📊 Overview", "🗺️ Map", "💬 Conversations", "👤 Agents", "🗳️ Voting"]
        )
        ...
        if st.session_state.get("simulation_running") and not st.session_state.get(
            "simulation_paused"
        ):
            run_simulation_step()
...
def render_town_overview():
    ...
    with col2:
        active_convos = len(
            [c for c in town.conversation_manager.conversations.values() if c.is_active()]
        )
    ...
    auto_refresh = st.checkbox(
        "🔄 Auto-refresh (1 second)", value=st.session_state.get("auto_refresh", False)
    )
...
def render_town_map():
    ...
    if PLOTLY_AVAILABLE:
        ...
        for agent in town.agents.values():
            ...
            if agent.current_conversation:
                ...
                if conv_id not in conversation_colors:
                    conversation_colors[conv_id] = color_palette[
                        len(conversation_colors) % len(color_palette)
                    ]
        ...
        st.info(
            "Plotly not available. Install with: `pip install plotly` for interactive map visualization."
        )
...
def render_conversations():
    ...
    if active_conversations:
        for conv in active_conversations:
            with st.expander(
                f"Conversation {conv.conversation_id[:8]}... ({len(conv.messages)} messages)"
            ):
                # Participants
                participant_names = [
                    town.agents[p].name if p in town.agents else p for p in conv.participants
                ]
    ...
    past_conversations = [
        c for c in town.conversation_manager.conversations.values() if not c.is_active()
    ]
            ...
            with col2:
                ...
                if agent.memories:
                    for memory in agent.memories[-10:]:  # Show last 10
                        with st.expander(
                            f"Memory {memory.get('conversation_id', 'unknown')[:8]}..."
                        ):
...
def render_add_agent_modal():
    ...
    with st.form("add_agent"):
        agent_name = st.text_input(
            "Agent Name", value=f"Agent_{len(st.session_state.town_world.agents) + 1}"
        )
