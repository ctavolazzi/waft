...
def create_decision(decision_id: str, title: str, description: str, options: List[str], decision_type: str):
    ...
    c.execute('''INSERT INTO decisions 
                 ...
                 VALUES (?, ?, ?, ?, ?, ?)''',
              (decision_id, title, description, json.dumps(options), decision_type, datetime.now().isoformat()))
    conn.commit()
    conn.close()
...
def get_votes_for_decision(decision_id: str) -> List[Dict[str, Any]]:
    ...
    conn = get_db_connection()
    c = conn.cursor()
    c.execute('''SELECT vote_id, voter_id, vote_choice, reasoning, timestamp
...
def add_council_member(member_id: str, name: str, role: str):
    ...
    conn = get_db_connection()
    c = conn.cursor()
    c.execute('''INSERT OR REPLACE INTO council_members 
...
def main():
    ...
    st.set_page_config(
        page_title="WAFT Town Voting System",
        page_icon="⚖️",
        layout="wide"
    )
    ...
    page = st.sidebar.selectbox(
        "Navigation",
        ["🏠 Dashboard", "🗳️ Cast Vote", "📊 Voting Results", "👥 Council Members", "📜 Court Proceedings", "📄 Generate Document"]
    )
...
def show_voting_results():
    ...
    selected_decision = st.selectbox(
        "Select Decision",
        decisions,
        format_func=lambda x: f"{x['title']} ({x['status']})"
    )
    ...
    if selected_decision:
        ...
        if results["total_votes"] > 0:
            ...
            if not votes_df.empty:
                st.dataframe(votes_df[["voter_id", "vote_choice", "reasoning", "timestamp"]], use_container_width=True)
        else:
...
def show_council_members():
    ...
    with st.expander("Add Council Member", expanded=False):
        ...
        member_role = st.selectbox("Role", ["Chief Justice", "Justice", "Court Clerk", "Council Member"])
...
def create_court_proceeding(proceeding_id: str, case_id: str, title: str, description: str):
    ...
    conn = get_db_connection()
    c = conn.cursor()
    c.execute('''INSERT INTO court_proceedings 
...
def show_generate_document():
    ...
    doc_type = st.selectbox(
        "Document Type",
        ["Court Resolution", "Voting Record", "Council Meeting", "Custom Document"]
    )
    
    # Document details
    doc_title = st.text_input("Document Title", value=f"{doc_type} - {datetime.now().strftime('%Y-%m-%d')}")
    ...
    # Custom content
    custom_content = st.text_area("Additional Content (HTML)", height=200, 
                                  help="Add custom HTML content for the document")
        ...
        try:
            ...
            if include_council and council_members:
                ...
                for member in council_members:
                    content_parts.append(f"<div class='council-member'>")
                    content_parts.append(f"<span class='council-role'>{member['role']}:</span> {member['name']}")
            ...
            if include_decisions and decisions:
                ...
                for decision in decisions[:5]:
                    ...
                    content_parts.append(f"<p><strong>Created:</strong> {decision['created_at']}</p>")
            ...
            if include_votes and decisions:
                ...
                for decision in decisions[:3]:
                    ...
                    if results["total_votes"] > 0:
                        content_parts.append(f"<h3>{decision['title']}</h3>")
                        content_parts.append(f"<p><strong>Total Votes:</strong> {results['total_votes']}</p>")
                        if results["winner"]:
                            content_parts.append(f"<p><strong>Winner:</strong> {results['winner']} ({results['winner_votes']} votes)</p>")
            
            if include_proceedings and proceedings:
                content_parts.append("<h2>COURT PROCEEDINGS</h2>")
                for proceeding in proceedings[:5]:
                    content_parts.append(f"<div class='proceeding-entry'>")
                    content_parts.append(f"<div class='proceeding-time'>{proceeding['timestamp']}</div>")
                    content_parts.append(f"<h3>{proceeding['title']}</h3>")
                    content_parts.append(f"<p class='proceeding-text'>{proceeding['description']}</p>")
                    content_parts.append("</div>")
