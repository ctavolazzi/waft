...
class WorkEffortObserver:
    ...
    def on_status_change(
        self, we_id: str, old_status: WorkEffortStatus, new_status: WorkEffortStatus
    ):
...
class Pyrite:
    ...
    def __init__(
        self,
        work_efforts_path: Path = Path("_work_efforts"),
        pyrite_path: Path = Path("_pyrite"),
        ...
        self._empirica_session_id: str | None = None
    ...
    def _ensure_empirica_session(self):
        ...
        if not self._empirica_session_id:
            self._empirica_session_id = self.empirica.create_session(
                ai_id="pyrite", session_type="work_efforts_management"
            )
    ...
    def get_lock_holder(self, we_id: str) -> str | None:
        ...
        for observer in self._observers:
            try:
                if event == "status_change":
                    observer.on_status_change(
                        kwargs["we_id"], kwargs["old_status"], kwargs["new_status"]
                    )
    ...
    def update_metric(self, metric_name: str, value: Any, we_id: str | None = None):
        """Update a metric."""
        if we_id:
    ...
    def get_metrics(self, we_id: str | None = None) -> dict[str, Any]:
        """Get metrics."""
        if we_id:
    ...
    def get_work_effort(self, we_id: str) -> WorkEffortNode | None:
    ...
    def get_children(self, we_id: str) -> list[WorkEffortNode]:
        ...
        return [
            self._work_effort_graph[cid] for cid in children_ids if cid in self._work_effort_graph
        ]
    ...
    def update_work_effort_status(self, we_id: str, new_status: WorkEffortStatus):
        ...
        self._notify_observers(
            "status_change", we_id=we_id, old_status=old_status, new_status=new_status
        )
        self._save_state()
        return True
    ...
    def initiate_evolution(
        ...
        if gate_result == "HALT":
            ...
            self.empirica.log_unknown(
                f"Evolution requires investigation before proceeding: {we_id}"
            )
            return None
        ...
        if cycle.selected_variant:
            ...
            if fitness < 0.5:
                self.empirica.log_unknown(
                    f"Why is fitness low for {we_id}? (fitness: {fitness:.2f})"
                )
        else:
            self.empirica.log_finding(
                f"Evolution failed: {we_id} - no variant selected", impact=0.4
            )
    ...
    def _spawn_variants(
        self, we_id: str, num_variants: int, strategy: EvolutionaryStrategy
    ) -> list[dict[str, Any]]:
    ...
    def _generate_mutation(
        self, strategy: EvolutionaryStrategy, node: WorkEffortNode
    ) -> dict[str, Any]:
        """Generate mutation based on strategy."""
        mutation = {"type": "code_change", "scope": "medium", "changes": []}
    ...
    def get_attribute(self, name: str) -> PyriteAttribute | None:
        """Get an attribute."""
    ...
    def create_secret(self, data: dict[str, Any], metadata: dict[str, Any] | None = None) -> str:
        """
    ...
    def get_secret_metadata(self, secret_id: str) -> dict[str, Any] | None:
    ...
    def _ability_think(self) -> dict[str, Any]:
        ...
        if self._empirica_session_id:
            epistemic_state = self.empirica.assess_state(
                self._empirica_session_id, include_history=False
            )
    ...
    def _ability_evolve(
        self, we_id: str, strategy: str = "adaptive", num_variants: int = 5
    ) -> dict[str, Any]:
        ...
        if cycle:
            return {
                ...
                "fitness": cycle.fitness_scores.get(cycle.selected_variant, 0.0),
                "empirica": {"goal_created": True, "findings_logged": True},
            }
        return {"status": "failed", "error": "Could not initiate evolution"}

    def _ability_monitor(self, we_id: str | None = None) -> dict[str, Any]:
        ...
        if we_id:
            ...
            if node:
                ...
                if node.fitness < 0.5:
                    self.empirica.log_unknown(
                        f"Why is fitness low for {we_id}? (fitness: {node.fitness:.2f})"
                    )
        ...
        return {
            "total_work_efforts": len(self._work_effort_graph),
            "active": len(
                [n for n in self._work_effort_graph.values() if n.status == WorkEffortStatus.ACTIVE]
            ),
    ...
    def _ability_organize(self) -> dict[str, Any]:
        ...
        organization = {"total_nodes": len(self._work_effort_graph), "trees": [], "orphans": []}
    ...
    def _ability_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "work_efforts": {
                ...
                "by_status": {
                    status.value: len(
                        [n for n in self._work_effort_graph.values() if n.status == status]
                    )
                    for status in WorkEffortStatus
                },
            },
            "locks": {"total": len(self._lock_holders), "holders": dict(self._lock_holders)},
            "evolution": {
                "active_cycles": len(self._active_cycles),
                "total_cycles": self._metadata["total_cycles"],
            },
            "secrets": {"total": len(self._secrets)},
        }

    def _ability_secrets(self) -> dict[str, Any]:
        """Ability: /secrets - List secrets (metadata only)."""
        return {"total_secrets": len(self._secrets), "secrets": self.list_secrets()}
        ...
        state = {
            ...
            "secrets": {
                secret_id: {
                    ...
                    "created": secret.created.isoformat(),
                    "access_count": secret.access_count,
                    "last_accessed": secret.last_accessed.isoformat()
            ...
            if "attributes" in state:
                for name, attr_data in state["attributes"].items():
                    if name in self._attributes:
                        ...
                        self._attributes[name].last_updated = datetime.fromisoformat(
                            attr_data["last_updated"]
                        )
            ...
            if "secrets" in state:
                for secret_id, secret_data in state["secrets"].items():
                    secret = PyriteSecret(
                        ...
                        last_accessed=datetime.fromisoformat(secret_data["last_accessed"])
