...
class TownVotingSystem:
    ...
    def _being_ranked_vote(self, being: Any, options: list[str]) -> list[dict[str, Any]]:
        ...
        return [{"option": option, "rank": i + 1} for i, option in enumerate(ranked)]
    ...
    def _generate_reasoning(
        self, being: Any, vote_choice: Any, question: str, options: list[str]
    ) -> str:
    ...
    def conduct_town_vote(
        ...
        voting_record = {
            ...
            "non_selected_beings": [
                getattr(b, "being_id", str(id(b))) for b in non_selected_beings
            ],
    ...
    def calculate_results(
        self, votes: list[dict[str, Any]], vote_type: VoteType = VoteType.BINARY
    ) -> dict[str, Any]:
        """
        ...
        """
        if not votes:
            return {"result": None, "vote_counts": {}, "is_tie": False, "total_votes": 0}
        ...
        return {"result": None, "vote_counts": {}, "is_tie": False, "total_votes": len(votes)}

    def _oracle_break_tie(
        self, oracle: Any, question: str, options: list[str], votes: list[dict[str, Any]]
    ) -> str:
        """
    ...
    def get_voting_history(self, decision_id: str | None = None) -> list[dict[str, Any]]:
        """
        Get voting history.
