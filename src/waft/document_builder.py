...
from .binder import Binder, DocumentEntry
from .pdf_improvements import PDFContentProcessor, PDFStylingEnhancer
from .templates.registry import TemplateMetadata, TemplateRegistry, get_registry
...
class DocumentConfig:
    ...
    template: TemplateType | str  # Can be enum or template name
    title: str
    content: str
...
class DocumentBuilder:
    ...
    def __init__(self, config: DocumentConfig):
        ...
        self._analysis: PDFAnalysis | None = None
    ...
    @classmethod
    def from_pdf(cls, pdf_path: str | Path) -> "DocumentBuilder":
        """
    ...
    def _analyze_pdf(cls, pdf_path: Path) -> PDFAnalysis:
            ...
            for line in lines:
                ...
                if len(line) < 100 and (
                    ...
                    or any(
                        ...
                        for keyword in ["abstract", "introduction", "conclusion", "references"]
                    )
                ):
                    if current_section:
                        sections.append(current_section)
                    current_section = {"title": line, "page": page_num, "content": ""}
        ...
        styling_hints = {
            ...
            "is_academic": any(
                keyword in full_text.lower()[:1000]
                for keyword in ["technical report", "we report", "we present", "abstract"]
            ),
    ...
    def recreate(self, output_path: Path | None = None) -> Path:
        """
                ...
                json.dumps(
                    {
                        ...
                        "runId": "post-fix",
                        "hypothesisId": "F",
                        "location": "document_builder.py:557",
                        "message": "_render_template entry",
                        "data": {
                            "content_length": len(self.config.content)
                            if self.config.content
                            else 0,
                            "content_preview": self.config.content[:300]
                            if self.config.content
                            else "",
                            "content_is_html": bool(re.search(r"<[^>]+>", self.config.content))
                            if self.config.content
                            else False,
                            "has_h1": bool(re.search(r"<h1[^>]*>", self.config.content))
                            if self.config.content
                            else False,
                            "has_hr": bool(re.search(r"<hr[^>]*>", self.config.content))
                            if self.config.content
                            else False,
                        },
                ...
                json.dumps(
                    {
                        ...
                        "runId": "post-fix",
                        "hypothesisId": "G",
                        "location": "document_builder.py:565",
                        "message": "before content processing",
                        "data": {
                            "content_length": len(processed_content) if processed_content else 0,
                            "content_is_html": bool(re.search(r"<[^>]+>", processed_content))
                            if processed_content
                            else False,
                            "has_h1": bool(re.search(r"<h1[^>]*>", processed_content))
                            if processed_content
                            else False,
                        },
                        "timestamp": int(time.time() * 1000),
                ...
                json.dumps(
                    {
                        ...
                        "runId": "post-fix",
                        "hypothesisId": "G",
                        "location": "document_builder.py:578",
                        "message": "after content processing",
                        "data": {
                            "processed_length": len(processed_content) if processed_content else 0,
                            "has_h1_after": bool(re.search(r"<h1[^>]*>", processed_content))
                            if processed_content
                            else False,
                            "has_hr_after": bool(re.search(r"<hr[^>]*>", processed_content))
                            if processed_content
                            else False,
                        },
                        "timestamp": int(time.time() * 1000),
                ...
                json.dumps(
                    {
                        ...
                        "runId": "post-fix",
                        "hypothesisId": "G",
                        "location": "document_builder.py:595",
                        "message": "before CSS injection",
                        "data": {
                            "has_style_tag": bool("<style>" in html_output),
                ...
                json.dumps(
                    {
                        ...
                        "runId": "post-fix",
                        "hypothesisId": "G",
                        "location": "document_builder.py:603",
                        "message": "after CSS injection",
                        "data": {
                            "html_output_length": len(html_output) if html_output else 0,
                            "has_enhanced_css": bool(enhanced_css in html_output)
                            if enhanced_css
                            else False,
                            "has_h1_in_final": bool(re.search(r"<h1[^>]*>", html_output))
                            if html_output
                            else False,
                        },
                        "timestamp": int(time.time() * 1000),
                ...
                json.dumps(
                    {
                        ...
                        "runId": "post-fix",
                        "hypothesisId": "F",
                        "location": "document_builder.py:577",
                        "message": "_render_template exit",
                        "data": {
                            "html_output_length": len(html_output) if html_output else 0,
                            "html_output_preview": html_output[
                                html_output.find('<div class="content">') : html_output.find(
                                    '<div class="content">'
                                )
                                + 500
                            ]
                            if html_output and '<div class="content">' in html_output
                            else html_output[:500]
                            if html_output
                            else "",
                            "has_h1_in_output": bool(re.search(r"<h1[^>]*>", html_output))
                            if html_output
                            else False,
                            "has_hr_in_output": bool(re.search(r"<hr[^>]*>", html_output))
                            if html_output
                            else False,
                            "has_raw_hash": bool(re.search(r"#\s+WAFT", html_output))
                            if html_output
                            else False,
                        },
                        "timestamp": int(time.time() * 1000),
...
class DocumentCollection:
    ...
    def add(self, document: DocumentBuilder, section: str | None = None) -> "DocumentCollection":
    ...
    def save(self, output_path: Path, include_dividers: bool = True) -> Path:
        ...
        for section_name, docs in self.sections.items():
            binder_section = self.binder.add_section(
                section_name, description=f"{len(docs)} document(s)"
            )
...
def quick_field_guide(
    title: str, content: str, output: str, printer_friendly: bool = False
) -> Path:
    """Quick field guide generation."""
    return DocumentBuilder.field_guide(
        title=title, content=content, output_path=Path(output), printer_friendly=printer_friendly
    ).save()
