...
class KarmaCollector:
    ...
    def collect_from_life_log_file(
        self,
        life_log_path: Path,
        soul_id: str
    ) -> Dict[str, Any]:
        ...
        # Load life log
        with open(life_log_path, "r") as f:
        ...
        if collected_file.exists():
            ...
            collected_file = self.collected_path / f"{life_log_path.stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}{life_log_path.suffix}"
        life_log_path.rename(collected_file)
        
        return result
    
    def collect_all_pending(
        self,
    ...
    def _extract_soul_id_from_file(
        self,
        life_log_file: Path,
        default_soul_id: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            with open(life_log_file, "r") as f:
                life_log = json.load(f)
