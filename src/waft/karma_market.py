...
class KarmaMarket:
    ...
    def purchase_lifetime(
        self, lifetime_id: str, soul_id: str, custom_config: dict[str, Any] | None = None
    ) -> Lifetime:
        """
        ...
        # Add tool costs
        tools = (
            custom_config.get("tools", lifetime_config.get("tools", []))
            if custom_config
            else lifetime_config.get("tools", [])
        )
        ...
        personality = (
            custom_config.get("personality", lifetime_config.get("personality", {}))
            if custom_config
            else lifetime_config.get("personality", {})
        )
        ...
        if current_karma < base_cost:
            ...
            raise InsufficientKarmaError(f"Insufficient karma: {current_karma} < {base_cost}")
        ...
        lifetime = Lifetime(
            ...
            objectives=custom_config.get("objectives", lifetime_config.get("objectives", []))
            if custom_config
            else lifetime_config.get("objectives", []),
    ...
    def end_lifetime(self, lifetime_id: str, karma_earned: float | None = None) -> Lifetime:
        """
    ...
    def get_active_lifetimes(self, soul_id: str | None = None) -> list[Lifetime]:
    ...
    def get_lifetime_remaining_time(self, lifetime_id: str) -> timedelta | None:
        """
    ...
    def _transfer_karma_to_soul(
        self, soul_id: str, lifetime: Lifetime, karma_earned: float
    ) -> None:
...
class AfterlifeKarmaMarket:
    ...
    def purchase_treasure(
        self, treasure_type: str, treasure_id: str, soul_id: str
    ) -> dict[str, Any]:
        """
        ...
        if current_karma < treasure_cost:
            ...
            raise InsufficientKarmaError(f"Insufficient karma: {current_karma} < {treasure_cost}")

        # Deduct karma
        self.karma_market._deduct_karma(
            soul_id, treasure_cost, reason=f"Purchased treasure: {treasure_type}/{treasure_id}"
        )
    ...
    def _apply_treasure_to_soul(self, soul_id: str, treasure_type: str, treasure_id: str) -> None:
        ...
        soul_data["treasures"][treasure_type].append(
            {"treasure_id": treasure_id, "purchased_at": datetime.now().isoformat()}
        )
