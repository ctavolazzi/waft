...
def map_being_to_cv_data(being: "Being") -> dict[str, Any]:
    ...
    personal = {
        "firstname": being.custom_name.split()[0]
        if being.custom_name and " " in being.custom_name
        else (being.custom_name or being.being_id.split("_")[0]),
        "lastname": " ".join(being.custom_name.split()[1:])
        if being.custom_name and " " in being.custom_name
        else (being.being_id.split("_")[-1] if "_" in being.being_id else ""),
    ...
    for memory in being.memories[:10]:  # Limit to 10 most recent
        ...
        if memory_type in ["work", "experience", "achievement"] or "experience" in metadata.get(
            "tags", []
        ):
            experience.append(
                {
                    ...
                    "date": metadata.get("timestamp", memory.get("recorded_at", being.created_at))[
                        :10
                    ]
                    if metadata.get("timestamp") or memory.get("recorded_at")
                    else being.created_at[:10],
                    "location": metadata.get("reality_id", being.reality_id),
                    "subtitle": metadata.get("subtitle"),
                    "bullets": metadata.get("details", [])
                    if isinstance(metadata.get("details"), list)
                    else [memory.get("content", "")[:100]],
                }
            )
        ...
        experience.append(
            {
                ...
                "bullets": [
                    f"Developed expertise in {skill} (Level {level:.1f})"
