...
def _create_invoice_typ(
    ...
    issue_date_typst = (
        issue_date.strftime("%Y-%m-%d") if isinstance(issue_date, datetime) else str(issue_date)
    )
    due_date_typst = (
        due_date.strftime("%Y-%m-%d") if due_date and isinstance(due_date, datetime) else None
    )
...
def generate_invoice_from_transaction(
    ...
    if transaction.transaction_type == TransactionType.SALARY:
        ...
        sender = {"name": corporation_name, "address": corporation_address}
        recipient = {
        ...
        sender = {"name": transaction.to_party or "Vendor", "address": recipient_address or ""}
        recipient = {"name": corporation_name, "address": corporation_address}
