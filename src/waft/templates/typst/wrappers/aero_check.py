...
def _build_topic_typst(
    topic: ChecklistTopic, use_shadows: bool = False, shadow_config: ShadowConfig | None = None
) -> str:
    """
    ...
    if use_shadows and shadow_config:
        ...
        typst_lines.append("  #shadow-path(")
        typst_lines.append("    (2%, 2%), (2%, 98%), (98%, 98%), (98%, 2%),")
        typst_lines.append("    closed: true,")
        ...
        typst_lines.append("  )")
        typst_lines.append("  #pad(x: 2cm, y: 0.6cm)[")
        typst_lines.append('    #text(size: 16pt, weight: "bold", fill: gray.darken(30%))[')
...
def generate_aero_checklist(
    ...
    **kwargs,
    ...
    typst_parts.append("#show: checklist.with(")
