...
class Character:
    ...
    ability_scores: dict[str, int] | None = None  # {"STR": 16, "DEX": 14, ...}
    hit_points: dict[str, int] | None = None  # {"current": 45, "max": 50}
        ...
        if self.ability_scores:
            ...
            for ability in self.ability_scores:
                if ability not in valid_abilities:
                    raise ValueError(
                        f"Invalid ability score: {ability}. Must be one of {valid_abilities}"
                    )
                score = self.ability_scores[ability]
                if not isinstance(score, int) or score < 1 or score > 30:
                    raise ValueError(
                        f"Ability score {ability} must be an integer between 1 and 30, got {score}"
                    )
...
class StatBlock:
    ...
    traits: list[dict[str, str]] | None = None  # [{"name": "Trait Name", "text": "Description"}]
    ...
    if character.skills:
        ...
        for skill in character.skills:
            if isinstance(skill, dict):
                ...
                typst_lines.append(
                    f"- *{_sanitize_typst_content(skill_name)}*: {skill_value} {skill_mod}"
                )
            else:
...
def _build_stat_block_typst(stat_block: StatBlock) -> str:
    ...
    if stat_block.skills:
        typst_lines.append(
            f"*Skills*: {', '.join(_sanitize_typst_content(s) for s in stat_block.skills)}"
        )

    if stat_block.damage_resistances:
        typst_lines.append(
            f"*Damage Resistances*: {', '.join(_sanitize_typst_content(d) for d in stat_block.damage_resistances)}"
        )

    if stat_block.damage_immunities:
        typst_lines.append(
            f"*Damage Immunities*: {', '.join(_sanitize_typst_content(d) for d in stat_block.damage_immunities)}"
        )

    if stat_block.condition_immunities:
        typst_lines.append(
            f"*Condition Immunities*: {', '.join(_sanitize_typst_content(c) for c in stat_block.condition_immunities)}"
        )
    ...
    if stat_block.challenge_rating:
        typst_lines.append(
            f"*Challenge Rating*: {_sanitize_typst_content(stat_block.challenge_rating)}"
        )
...
def generate_dnd_game(
    ...
    stat_blocks: list[StatBlock] | None = None,
    encounter_participants: list[EncounterParticipant] | None = None,
    ...
    **kwargs,
    ...
    if document_type not in valid_document_types:
        raise ValueError(
            f"Invalid document_type: {document_type}. Must be one of {valid_document_types}"
        )
    ...
    if template_package not in valid_packages:
        raise ValueError(
            f"Invalid template_package: {template_package}. Must be one of {valid_packages}"
        )
