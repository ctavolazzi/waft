...
def generate_worldbuild_agenda(
    ...
    output_path: Path,
    invited: Optional[List[str]] = None,
    ...
    invited_typst = "[" + ", ".join([f'"{name}"' for name in invited_list]) + "]" if invited_list else "[]"
    ...
    # Build YAML string
    topics_yaml = yaml.dump(topics_dict, default_flow_style=False, allow_unicode=True, sort_keys=False)
