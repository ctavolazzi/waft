...
class TypstCompiler:
    ...
    def compile(
        self, typst_content: str, output_path: Path, working_dir: Path | None = None
    ) -> Path:
        """
        ...
        if working_dir is None:
            with tempfile.TemporaryDirectory() as tmpdir:
                return self._compile_in_dir(typst_content, output_path, Path(tmpdir))
        else:
            ...
            return self._compile_in_dir(typst_content, output_path, working_dir)

    def _compile_in_dir(self, typst_content: str, output_path: Path, working_dir: Path) -> Path:
        """
    ...
    def compile_file(self, typ_file: Path, output_path: Path) -> Path:
        """
        ...
        if not os.access(typ_file, os.R_OK):
            raise PermissionError(
                f"Cannot read input file: {typ_file}. Please check file permissions."
            )
