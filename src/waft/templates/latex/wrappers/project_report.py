    ...
    **kwargs,
    ...
    if not template_file.exists():
        raise FileNotFoundError(
            f"Project Report template {template_version} not found: {template_file}"
        )
    ...
    if template_version == 1:
        ...
        member1_email = (
            members[0].get("email", "") if isinstance(members[0], dict) else "member1@gmail.com"
        )
        ...
        member2_email = (
            members[1].get("email", "") if isinstance(members[1], dict) else "member2@gmail.com"
        )
        ...
        member3_email = (
            members[2].get("email", "") if isinstance(members[2], dict) else "member3@gmail.com"
        )
        ...
        if background:
            filled_latex = filled_latex.replace(
                "\\section*{Background}", f"\\section*{{Background}}\n\n{background}"
            )
        if project_specification:
            filled_latex = filled_latex.replace(
                "\\section*{Project Specification}",
                f"\\section*{{Project Specification}}\n\n{project_specification}",
            )
        if problem_analysis:
            filled_latex = filled_latex.replace(
                "\\section*{Problem Analysis}",
                f"\\section*{{Problem Analysis}}\n\n{problem_analysis}",
            )
        if solution_design:
            filled_latex = filled_latex.replace(
                "\\section*{Solution Design}", f"\\section*{{Solution Design}}\n\n{solution_design}"
            )
        if implementation_testing:
            filled_latex = filled_latex.replace(
                "\\section*{Implementation \\& Testing}",
                f"\\section*{{Implementation \\& Testing}}\n\n{implementation_testing}",
            )
        if project_breakdown:
            filled_latex = filled_latex.replace(
                "\\section*{Project Breakdown Structure}",
                f"\\section*{{Project Breakdown Structure}}\n\n{project_breakdown}",
            )
        if results:
            filled_latex = filled_latex.replace(
        ...
        filled_latex = filled_latex.replace(
            "MEMBER1@gmail.com", member1_email or "MEMBER1@gmail.com"
        )
        ...
        filled_latex = filled_latex.replace(
            "MEMBER2@gmail.com", member2_email or "MEMBER2@gmail.com"
        )
        ...
        if implementation_testing:
            filled_latex = filled_latex.replace(
                ...
                f"\\section{{Implementation and Execution}}\n\n{implementation_testing}",
            )
        if experimental_setup:
            filled_latex = filled_latex.replace(
                "\\section{Experimental Setup}",
                f"\\section{{Experimental Setup}}\n\n{experimental_setup}",
            )
        if conclusion:
            filled_latex = filled_latex.replace(
                "\\section{Conclusion}", f"\\section{{Conclusion}}\n\n{conclusion}"
