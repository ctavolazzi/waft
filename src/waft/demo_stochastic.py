...
for i in range(10):
    ...
    print(f"  Roll {i+1:2d}: aesthetic={eval_result.aesthetic.value:.3f}, overall={eval_result.overall.value:.3f}")
...
eval2 = evaluate_answer("This is a much longer answer with more content and detail", "Complex problem requiring detailed analysis")
...
for mode, name in [(GuideMode.STRICT, "Strict"), (GuideMode.LENIENT, "Lenient"), (GuideMode.BASIC, "Basic")]:
    aesthetics = []
    for _ in range(5):
...
for i in range(5):
    ...
    print(f"  Trial {i+1}: A={eval_a.aesthetic.value:.3f}, B={eval_b.aesthetic.value:.3f}, diff={abs(eval_a.aesthetic.value - eval_b.aesthetic.value):.3f}")
