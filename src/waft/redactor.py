...
class PDFRedactor:
    ...
    def __init__(self, pdf_path: Union[str, Path]):
        ...
        if not HAS_PYPDF:
            raise ImportError("pypdf is required for PDF redaction. Install with: pip install pypdf")
    ...
    def redact_pattern(self, pattern: str, description: Optional[str] = None) -> "PDFRedactor":
        """
...
class SimplePDFRedactor:
    ...
    def add_redaction_area(self, x: float, y: float, width: float, height: float) -> "SimplePDFRedactor":
        """
...
def quick_redact(pdf_path: Union[str, Path], terms: List[str], output_path: Optional[Path] = None) -> Path:
    """
