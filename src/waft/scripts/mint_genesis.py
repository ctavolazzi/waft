...
def mint_genesis_artifact() -> Path:
    ...
    pdf.set_text_color(0, 0, 0)
    checklist_y = 50
    checklist_items = ["[X] MEM_CHK", "[X] BIO_SIG", "[ ] KARMA", "[X] ONTOLOGY", "[ ] SOUL_SYNC"]
    ...
    pdf.cell(
        page_width - left_margin_width - sidebar_width,
        14,
    ...
    pdf.rect(
        left_margin_width, hazard_y, page_width - left_margin_width - sidebar_width, 8, style="F"
    )
    ...
    pdf.set_fill_color(0, 0, 0)
    pdf.rect(
        left_margin_width, seal_line_y, page_width - left_margin_width - sidebar_width, 1, style="F"
    )
