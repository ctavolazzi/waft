...
from ..pyrite import WorkEffortStatus, get_pyrite
...
@click.option(
    "--strategy",
    default="adaptive",
    type=click.Choice(["conservative", "aggressive", "adaptive", "exploratory"]),
)
...
@app.command()
def monitor(
    we_id: str | None = typer.Argument(None),
    work_efforts_path: str = typer.Option("_work_efforts", "--work-efforts-path"),
    pyrite_path: str = typer.Option("_pyrite", "--pyrite-path"),
):
...
def set_status(
    ...
    status: str = typer.Argument(
        ..., help="Status: dormant, active, locked, evolving, completed, archived, corrupted"
    ),
    work_efforts_path: str = typer.Option("_work_efforts", "--work-efforts-path"),
    pyrite_path: str = typer.Option("_pyrite", "--pyrite-path"),
):
...
def get_children(
    ...
    result = [
        {"we_id": child.we_id, "title": child.title, "status": child.status.value}
...
def get_ancestors(
    ...
    result = [
        {"we_id": ancestor.we_id, "title": ancestor.title, "status": ancestor.status.value}
