...
class PantheonCouncil:
    ...
    def summon_pantheon(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.console.print("[yellow]→[/yellow] [bold]Magistrate[/bold] (God of Precedent) considers the evidence...")
        weigh_ins["magistrate"] = self._magistrate_weighs_in(evidence)
        
        # Judge weighs in (Judgment and Evaluation)
        self.console.print("[yellow]→[/yellow] [bold]Judge[/bold] (God of Judgment) reviews the case...")
        weigh_ins["judge"] = self._judge_weighs_in(evidence)
        
        # Storyteller weighs in (Narrative and Story)
        self.console.print("[yellow]→[/yellow] [bold]Storyteller[/bold] (God of Narrative) sees the story...")
        weigh_ins["storyteller"] = self._storyteller_weighs_in(evidence)
        
        # Librarian weighs in (Knowledge and Records)
        self.console.print("[yellow]→[/yellow] [bold]Librarian[/bold] (Keeper of Records) examines the archives...")
        weigh_ins["librarian"] = self._librarian_weighs_in(evidence)
        
        # TestRunner weighs in (Testing and Verification)
        self.console.print("[yellow]→[/yellow] [bold]TestRunner[/bold] (God of Verification) checks the tests...")
        weigh_ins["test_runner"] = self._test_runner_weighs_in(evidence)
        
        # MilitaryBrass weighs in (Missions and Strategy)
        self.console.print("[yellow]→[/yellow] [bold]MilitaryBrass[/bold] (God of Missions) reviews operations...")
        weigh_ins["military_brass"] = self._military_brass_weighs_in(evidence)
        
        # Fae weighs in (Quests and Adventure)
        self.console.print("[yellow]→[/yellow] [bold]Fae[/bold] (God of Quests) considers the journey...")
        weigh_ins["fae"] = self._fae_weighs_in(evidence)
        
        # MissionControl weighs in (Coordination and Status)
        self.console.print("[yellow]→[/yellow] [bold]MissionControl[/bold] (God of Coordination) assesses status...")
        weigh_ins["mission_control"] = self._mission_control_weighs_in(evidence)
        
        # TheVillage weighs in (Community and Connection)
        self.console.print("[yellow]→[/yellow] [bold]TheVillage[/bold] (God of Community) listens to the gathering...")
        weigh_ins["village"] = self._village_weighs_in(evidence)
        
        # Oracle weighs in (Epistemic Intelligence)
        if self.oracle_available:
            self.console.print("[yellow]→[/yellow] [bold]TheOracle[/bold] (Epistemic Intelligence) provides insight...")
    ...
    def _judge_weighs_in(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        work_efforts = evidence.get("work_efforts", [])
        if work_efforts:
            perspective += f"Active work efforts need evaluation against the Body of Proof. "
    ...
    def _storyteller_weighs_in(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        recommendation = "Document the current chapter. Ensure the narrative is clear and compelling."
    ...
    def _librarian_weighs_in(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        recommendation = "Maintain the archives. Ensure knowledge is properly cataloged and accessible."
    ...
    def _military_brass_weighs_in(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if missions:
            ...
            if high_priority > 0:
                perspective += f"{high_priority} high-priority missions require immediate attention. "
        else:
    ...
    def display_judgment(self, judgment: Dict[str, Any], weigh_ins: Dict[str, Any]):
        ...
        self.console.print(f"  • Work Efforts: {len(judgment.get('all_recommendations', []))} entities consulted")
