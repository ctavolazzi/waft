...
class PDFMetrics:
    ...
    def compute_quality_score(self) -> float:
        ...
        quality = (
            self.fitness_overall * fitness_weight + self.fitness_constraint * constraint_weight
        )
...
class PDFMetricsCollector:
    ...
    def collect_metrics(
        ...
        if pdf_path:
            ...
            pdf_id = hashlib.sha256(result.get("html_content", "").encode()).hexdigest()[:16]
        ...
        metrics = PDFMetrics(
            ...
            # Input
            input_ideas_total=distilled_chat.total_ideas
            if hasattr(distilled_chat, "total_ideas")
            else 0,
            ...
            chat_summary_length=len(distilled_chat.summary)
            if hasattr(distilled_chat, "summary")
            else 0,
            # Generation
            ...
            if hasattr(styling_genome, "genome_id")
            else "",
            styling_scientific_name=styling_genome.scientific_name
            if hasattr(styling_genome, "scientific_name")
            else "",
            font_family=styling_genes.font.family
            if styling_genes and hasattr(styling_genes, "font")
            else "",
            font_size_body=styling_genes.font.size_body
            if styling_genes and hasattr(styling_genes, "font")
            else 0.0,
            font_size_h1=styling_genes.font.size_h1
            if styling_genes and hasattr(styling_genes, "font")
            else 0.0,
            color_scheme="custom" if styling_genes and hasattr(styling_genes, "color") else "",
            layout_density=styling_genes.layout.density
            if styling_genes and hasattr(styling_genes, "layout")
            else "",
            margin_top=styling_genes.margin.top
            if styling_genes and hasattr(styling_genes, "margin")
            else 0.0,
            margin_bottom=styling_genes.margin.bottom
            if styling_genes and hasattr(styling_genes, "margin")
            else 0.0,
            margin_left=styling_genes.margin.left
            if styling_genes and hasattr(styling_genes, "margin")
            else 0.0,
            margin_right=styling_genes.margin.right
            if styling_genes and hasattr(styling_genes, "margin")
            else 0.0,
            # Fitness
            ...
            content_density_words_per_page=content_stats.get("density", 0.0)
            if content_stats
            else 0.0,
    ...
    def _compute_avg_importance(self, distilled_chat: Any) -> float:
        ...
        if not hasattr(distilled_chat, "ideas") or not distilled_chat.ideas:
            return 0.0
        importances = [
            idea.importance for idea in distilled_chat.ideas if hasattr(idea, "importance")
        ]
