...
class ComponentPDFGenerator:
    ...
    def generate_one_pager(
        ...
        if output_path is None:
            ...
            output_path = (
                self.project_path / "_work_efforts" / "one_pagers" / f"ComponentPDF_{timestamp}.pdf"
            )
        ...
        # 4. Attribution
        components.append(
            self.builder.build_attribution_component(author, datetime.now().strftime("%Y-%m-%d"))
        )
        ...
        components.append(
            self.builder.build_metadata_component(
                authors=authors_list, generation_info=generation_info
            )
        )
        ...
        for section_title, section_ideas in sections:
            if section_ideas:
                components.append(
                    self.builder.build_section_component(section_title, section_ideas, level=2)
                )
