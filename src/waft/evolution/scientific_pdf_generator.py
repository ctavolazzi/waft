...
class ScientificPDFGenerator(PDFGenerator):
    ...
    def __init__(
        ...
        study_gym_dir: Optional[Path] = None,
    ...
    def analyze_quality(self) -> Dict[str, Any]:
        ...
        has_intro = any(word in content_lower for word in ["introduction", "overview", "background"])
        ...
        if self.research_db.get("pdfs"):
            previous_avg = sum(p.get("quality_score", 0) for p in self.research_db["pdfs"]) / len(self.research_db["pdfs"])
    ...
    def test_hypothesis(
        ...
        test_plan: str = "",
        wager_karma: Optional[float] = None
    ) -> Dict[str, Any]:
        """
    ...
    def compare_with_previous(self, category: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        previous_pdfs = [
            p for p in self.research_db.get("pdfs", [])
        ...
        current_quality = sum(current_analysis["scores"].values()) / max(len(current_analysis["scores"]), 1)
        
        previous_qualities = [p.get("quality_score", 0) for p in previous_pdfs]
        avg_previous = sum(previous_qualities) / len(previous_qualities) if previous_qualities else 0
    ...
    def identify_patterns(self) -> Dict[str, Any]:
        ...
        if styles:
            ...
            patterns["styling_patterns"]["most_common"] = style_counts.most_common(1)[0][0] if style_counts else None
        
        # Analyze quality patterns
        qualities = [p.get("quality_score", 0) for p in pdfs]
        if qualities:
            patterns["quality_patterns"]["average"] = sum(qualities) / len(qualities)
            patterns["quality_patterns"]["trend"] = "improving" if qualities[-1] > qualities[0] else "declining"
                ...
                try:
                    ...
                    if "metrics" in result:
                        ...
                        analysis["metrics"] = {
                            ...
                            "fitness_overall": metrics.get("fitness_metrics", {}).get("overall", 0.0)
                        }
                except Exception:
                    pass
...
def generate_scientific_pdf(
    ...
    **kwargs
    ...
    if scientific_mode:
        ...
        print(f"   Quality Score: {sum(analysis['scores'].values()) / max(len(analysis['scores']), 1):.2f}")
        print(f"   Gaps: {len(analysis['gaps'])}")
        print(f"   Suggestions: {len(analysis['suggestions'])}")
    
    return generator.save(output_path=output_path, open_pdf=open_pdf, collect_metrics=scientific_mode)
