...
class ComponentTrait:
    ...
    max_pages_allowed: Optional[float] = None  # Maximum pages where component works
    ...
    section_preferences: Dict[SectionPreference, float] = field(default_factory=lambda: {
        SectionPreference.ANYWHERE: 1.0
    })
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ComponentTrait":
        ...
        section_prefs = {
            SectionPreference(k): v 
            for k, v in data.get('section_preferences', {}).items()
        }
    ...
    def mutate(self, mutation_rate: float = 0.1) -> "ComponentTrait":
        ...
        if random.random() < mutation_rate:
            ...
            new_trait.height_estimate = max(0.05, min(0.5, 
                self.height_estimate + random.gauss(0, 0.02)
            ))
            new_trait.mutations_applied.append("height_estimate")
        
        if random.random() < mutation_rate:
            # Mutate min pages
            new_trait.min_pages_required = max(1.0, 
                self.min_pages_required + random.gauss(0, 0.1)
            )
    ...
    def learn_from_result(
        ...
        fitness: float,
        pages_used: int,
        section_used: Optional[SectionPreference] = None,
    ):
        ...
        if success:
            ...
            self.avg_fitness_when_used = (
                (self.avg_fitness_when_used * (total - 1) + fitness) / total
            )
        else:
    ...
    def learn_from_user_feedback(self, liked: bool, feedback_strength: float = 1.0):
        ...
        if liked:
            self.user_likes += 1
            self.user_feedback_score = min(1.0, 
                self.user_feedback_score + (0.1 * feedback_strength)
            )
        else:
            self.user_dislikes += 1
            self.user_feedback_score = max(0.0,
                self.user_feedback_score - (0.1 * feedback_strength)
            )
...
class EvolvedComponent:
    ...
    content_template: Optional[str] = None  # Template for generating content
