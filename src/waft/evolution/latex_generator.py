...
class LaTeXGenerator:
    ...
    def from_content(
        ...
        if style not in PDFGenerator.PRESETS:
            raise ValueError(
                f"Unknown style: {style}. Available: {list(PDFGenerator.PRESETS.keys())}"
            )
    ...
    def generate(self) -> str:
        ...
        latex.append("\\author{WAFT Document Generator}")
    ...
    def save(self, output_path: Path | None = None, compile_pdf: bool = False) -> Path:
    ...
    def _compile_pdf(self, tex_path: Path) -> Path:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print("⚠️  LaTeX compilation warnings (PDF may still be generated):")
                print(result.stderr)
                if pdf_path.exists():
                    return pdf_path
                else:
...
def generate_latex(
    ...
    """
    generator = LaTeXGenerator.from_content(
        content=content, title=title, document_class=document_class, style=style
    )
