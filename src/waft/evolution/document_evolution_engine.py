...
class DocumentEvolutionEngine(ComponentPDFGenerator):
    ...
    def __init__(
        ...
        if evolution_dir is None:
            evolution_dir = (
                project_path / "_genetics" / "document_evolution"
                if project_path
                else Path("_genetics/document_evolution")
            )
    ...
    def generate_one_pager(
        ...
        styling_genome: StylingGenome | None = None,
        ...
        if output_path is None:
            ...
            output_path = (
                self.project_path / "_work_efforts" / "one_pagers" / f"EvolvedPDF_{timestamp}.pdf"
            )
    ...
    def _build_evolved_components(
        ...
        from .document_components import ComponentBuilder, ComponentType
        ...
        if image_paths:
            for img_name, img_path in image_paths.items():
                img_comp = self.evolution_engine.get_or_create_component(
                    "image", f"image_{img_name}"
                )
                if allowed_pages >= img_comp.trait.min_pages_required:
                    img_uri = (
                        Path(img_path).absolute().as_uri() if Path(img_path).exists() else img_path
                    )
        ...
        if allowed_pages >= attr_comp.trait.min_pages_required:
            components.append(
                builder.build_attribution_component(author, datetime.now().strftime("%Y-%m-%d"))
            )
        ...
        for section_title, section_ideas in sections:
            if section_ideas:
                ...
                section_comp = self.evolution_engine.get_or_create_component(
                    "section", f"section_{section_title.lower().replace(' ', '_')}"
                )

                # Check if component wants to be in this section
                section_pref = self._get_section_preference(section_title)
                preference_weight = section_comp.trait.section_preferences.get(section_pref, 0.5)
                ...
                if use_component and allowed_pages >= section_comp.trait.min_pages_required:
                    components.append(
                        builder.build_section_component(section_title, section_ideas, level=2)
                    )
    ...
    def _consider_evolution(self):
        ...
        low_fitness = [
            comp
            for comp in self.evolution_engine.components.values()
            if comp.trait.get_fitness() < 0.3
            and comp.trait.success_count + comp.trait.failure_count > 5
        ]
