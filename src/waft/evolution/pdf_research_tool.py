...
from ..core.agent.state import EvolutionaryEventType
...
class PDFResearchTool:
    ...
    def __init__(self, research_db_path: Path | None = None):
        """
    ...
    def _load_db(self):
        ...
        if self.research_db_path.exists():
            ...
            self.db = {"pdfs": [], "hypotheses": [], "findings": [], "knowledge": []}

    def compare_pdfs(
        self, pdf_paths: list[Path], metrics: list[str] | None = None
    ) -> dict[str, Any]:
        """
        ...
        for pdf_path in pdf_paths:
            pdf_info = next(
                (p for p in self.db.get("pdfs", []) if p.get("path") == str(pdf_path)), None
            )
            if pdf_info:
        ...
        self._record_event(
            ...
            fitness_metrics={
                "average_quality": comparison["metrics"].get("quality_score", {}).get("average", 0)
    ...
    def analyze_trends(
        self, time_period: str = "30 days", category: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        analysis = {
            ...
            "quality_over_time": [
                {"date": p.get("timestamp", ""), "quality": p.get("quality_score", 0)} for p in pdfs
            ],
            "insights": [],
        }
        ...
        if trend == "improving":
            analysis["insights"].append(
                f"Quality improving by {trend_strength:.2f} over {days} days"
            )
        else:
            analysis["insights"].append(
                f"Quality declining by {abs(trend_strength):.2f} over {days} days"
            )
    ...
    def identify_patterns(self, category: str | None = None, min_pdfs: int = 3) -> dict[str, Any]:
        ...
        Returns:
            Pattern analysis
        """
        pdfs = [p for p in self.db.get("pdfs", []) if not category or p.get("category") == category]
        ...
        if len(pdfs) >= 2:
            ...
            older_qualities = (
                [p.get("quality_score", 0) for p in pdfs_by_date[:-5]]
                if len(pdfs_by_date) > 5
                else []
            )
