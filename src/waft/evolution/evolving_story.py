...
from .story_state import StoryState, Character, Event, PlotPoint
from .narrative_decisions import NarrativeDecision, DecisionValidator
...
class EvolvingStory:
    ...
    def generate_pdf(
        ...
        pdf_path = storyteller.tell_story(
            ...
            title=f"{self.title} - Generation {gen}",
            open_pdf=open_pdf
        )
        
        return pdf_path
    
    def save_state(self, file_path: Optional[Path] = None) -> Path:
        """
