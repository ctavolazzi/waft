...
class ScenarioDecisionTree:
    ...
    def __init__(self, max_depth: Optional[int] = 10, min_samples_split: int = 2):
        """
    ...
    def _build_feature_names(self, states: List[ScenarioState]) -> List[str]:
        ...
        # History features
        names.extend([
            "choice_count_aggressive",
            "choice_count_cautious",
            "choice_count_exploratory"
        ])
    ...
    def train(self, states: List[ScenarioState], choices: List[str]) -> None:
        ...
        if len(states) != len(choices):
            raise ValueError(f"States ({len(states)}) and choices ({len(choices)}) must have same length")
