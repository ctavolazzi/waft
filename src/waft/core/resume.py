...
class ResumeManager:
    ...
    def run_resume(
        self, session_file: str | None = None, compare: bool = True, full_context: bool = False
    ) -> dict[str, Any]:
        """
    ...
    def _find_last_session(self, session_file: str | None = None) -> Path | None:
        """
        ...
        session_files = sorted(
            checkout_dir.glob("session-*.md"), key=lambda p: p.stat().st_mtime, reverse=True
        )
    ...
    def _compare_states(
        self, session_data: dict[str, Any], current_state: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        if last_uncommitted > 0 and current_uncommitted < last_uncommitted:
            comparison["progress"].append(
                f"✅ Committed {last_uncommitted - current_uncommitted} file(s)"
            )
        elif current_uncommitted > last_uncommitted:
            comparison["progress"].append(
                f"📝 {current_uncommitted - last_uncommitted} new uncommitted file(s)"
            )
    ...
    def _identify_in_progress(
        ...
        for step in next_steps[:3]:  # Top 3 next steps
            in_progress.append({"item": step, "status": "Pending", "action": "Continue work"})
    ...
    def _generate_next_steps(
        ...
        for step in last_next_steps[:3]:
            next_steps.append({"action": step, "why": "Planned in last session", "command": None})
    ...
    def _display_resume_report(
        ...
        self.console.print("  • Structure: Valid")
