...
class TamPsyche(BaseModel):
    ...
    coherence: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Psychological coherence (higher = more stable)"
    )
    chaos: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Conflicting information/chaos level"
    )
    emotional_energy: float = Field(
        default=50.0, ge=0.0, le=100.0, description="Current emotional/mental energy"
    )
    realization_progress: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Progress toward realization threshold"
    )
    has_realized: bool = Field(default=False, description="Whether realization has occurred")
    realization_memory: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Memory strength of realization (decays to 0)"
    )
    forgetfulness_rate: float = Field(
        default=0.02, description="Base rate at which realization memory decays"
    )
    last_realization_timestamp: datetime | None = Field(
        default=None, description="When realization last occurred"
    )
        ...
        if file_path.exists():
            with open(file_path, encoding="utf-8") as f:
                ...
                if "last_realization_timestamp" in data and data["last_realization_timestamp"]:
                    data["last_realization_timestamp"] = datetime.fromisoformat(
                        data["last_realization_timestamp"]
                    )
                return cls(**data)
