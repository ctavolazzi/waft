...
class TheOracle:
    ...
    def get_epistemic_phase(self, show_calculation: bool = False) -> str:
        ...
        context = self.empirica.project_bootstrap()
        if not context:
            return {"phase": "UNKNOWN", "reason": "No context from project_bootstrap"} if show_calculation else "UNKNOWN"
    ...
    def _empirica_check(
        ...
        if uncertainty >= 0.99 and findings_count == 0 and unknowns_count == 0:
            ...
            decision_reasoning.append(f"Special case: uncertainty({uncertainty:.3f}) >= 0.99 AND no findings/unknowns → PROCEED (fallback mode)")
        elif confidence >= 0.7 and uncertainty < 0.3:
            decision = "PROCEED"
            decision_reasoning.append(f"confidence({confidence:.3f}) >= 0.7 AND uncertainty({uncertainty:.3f}) < 0.3 → PROCEED")
        elif confidence < 0.3 or uncertainty > 0.7:
            decision = "HALT"
            decision_reasoning.append(f"confidence({confidence:.3f}) < 0.3 OR uncertainty({uncertainty:.3f}) > 0.7 → HALT")
        elif unknowns_count > findings_count:
            decision = "BRANCH"  # Need investigation
            decision_reasoning.append(f"unknowns({unknowns_count}) > findings({findings_count}) → BRANCH (investigation needed)")
        else:
            decision = "REVISE"  # Need refinement
            decision_reasoning.append(f"Otherwise → REVISE (needs refinement)")
    ...
    def _answer_question_without_epistemic_state(
        ...
        if reflection:
            ...
            if relevant_insights:
                insights_text = ". ".join([
                    str(insight.get("insight", insight) if isinstance(insight, dict) else insight)
        ...
        if any(word in question_lower for word in ["architecture", "design", "structure", "system"]):
            return (
