"""
SessionReportGenerator: Scientific Report Generator for the Multiverse.
...
class SessionReportGenerator:
    """
    Generator for scientific session reports.

    Aggregates data from laboratory.jsonl and current Biome state
    to create comprehensive Markdown reports.
    """

    def __init__(self, project_path: Path, observer: TheObserver | None = None):
        """
        Initialize SessionReportGenerator.
    ...
    def generate_session_report(self, biome: Optional["Biome"] = None) -> str:
        ...
        lines.append("---")
        lines.append("")
        lines.append(f"*Report generated by SessionReportGenerator at {timestamp}*")
    ...
    def _section_biodiversity(self, lab_data: list[dict], biome: Optional["Biome"]) -> list[str]:
        ...
        # Remove death events
        death_events = [
            e for e in lab_data if e.get("event_type") in ["fitness_death", "boundary_death"]
        ]
        ...
        if symbol_dist:
            ...
            archetype_names = {
                "☿": "The Weaver (Social/Fluid)",
                "⚥": "The Balanced (Versatile)",
                "⚲": "The Static (High-Speed/Efficient)",
                "ⴲ": "The Foundation (Storage/Protective)",
            }
    ...
    def _section_phylogenetic_tree(self, lab_data: list[dict]) -> list[str]:
        ...
        for generation in sorted(by_generation.keys()):
            ...
            for org in by_generation[generation]:
                ...
                parent_short = (
                    org["parent_id"][:16] + "..." if org["parent_id"] else "None (Genesis)"
                )
                ...
                if payload.get("event") == "born_from_conjugation":
                    ...
                    lines.append(f"  - **Hybrid**: Child of {parent_a} × {parent_b}")

                lines.append("")

        return lines

    def _section_metabolic_health(
        self, lab_data: list[dict], biome: Optional["Biome"]
    ) -> list[str]:
    ...
    def _section_breach_incidents(self, lab_data: list[dict]) -> list[str]:
        ...
        breach_events = [
            ...
            for e in lab_data
            if (
                e.get("event_type") == "boundary_death"
                or e.get("payload", {}).get("event") == "boundary_death"
            )
        ]
...
class ObsidianGenerator:
            ...
            for entry in organism.state.journal[-20:]:  # Last 20 entries
                ...
                if entry_type == "Thought":
                    ...
                    if "state_snapshot" in entry:
                        lines.append(
                            f"**State**: Energy={entry['state_snapshot'].get('energy', 'N/A')}"
                        )
                    lines.append("")
    ...
    def generate_index(self, biome: Optional["Biome"] = None) -> Path:
        ...
        death_events = [
            e
            for e in lab_data
            if e.get("event_type") in ["fitness_death", "boundary_death", "death"]
        ]
        for event in death_events:
            genome_id = event.get("genome_id")
            if genome_id in living_genomes:
        ...
        for genome_id in living_genomes:
            ...
            if spawn_event:
                ...
                by_generation[generation].append({"genome_id": genome_id, "name": scientific_name})
