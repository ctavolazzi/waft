...
class CritiqueParser:
    ...
    def parse_critique(self, critique_path: Path) -> CritiqueData:
        ...
        self._parse_section(
            content, r"## 🔴 HIGH: Safety Issues", data.high_safety_issues, "HIGH", "Safety Issues"
        )
        self._parse_section(
            content,
    ...
    def _parse_section(
        ...
        for i, match in enumerate(matches, 1):
            ...
            criticism = self._parse_criticism(
                criticism_text, title, default_severity, section_name, i
            )
            if criticism:
                criticisms_list.append(criticism)

    def _parse_criticism(
        self, text: str, title: str, severity: str, section: str, number: int
    ) -> Criticism | None:
        ...
        criticism = Criticism(
            title=title.strip(), severity=severity, section=section, number=number, issue=issue_text
        )
