...
class CheckAssumptionsManager:
    ...
    def run_check_assumptions(
        ...
        self.console.print(f"[yellow]→[/yellow] Checking assumptions about assumption checking...")
        ...
        self.console.print(f"[yellow]→[/yellow] Gathering validation evidence for {len(assumptions)} assumptions...")
        evidence_sources = self._gather_evidence_sources()
        
        # Step 4: Validate each assumption
        self.console.print(f"[yellow]→[/yellow] Validating assumptions...")
    ...
    def _extract_assumptions(self, conversation_context: Optional[str] = None) -> List[Assumption]:
        """
        ...
        for category, category_patterns in patterns.items():
            for pattern in category_patterns:
                ...
                for match in matches:
                    ...
                    risk = "critical" if any(word in statement.lower() for word in ["must", "required", "critical", "essential"]) else "minor"
    ...
    def _validate_assumption(
                ...
                if gate_result:
                    if gate_result == "PROCEED":
                        ...
                        evidence_points.append({
                            ...
                            "type": "supporting",
                            "message": f"Oracle CHECK gate: PROCEED - Epistemic state supports this assumption",
                        })
                        ...
                        evidence_points.append({
                            "source": "oracle_epistemic",
                            "type": "contradicting",
                            "message": f"Oracle CHECK gate: HALT - High risk or insufficient knowledge",
                        })
                    elif gate_result == "BRANCH":
                        evidence_points.append({
                            "source": "oracle_epistemic",
                            "type": "info",
                            "message": f"Oracle CHECK gate: BRANCH - Need investigation first",
                        })
                        ...
                        evidence_points.append({
                            "source": "oracle_epistemic",
                            "type": "contradicting",
                            "message": f"Oracle CHECK gate: REVISE - Approach needs revision",
                        })
        ...
        if assumption.status == "DISPROVEN":
            assumption.recommendation = "This assumption is incorrect. Review and correct the underlying belief."
            ...
            assumption.recommendation = "Mixed evidence. Verify with additional testing or investigation."
        else:
    ...
    def _display_report(self, verbose: bool = False):
        ...
        for i, assumption in enumerate(self.assumptions, 1):
            ...
            self.console.print(f"  [dim]Category:[/dim] {assumption.category} | [dim]Risk:[/dim] [{risk_color}]{assumption.risk}[/{risk_color}]")
            self.console.print(f"  [dim]Status:[/dim] {assumption.status} | [dim]Confidence:[/dim] {assumption.confidence:.2f}")
            
            if verbose and assumption.evidence:
                self.console.print(f"  [dim]Evidence:[/dim]")
                for evidence in assumption.evidence:
                    icon = "✅" if evidence["type"] == "supporting" else "❌" if evidence["type"] == "contradicting" else "ℹ️"
        ...
        # Critical findings
        critical_findings = [a for a in self.assumptions if a.risk == "critical" and a.status in ["DISPROVEN", "PARTIALLY_PROVEN"]]
        if critical_findings:
            self.console.print("\n[bold red]⚠️ CRITICAL FINDINGS[/bold red]\n")
            for finding in critical_findings:
                self.console.print(f"[red]❌[/red] {finding.statement}")
                self.console.print(f"  Status: {finding.status} | Confidence: {finding.confidence:.2f}")
            ...
            for assumption in self.assumptions:
                if assumption.status == "PROVEN":
                    ...
                    empirica.log_unknown(
                        f"Assumption needs validation: {assumption.statement}"
                    )
        except Exception:
    ...
    def _is_testable(self, assumption: Assumption) -> bool:
        ...
        testable_keywords = [
            "returns", "produces", "creates", "generates", "affects", "changes",
            "improves", "decreases", "increases", "causes", "results in"
        ]
    ...
    def _convert_to_hypothesis(self, assumption: Assumption):
        ...
        try:
            import sys
            from pathlib import Path
            project_root = self.project_path.parent if self.project_path.name != "waft" else self.project_path
