...
from ...evolution.narrative_decisions import DecisionType, NarrativeDecision
...
class StoryDirector(BaseAgent):
    ...
    def __init__(
        ...
        config: AgentConfig,
        project_path: Path,
        story: Any | None = None,  # EvolvingStory
    ):
        """
    ...
    async def observe(self) -> dict[str, Any]:
        ...
        """
        if not self.story:
            return {"status": "error", "message": "No story set for director"}
        ...
        observation = {
            ...
            "characters": {
                name: {
                    ...
                    "last_appearance": char.last_appearance.isoformat()
    ...
    async def decide(self, state: AgentState) -> dict[str, Any]:
        ...
        """
        if not self.story:
            return {"status": "error", "message": "No story set for director", "stop": True}
        ...
        return {"status": "success", "decision": decision.dict(), "stop": False}
    ...
    async def act(self, decision: dict) -> dict[str, Any]:
        ...
        if decision.get("status") != "success" or "decision" not in decision:
            return {"status": "error", "message": "Invalid decision", "applied": False}
    ...
    async def evolve_story(
        self, story: Any | None = None
    ) -> NarrativeDecision:  # Optional[EvolvingStory]
        """
    ...
    def _generate_decision_content(
        self, decision_type: DecisionType, story_state: StoryState
    ) -> dict[str, Any]:
        """Generate decision content based on type and story state."""
        content = {"description": "", "narrative_text": "", "importance": 0.5, "confidence": 0.7}
        ...
        if decision_type == DecisionType.CHARACTER_ACTION:
            ...
            content["narrative_text"] = (
                f"{character} {action}. " if character else f"Someone {action}. "
            )
            ...
            content["description"] = (
                f"{character} {development}" if character else f"Someone {development}"
            )
            content["narrative_text"] = (
                f"{character} {development}. " if character else f"Someone {development}. "
            )
