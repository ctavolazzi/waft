...
class RecapAndReviewManager:
    ...
    def run_recap_and_review(self, output_path: str | None = None) -> dict[str, Any]:
        """
    ...
    def _gather_mindspace_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            data["current_state"]["stats"] = {
                ...
                "net_lines": stats.get("code", {}).get("lines_written", 0)
                - stats.get("code", {}).get("lines_deleted", 0),
            }
        except Exception:
            data["current_state"]["stats"] = {}
        ...
        try:
            ...
            data["work_in_progress"] = [
                {"file": str(f.name), "path": str(f)} for f in active_files[:10]
            ]
        except Exception:
            pass
    ...
    def _generate_mindspace_document(self, mindspace_data: dict[str, Any]) -> str:
        ...
        content.append(
            f"**Captured**: {mindspace_data['moment']['date']} {mindspace_data['moment']['time']}\n"
        )
    ...
    def _generate_pdf(self, md_file: Path, timestamp: str) -> Path | None:
    ...
    def _open_pdf_on_desktop(self, pdf_path: Path) -> bool:
        ...
        try:
            ...
            self.console.print("[green]🖥️  Opened PDF on desktop[/green]")
            return True
        except Exception as e:
    ...
    def _display_summary(
        self, mindspace_data: dict[str, Any], md_file: Path, pdf_file: Path | None
    ):
        """Display recap and review summary."""
        self.console.print("[bold]📋 Recap and Review Summary[/bold]\n")
        self.console.print(
            f"  • Moment: {mindspace_data['moment']['date']} {mindspace_data['moment']['time']}"
        )
        ...
        if pdf_file:
            self.console.print(
                f"[bold green]✅ PDF generated:[/bold green] {pdf_file.relative_to(self.project_path)}"
            )
            self.console.print("[bold green]🖥️  PDF opened on desktop[/bold green]\n")
        else:
            self.console.print("[yellow]⚠️  PDF not generated (markdown available)[/yellow]\n")
