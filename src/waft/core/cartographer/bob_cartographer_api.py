...
class NominatimAPI:
    ...
    def geocode_address(
        self, address: str, limit: int = 1, format: str = "json"
    ) -> list[dict[str, Any]]:
        """
        ...
        params = {"q": address, "format": format, "limit": limit, "addressdetails": 1}

        try:
            response = requests.get(
                f"{self.BASE_URL}/search", params=params, headers=self.headers, timeout=10
            )
    ...
    def reverse_geocode(
        self, lat: float, lon: float, format: str = "json"
    ) -> dict[str, Any] | None:
        """
        ...
        params = {"lat": lat, "lon": lon, "format": format, "addressdetails": 1}

        try:
            response = requests.get(
                f"{self.BASE_URL}/reverse", params=params, headers=self.headers, timeout=10
            )
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
    ...
    def search_place(
        self, query: str, limit: int = 10, format: str = "json"
    ) -> list[dict[str, Any]]:
        """
