...
class Node:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {**asdict(self), "connections": list(self.connections)}
...
class TendrilNetwork:
    ...
    def _save_network(self) -> None:
        ...
        try:
            ...
            self.nodes_file.write_text(json.dumps(nodes_data, indent=2), encoding="utf-8")
            ...
            self.tendrils_file.write_text(json.dumps(tendrils_data, indent=2), encoding="utf-8")
            ...
            self.strings_file.write_text(json.dumps(strings_data, indent=2), encoding="utf-8")
        except Exception as e:
            print(f"   ⚠️  Could not save network: {e}")

    def create_node(
        self, path: Path, node_type: str, metadata: dict[str, Any] | None = None
    ) -> Node:
        """
        ...
        path_str = str(
            path.relative_to(self.realm_path) if path.is_relative_to(self.realm_path) else path
        )
            ...
            while pq:
                ...
                for connected_id in self.nodes[current_id].connections:
                    ...
                    for tendril in self.tendrils.values():
                        if (
                            tendril.from_node_id == current_id
                            and tendril.to_node_id == connected_id
                        ) or (
                            tendril.from_node_id == connected_id
                            and tendril.to_node_id == current_id
                        ):
    ...
    def build_realm_network(self, max_depth: int = 3) -> dict[str, Any]:
        ...
        Returns:
            Build results
        """
        print("   🌐 Building Tendril Network...")

        nodes_created = 0
        tendrils_created = 0

        def explore_directory(dir_path: Path, depth: int = 0, parent_node_id: str | None = None):
