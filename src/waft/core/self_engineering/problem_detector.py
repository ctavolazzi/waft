...
class ProblemDetector:
    ...
    def monitor_execution(
        self, execution_result: dict[str, Any], execution_time: float | None = None
    ) -> list[Problem]:
        """
        ...
        if "exception" in execution_result or "error" in execution_result:
            ...
            exc_msg = (
                str(exception)
                if exception
                else execution_result.get("error_message", "Unknown error")
            )
        ...
        if self.notebook_manager:
            for problem in problems:
                ...
                self.notebook_manager.journal_problem(problem, context=sanitized_context)
    ...
    def get_recent_problems(self, count: int = 10) -> list[Problem]:
        ...
        return sorted(self.detected_problems, key=lambda p: p.timestamp, reverse=True)[:count]
