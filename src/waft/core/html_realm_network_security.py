...
def parse_html_safely(html_path: Path, timeout: int = MAX_PARSING_TIME) -> Optional[Dict[str, Any]]:
    """
...
def _parse_html_basic(html_path: Path, timeout: int) -> Optional[Dict[str, Any]]:
    """
...
def _read_file_with_timeout(file_path: Path, timeout: int) -> Optional[str]:
    """
...
def extract_html_metadata(soup: Optional[BeautifulSoup]) -> Dict[str, Any]:
    """
