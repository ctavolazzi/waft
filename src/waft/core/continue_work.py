...
class ContinueManager:
    ...
    def run_continue(
        self, deep: bool = False, focus: str | None = None, save: bool = False
    ) -> dict[str, Any]:
        """
    ...
    def _reflect_on_work(
        self, current_state: dict[str, Any], deep: bool, focus: str | None
    ) -> dict[str, Any]:
        """
        ...
        if uncommitted:
            ...
            config_files = [
                ...
                if any(f.endswith(ext) for ext in (".toml", ".json", ".yaml", ".yml"))
            ]

            if code_files:
                work_items.append(
                    {"type": "Code", "files": code_files[:5], "count": len(code_files)}
                )
            if doc_files:
                work_items.append(
                    {"type": "Documentation", "files": doc_files[:5], "count": len(doc_files)}
                )
            if config_files:
                work_items.append(
                    {"type": "Configuration", "files": config_files[:5], "count": len(config_files)}
                )
        ...
        reflection["how"] = {
            "methodology": " ".join(approach_indicators)
            if approach_indicators
            else "Standard development",
            "efficiency": "Good"
            if current_state["git"]["uncommitted_count"] < 50
            else "Many changes",
    ...
    def _identify_patterns(
        self, current_state: dict[str, Any], stats: dict[str, Any]
    ) -> list[dict[str, str]]:
        """
    ...
    def _meta_cognitive_analysis(self, reflection: dict[str, Any], deep: bool) -> dict[str, Any]:
        """
        ...
        meta["understanding_of_understanding"] = {
            ...
            "purpose_understanding": "Understood"
    ...
    def _generate_insights(
        self, reflection: dict[str, Any], meta_analysis: dict[str, Any]
    ) -> list[dict[str, str]]:
        """
    ...
    def _adjusted_continuation(
        self, reflection: dict[str, Any], insights: list[dict[str, str]]
    ) -> dict[str, Any]:
        """
        ...
        for insight in insights[:3]:
            continuation["insights_to_apply"].append(
                {"insight": insight.get("insight", ""), "how_to_apply": insight.get("action", "")}
            )
    ...
    def _display_reflection_report(
        ...
        self.console.print(
            f"  • Direction: {reflection.get('effectiveness', {}).get('direction', 'Unknown')}"
        )
        ...
        if patterns:
            ...
            for pattern in patterns:
                self.console.print(
                    f"  • {pattern.get('pattern', '')}: {pattern.get('observation', '')}"
                )
    ...
    def _save_reflection(
        ...
        content.append(
            f"### What: {reflection.get('what', {}).get('work_items', [{}])[0].get('type', 'Development') if reflection.get('what', {}).get('work_items') else 'Development'}\n\n"
        )
        content.append(
            f"### Why: {reflection.get('why', {}).get('inferred_purpose', 'Continuing work')}\n\n"
        )
        content.append(
            f"### How: {reflection.get('how', {}).get('methodology', 'Standard approach')}\n\n"
        )
