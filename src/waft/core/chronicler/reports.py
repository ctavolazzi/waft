...
class ReportGenerator:
    ...
    def generate_hourly_report(self, hour: Optional[int] = None, date: Optional[datetime] = None) -> Path:
        """
    ...
    def generate_daily_report(self, date: Optional[datetime] = None) -> Path:
        ...
        if by_observer:
            ...
            for observer, counts in sorted(by_observer.items(), key=lambda x: sum(x[1].values()), reverse=True):
