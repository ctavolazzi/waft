...
def validate_experiment_id(experiment_id: str) -> Tuple[bool, Optional[str]]:
    ...
    if len(experiment_id) > MAX_EXPERIMENT_ID_LENGTH:
        return False, f"Experiment ID exceeds maximum length of {MAX_EXPERIMENT_ID_LENGTH} characters"
    
    if not EXPERIMENT_ID_PATTERN.match(experiment_id):
        return False, "Experiment ID can only contain alphanumeric characters, underscores, and hyphens"
...
def validate_iteration(iteration: int) -> Tuple[bool, Optional[str]]:
    """
