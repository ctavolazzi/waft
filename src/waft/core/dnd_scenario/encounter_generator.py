...
class EncounterGenerator:
    ...
    def generate_encounter(
        ...
        if not encounter_name:
            encounter_names = [
                "Goblin Ambush", "Orc Raiders", "Dark Cultists",
                "Shadow Wolves", "Corrupted Treant", "Undead Warriors"
            ]
...
After {rounds} rounds of combat, {encounter_name} falls, defeated by the heroes' resolve.
        ...
        # Display encounter
        console.print(Panel(encounter["content"], title=f"[bold]{encounter_name}[/bold]", border_style="red"))
        console.print(f"   [green]✓[/green] Encounter complete! Party HP: {sum([m.hp for m in party])}/{sum([m.max_hp for m in party])}")
