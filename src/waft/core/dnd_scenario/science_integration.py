...
class DnDScenarioScienceIntegration:
    ...
    def __init__(self, scenario_realm: ScenarioRealm, enable_oracle: bool = True):
        ...
        if not SCIENTIFIC_METHOD_AVAILABLE:
            raise ImportError("scientific_method_tool not available. Install it to use science integration.")
        ...
        self.state_preserver = RealmStatePreserver(scenario_realm.realm_path, scenario_realm.project_path)
    ...
    def capture_initial_state_as_crystallized(
        ...
        initial_state = self.experiment_manager.capture_initial_state(
            ...
            components,
            metadata={
                "crystallization_manifest": manifest,
                **(metadata or {})
            }
        )
    ...
    def run_scenario_as_experiment(
        ...
        data_collector_callback: Optional[Callable] = None
    ) -> Dict[str, Any]:
        """
        ...
        data_collector.record("party_average_level", sum([m.level for m in party]) / len(party) if party else 0)
        ...
        if mode == "encounter" and "encounter" in scenario_result:
            ...
            data_collector.record("encounter_damage_taken", sum(encounter.get("damage_taken", {}).values()))
    ...
    def run_iterative_experiment(
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            if restore_initial_state and iteration > 1:
                ...
                if "party_state" in state_data:
                    self.orchestrator.party_state_manager.save_party_state(state_data["party_state"])
        ...
        result = {
            ...
            "empirica_session_id": (
                getattr(self.oracle, '_session_id', None) or 
                (getattr(self.oracle, '_readiness_status', {}).get('session_id') if hasattr(self.oracle, '_readiness_status') else None) or
                (self.oracle.empirica.get_current_session_id() if hasattr(self.oracle.empirica, 'get_current_session_id') else None)
            ) if self.oracle_enabled and self.oracle else None
        }
