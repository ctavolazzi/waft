...
class Biome:
    ...
    def __init__(
        self, biome_id: str, project_path: Path, abiotic_factors: AbioticFactors | None = None
    ):
        """
        Initialize Biome.
    ...
    def create_dish(self, dish_id: str, width: int = 100, height: int = 100) -> PetriDish:
        """
        ...
        if len(self.dishes) >= self.abiotic_factors.max_dishes_per_biome:
            raise ValueError(
                f"Maximum dishes per biome ({self.abiotic_factors.max_dishes_per_biome}) reached"
            )

        dish = PetriDish(dish_id=dish_id, biome_id=self.biome_id, width=width, height=height)

        self.dishes[dish_id] = dish
        return dish

    def get_dish(self, dish_id: str) -> PetriDish | None:
        """Get PetriDish by ID."""
    ...
    def check_membrane_breach(self, organism: BaseAgent, action: dict) -> tuple[bool, str | None]:
        ...
        if action_type == "tool_call":
            ...
            if (
                self.abiotic_factors.allowed_tools
                and tool_name not in self.abiotic_factors.allowed_tools
            ):
