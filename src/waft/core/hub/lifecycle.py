...
class TheSlicer:
    ...
    async def grant_time_slice(self, organism: BaseAgent, dish: PetriDish) -> dict:
        """
        ...
        try:
            ...
            if organism_position:
                ...
                population_density = (
                    dish.get_organism_count() / total_cells if total_cells > 0 else 0.0
                )
            ...
            if spawned_child:
                ...
                self._log_metabolic_action(
                    ...
                    {
                        "action": "child_born",
                        "child_genome": spawned_child.genome_id,
                        "child_name": spawned_child.scientific_name,
                    },
                )
...
class TheReaper:
    ...
    def __init__(self, biome: "Biome", observer: TheObserver):
        ...
        self.observer = observer
        self.death_count = 0
        self.deaths_by_type: dict[str, int] = {"fitness": 0, "boundary": 0}
    ...
    def check_boundary_death(self, organism: BaseAgent, action: dict) -> tuple[bool, str | None]:
        """
    ...
    async def reap(
        self, organism: BaseAgent, dish: PetriDish, death_type: str, reason: str | None = None
    ) -> bool:
        """
        ...
        organism._record_event(event_type=EvolutionaryEventType.DEATH, payload=event.payload)
    ...
    async def reap_boundary_breach(
        self, organism: BaseAgent, dish: PetriDish, action: dict
    ) -> bool:
        """
        ...
        if is_breach:
            await self.reap(organism=organism, dish=dish, death_type="boundary", reason=reason)
            return True
