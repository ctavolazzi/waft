...
from ..utils import _validate_project_name, detect_external_drive, get_external_drive_base
...
class RealmScout(Being):
    ...
    def explore_realm(self) -> dict[str, Any]:
        ...
        return {"findings": findings, "exploration_log": exploration_log}

    def _document_directory_structure(self, path: Path, max_depth: int = 3) -> dict[str, Any]:
        """Document directory structure."""
        structure = {"root": str(path), "directories": [], "files": [], "total_size": 0}
    ...
    def _analyze_files(self, path: Path) -> dict[str, Any]:
        ...
        analysis = {"file_types": {}, "total_files": 0, "total_size": 0, "patterns": []}
...
"""
    ...
    def adversarial_inspection(self, perspective: str = "military") -> dict[str, Any]:
        """
...
class RealmColonizationSystem:
    ...
    def _ensure_colonization_state(self) -> None:
        ...
        if not self.colonized_realms_file.exists():
            ...
            try:
                self.colonized_realms_file.write_text(json.dumps(state, indent=2), encoding="utf-8")
            except (OSError, PermissionError):
                # Log error but don't crash - state file will be created on first use
                pass

    def detect_and_colonize_realm(
        self, drive_name: str = "Easystore", realm_name: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if not drive_path:
                return {"success": False, "error": f"External drive '{drive_name}' not detected"}
            ...
            try:
                ...
                return {"success": False, "error": f"Failed to update colonization state: {e}"}
            ...
            self._cleanup_partial_colonization(created_resources)
            return {"success": False, "error": f"Colonization failed: {e}"}

    def _create_realm_prime_being(self, realm_name: str, realm_path: Path) -> Being:
        ...
        try:
            ...
            prime_being = Being(
                ...
                skills={"realm_governance": 10.0, "tether_management": 10.0, "coordination": 10.0},
            )
            # Save the being
    ...
    def _launch_scouting_mission(
        self, realm_name: str, realm_path: Path, prime_being_id: str
    ) -> dict[str, Any]:
        ...
        if base_path and not self._validate_realm_path(realm_path, base_path):
            raise ValueError(
                f"Invalid realm_path: {realm_path} (path traversal or symlink detected)"
            )
            ...
            if created_resources.get("realm_name"):
                try:
                    ...
                    state["colonized_realms"] = [
                        r
                        for r in state["colonized_realms"]
    ...
    def _report_to_mission_control(
        ...
        try:
            state = json.loads(self.colonized_realms_file.read_text(encoding="utf-8"))
            realm_info = next(
                (r for r in state["colonized_realms"] if r["realm_name"] == realm_name), None
            )
