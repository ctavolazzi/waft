...
class SessionRecord:
    ...
    prompt_signature: str | None = None  # Hash of key prompt characteristics
    approach_category: str | None = None  # e.g., "feature_creation", "debugging", "refactoring"
    iteration_chain: str | None = None  # Links to previous session if part of chain
...
class SessionAnalytics:
    ...
    def analyze_prompt_drift(self, days: int = 30) -> dict[str, Any]:
        """
        ...
        by_prompt = defaultdict(
            lambda: {"count": 0, "avg_files": 0, "avg_lines": 0, "success_rate": 0, "sessions": []}
        )
        ...
        for prompt, data in by_prompt.items():
            ...
            if prompt_sessions:
                data["avg_files"] = sum(
                    s.files_created + s.files_modified for s in prompt_sessions
                ) / len(prompt_sessions)
                data["avg_lines"] = sum(s.net_lines for s in prompt_sessions) / len(prompt_sessions)
                data["success_rate"] = sum(
                    1 for s in prompt_sessions if s.success_indicators
                ) / len(prompt_sessions)
    ...
    def get_iteration_chains(self) -> dict[str, list[str]]:
        ...
        cursor.execute(
            "SELECT iteration_chain, session_id FROM sessions WHERE iteration_chain IS NOT NULL"
        )
        rows = cursor.fetchall()
        conn.close()
    ...
    def compare_approaches(self, category1: str, category2: str, days: int = 30) -> dict[str, Any]:
        """
        ...
        sessions1 = self.get_sessions(
            start_date=start_date, end_date=end_date, category=category1, limit=1000
        )
        sessions2 = self.get_sessions(
            start_date=start_date, end_date=end_date, category=category2, limit=1000
        )
        ...
        def calc_metrics(sessions):
            ...
            return {
                "count": len(sessions),
                "avg_files": sum(s.files_created + s.files_modified for s in sessions)
                / len(sessions),
    ...
    def generate_prompt_signature(self, commands: list[str], category: str | None = None) -> str:
        """
