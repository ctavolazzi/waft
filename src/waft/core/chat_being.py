...
from ..being import Being, BeingState, BeingSystem
...
class ChatBeing:
    ...
    def wake_up(
        self, context: dict[str, Any] | None = None, work_effort_id: str | None = None
    ) -> Being:
        """
        ...
        self.being = self.being_system.spawn_being(
            ...
            initial_skills={"chat_communication": 1.0, "context_awareness": 1.0, "tool_usage": 0.5},
        )
        ...
        if work_effort_id:
            self.being.goals = [{"goal": "Complete current task", "work_effort": work_effort_id}]
        ...
        if context:
            self.being.memories.append(
                {"type": "awakening", "timestamp": datetime.now().isoformat(), "context": context}
            )
    ...
    def wakeup(self) -> Being:
        ...
        is_enlightened = any(
            e.effect_id == "enlightenment" for e in get_active_status_effects(karma_balance)
        )
    ...
    def check_enlightenment_status(self) -> dict[str, Any]:
        ...
        """
        if not self.being:
            return {"awakened": False, "enlightened": False, "error": "Being not yet awakened"}

        karma_balance = self._get_karma_balance()
        previous_karma = self.being.personality.get("karma_balance")

        # Apply/remove karma status effects automatically
        status_changes = apply_status_effects_to_being(self.being, karma_balance, previous_karma)
    ...
    def become_demi_god(self, domain: str | None = None) -> Being:
        """
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        if not self.being:
            return {"awakened": False, "message": "Being not yet awakened. Use /j to awaken."}
    ...
    def _save_being(self) -> None:
        ...
        try:
            self.being_file.write_text(json.dumps(data, indent=2), encoding="utf-8")
...
def get_chat_being(chat_session_id: str, project_path: Path | None = None) -> ChatBeing:
    """
