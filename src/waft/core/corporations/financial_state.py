...
class FinancialState:
    ...
    def update_cash(self, amount: Decimal, description: str = "") -> None:
        ...
        if new_cash < 0:
            raise ValueError(
                f"Insufficient funds: cash would go negative (current: {self.cash}, change: {amount})"
            )
    ...
    def record_expense(self, amount: Decimal, description: str = "") -> None:
        ...
        if self.cash < Decimal(str(amount)):
            raise ValueError(
                f"Insufficient funds: cannot pay expense of {amount} (cash: {self.cash})"
            )
    ...
    def get_burn_rate(self, period_days: int = 30) -> Decimal:
        ...
        recent_expenses = [
            ...
            if t["type"] == "expense"
            and datetime.fromisoformat(t["timestamp"]).timestamp() > cutoff_date
        ]
    ...
    def get_runway_months(self) -> Decimal | None:
        """
