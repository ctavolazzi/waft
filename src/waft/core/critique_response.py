...
from .critique_parser import CritiqueData, CritiqueParser
...
class CritiqueResponseManager:
    ...
    def run_respond_to_critique(
        ...
        if not critique_path or not critique_path.exists():
            self.console.print("[red]❌ Critique file not found[/red]")
        ...
        try:
            ...
            self.console.print(
                f"[green]✅ Parsed {len(critique_data.get_all_criticisms())} criticisms[/green]\n"
            )
        ...
        for i, criticism in enumerate(all_criticisms, 1):
            self.console.print(
                f"[dim]Validating {i}/{len(all_criticisms)}: {criticism.title[:60]}...[/dim]"
            )
        ...
        if not validate_only:
            ...
            for i, (criticism, validation) in enumerate(valid_criticisms, 1):
                ...
                self.console.print(
                    f"[dim]Fixing {i}/{len(valid_criticisms)}: {criticism.title[:60]}...[/dim]"
                )
        ...
        report_path = self._generate_response_report(
            critique_path, critique_data, validation_results, fix_results, dry_run
        )

        self.console.print(
            f"[green]✅ Response report saved: {report_path.relative_to(self.project_path)}[/green]\n"
        )
    ...
    def _find_most_recent_critique(self) -> Path | None:
    ...
    def _generate_response_report(
        ...
        partial_count = sum(
            1 for r in validation_results if r.status == ValidationStatus.PARTIALLY_VALID
        )
        cannot_verify_count = sum(
            1 for r in validation_results if r.status == ValidationStatus.CANNOT_VERIFY
        )
            ...
            for criticism, validation in by_severity["CRITICAL"]:
                ...
                if validation.evidence:
                    ...
                    for evidence in validation.evidence:
                        ...
                        report_lines.append(
                            f"- {symbol} {evidence.description} (confidence: {evidence.confidence:.2f})"
                        )
                    report_lines.append("")
        ...
        # HIGH Issues
        if by_severity["HIGH"]:
            report_lines.extend(["## 🔴 HIGH Issues", ""])
        ...
        if by_severity["MEDIUM"]:
            report_lines.extend(["## ⚠️ MEDIUM Issues", ""])
        ...
        # LOW Issues
        if by_severity["LOW"]:
            report_lines.extend(["## ⚠️ LOW Issues", ""])
        ...
        if invalid_results:
            report_lines.extend(["## Invalid Criticisms (Disproven)", ""])
            ...
            for criticism, validation in invalid_results:
                ...
                for evidence in validation.evidence:
                    if not evidence.supports:
                        report_lines.append(
                            f"- ❌ {evidence.description} (confidence: {evidence.confidence:.2f})"
                        )
        ...
        if cannot_verify_results:
            report_lines.extend(["## Cannot Verify (Manual Review Required)", ""])
        ...
        if all_files_modified:
            report_lines.extend(["## Files Modified", ""])
    ...
    def _display_summary(
        ...
        table.add_row(
            "✅ Valid",
            str(sum(1 for r in validation_results if r.status == ValidationStatus.VALID)),
        )
        table.add_row(
            "❌ Invalid",
            str(sum(1 for r in validation_results if r.status == ValidationStatus.INVALID)),
        )
        table.add_row(
            "⚠️ Partially Valid",
            str(sum(1 for r in validation_results if r.status == ValidationStatus.PARTIALLY_VALID)),
        )
        table.add_row(
            "❓ Cannot Verify",
            str(sum(1 for r in validation_results if r.status == ValidationStatus.CANNOT_VERIFY)),
        )
        table.add_row("", "")
