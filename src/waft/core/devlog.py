...
class DevlogManager:
    ...
    def _detect_category(self, content: str) -> str:
        ...
        # Refactor indicators
        if any(word in content_lower for word in ["refactor", "restructure", "reorganize", "cleanup"]):
        ...
        # Research indicators
        if any(word in content_lower for word in ["research", "investigate", "analyze", "study", "explore"]):
            return self.CATEGORY_RESEARCH
        
        # Default to maintenance
        return self.CATEGORY_MAINTENANCE
    
    def _get_entry_path(self, source: str, category: str, timestamp: datetime) -> Tuple[Path, Path, Path]:
        """
...
"""
    ...
    def _append_to_legacy_devlog(self, title: str, content: str, timestamp: datetime, source: str, category: str):
...
"""
    ...
    def migrate_legacy_devlog(self) -> Dict[str, Any]:
        ...
        for match in matches:
            ...
            if time_match:
                try:
                    timestamp = datetime.strptime(f"{date_str} {time_match.group(1)}", "%Y-%m-%d %H:%M:%S")
                except ValueError:
                    pass
            ...
            category = category_match.group(1) if category_match else self._detect_category(content_text)
