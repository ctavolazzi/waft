...
from .plan_loader import PlanData, PlanLoader
...
class CritiqueAndReviseManager:
    ...
    def run_critique_and_revise(
        ...
        if critique_path and critique_path.exists():
            ...
            self.console.print(
                "[yellow]⚠️  No critique provided. Please run /critique first, or provide critique_path[/yellow]"
            )
            return {"success": False, "error": "No critique provided. Run /critique first."}

        self.console.print(
            f"[green]✅ Found {len(critique_data.get_all_criticisms())} criticisms[/green]\n"
        )
    ...
    def _generate_revision_report(
        ...
        for severity in ["CRITICAL", "HIGH", "MEDIUM", "LOW"]:
            ...
            if revisions:
                report_lines.extend([f"## {severity} Issues (Revised)", ""])
        ...
        if revision_result.sections_added or revision_result.sections_updated:
            report_lines.extend(["## Sections Modified", ""])
        ...
        if revision_result.todos_added:
            report_lines.extend(["## Todos Added", ""])
    ...
    def _display_summary(
        self, revision_result: RevisionResult, report_path: Path, dry_run: bool
    ) -> None:
        ...
        if revision_result.sections_updated:
            self.console.print(
                f"[green]✅ Updated sections: {', '.join(revision_result.sections_updated)}[/green]"
            )
    ...
    def rollback_last_revision(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "plan_path": str(plan_path), "backup_path": str(backup_path)}
        except Exception as e:
