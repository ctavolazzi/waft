...
def create_quest_from_plan(plan_path: Path, project_path: Path | None = None) -> dict[str, Any]:
    """
...
def create_mission_from_plan(plan_path: Path, project_path: Path | None = None) -> dict[str, Any]:
    ...
    success_criteria = [todo.get("content", str(todo)) for todo in plan_data.get("todos", [])]
    ...
    return {"mission": mission.to_dict(), "pdf_path": str(pdf_path)}


def hook_into_plan_creation(plan_path: Path, project_path: Path | None = None) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if plan_type == "mission":
            ...
            return {"type": "mission", "data": result["mission"], "pdf_path": result["pdf_path"]}
        else:
            result = create_quest_from_plan(plan_path, project_path)
            return {"type": "quest", "data": result}
