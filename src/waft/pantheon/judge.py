...
class Judge:
    ...
    def __init__(self, project_path: Optional[Path] = None, magistrate: Optional[Magistrate] = None):
        """
            ...
            try:
                with open(history_file, "r", encoding="utf-8") as f:
                    ...
                    self.judgment_history = [
                        Judgment.from_dict(j, self.magistrate)
    ...
    def evaluate_claim(
        ...
        if supporting_weight > contradicting_weight * 1.5:
            ...
            verdict = "INCONCLUSIVE"
            confidence = abs(supporting_weight - contradicting_weight) / total_weight if total_weight > 0 else 0.0
    ...
    def get_judgment_summary(self) -> Dict[str, Any]:
        ...
        avg_confidence = (
            sum(j.confidence for j in self.judgment_history) / total
            if total > 0 else 0.0
        )
