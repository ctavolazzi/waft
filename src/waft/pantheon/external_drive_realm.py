...
class ExternalDriveRealm:
    ...
    def register_realm(
        ...
        if not drive_path:
            return {
                "success": False,
                "error": f"External drive '{drive_name}' not available"
            }
    ...
    def _register_content_in_manifest(
        ...
        existing = [
            entry for entry in manifest["content"]
    ...
    def get_realm_content(self, realm_name: str) -> List[Dict[str, Any]]:
        ...
        manifest = json.loads(self.manifest_file.read_text(encoding="utf-8"))
        return [
            entry for entry in manifest.get("content", [])
            if entry.get("realm_name") == realm_name
        ]
