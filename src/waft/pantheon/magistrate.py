...
class BodyOfProof:
    ...
    def __init__(self, precedents: list[Precedent] | None = None):
        """
    ...
    def get_by_category(self, category: str, subcategory: str | None = None) -> list[Precedent]:
...
class Magistrate:
    ...
    def _load_precedents(self):
        ...
        if body_of_proof_file.exists():
            try:
                with open(body_of_proof_file, encoding="utf-8") as f:
                    data = json.load(f)
                    self.body_of_proof = BodyOfProof(
    ...
    def organize_case_file(
        ...
        if not category:
            category, subcategory = self._infer_category(case_path, metadata.get("claim"))
    ...
    def organize_all_cases(
        self, auto_categorize: bool = True, default_tags: list[str] | None = None
    ) -> list[Precedent]:
        """
        ...
        for case_file in case_files:
            ...
            try:
                precedent = self.organize_case_file(case_file, tags=default_tags)
    ...
    def get_precedent(self, case_id: str) -> Precedent | None:
    ...
    def get_body_of_proof_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tags": {
                tag: len(precedents) for tag, precedents in self.body_of_proof._index_by_tag.items()
            },
