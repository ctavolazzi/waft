...
def generate_dossier_v2(output_path: Path | None = None) -> Path:
    """
...
def generate_specimen_d_audit_v2(output_path: Path | None = None) -> Path:
    """
...
def generate_asset_labels(output_path: Path | None = None) -> Path:
    """
    ...
    engine.add(
        TextBlock("Cut along dashed lines. Apply to binder sections as needed.", style="Body")
    )
    engine.add(TextBlock(""))  # Spacing
...
def generate_all_artifacts() -> dict:
    ...
    if success_count == total_count:
        print(
            f"✅ Factory Complete: {success_count}/{total_count} artifacts generated successfully"
        )
    else:
