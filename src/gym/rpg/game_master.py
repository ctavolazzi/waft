...
from .scint import RegexScintDetector
...
class GameMaster:
    ...
    def start_encounter(
        self, hero: Hero, quest: Quest, agent_func: Callable[[str], str]
    ) -> BattleLog:
        """
        ...
        try:
            ...
            scints = self.scint_detector.detect_from_exception(
                e, agent_response, quest.difficulty, quest.description
            )
            if scints:
                ...
                for scint in scints:
                    self.console.print(
                        f"  [{scint.scint_type.name}] Severity {scint.severity:.2f}: {scint.evidence[:80]}..."
                    )
                ...
                if stab_success and corrected_output:
                    ...
                    self.console.print(
                        f"[bold green]✨ Scint stabilized on attempt {attempts}[/bold green]"
                    )
                    ...
                    try:
                        ...
                        self.console.print(
                            f"[bold red]⚠️ Error saving stabilized loot: {e}[/bold red]"
                        )
                else:
                    # Stabilization failed
                    self.console.print(
                        f"[bold red]❌ Stabilization failed after {attempts} attempt(s)[/bold red]"
                    )
                    if remaining_scints:
        ...
        if final_scints:
            ...
            if stabilization_successful and original_scints:
                ...
                hero.update_stat(
                    stat_category, success=True, weight=0.1
                )  # Extra boost for recovery
        ...
        if success:
            ...
            # Display result
            self._display_combat_result(
                hero, quest, success, error_message, xp_gained, level_result
            )
        else:
        ...
        final_scint_names = (
            [s.scint_type.name for s in final_scints]
            if final_scints
            else (scints_detected if scints_detected else None)
        )
    ...
    def _display_quest_start(self, hero: Hero, quest: Quest):
        ...
        table.add_row(
            "Stats:",
            f"INT:{hero.stats['INT']:.2f} WIS:{hero.stats['WIS']:.2f} CHA:{hero.stats['CHA']:.2f}",
        )

        panel = Panel(
            table, title="[bold cyan]QUEST START[/bold cyan]", border_style="cyan", padding=(1, 2)
        )
    ...
    def _display_combat_result(
        ...
        if success:
            ...
            if level_result and level_result.get("leveled_up"):
                result_text.append("\n🎉 ", style="bold gold1")
                result_text.append("LEVEL UP! ", style="bold gold1")
