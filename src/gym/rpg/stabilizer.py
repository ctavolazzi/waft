...
from .scint import RegexScintDetector, Scint
...
class StabilizationLoop:
    ...
    def __init__(
        self, max_attempts: int = 1, timeout: float = 30.0, enable_stabilization: bool = True
    ):
    ...
    def stabilize(
    ...
    ) -> tuple[str | None, bool, int, list[Scint]]:
        """
        ...
        while attempts < self.max_attempts:
            attempts += 1
            logger.info(
                f"🌀 Stabilization Attempt {attempts}/{self.max_attempts} for {len(current_scints)} Scints"
            )
    ...
    def _build_stabilization_prompt(
        self, base_context: str, scints: list[Scint], bad_output: str | None = None
    ) -> str:
        """
        Constructs the 'Reflexion' prompt.
        It shows the Agent the 'Fracture' it created and asks for a fix.
        """
        evidence_block = "\n".join([f"- [{s.scint_type.name}] {s.evidence}" for s in scints])

        hints_block = "\n".join([f"- {s.correction_hint}" for s in scints])
