...
from src.waft.core.world.biome import AbioticFactors, Biome
...
class GameOfLifeOrganism(BaseAgent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.preferred_neighbors = random.randint(
            1, 3
        )  # Each organism prefers different social density
    ...
    async def act(self, decision):
        ...
        neighbor_count = len(
            [n for n in dish.get_neighborhood(position, radius=1) if n[2] is not None]
        )
...
async def run_continuous():
    ...
    biome = Biome(biome_id="biome_game_of_life", project_path=project_root, abiotic_factors=abiotic)
    ...
    try:
        ...
        while display.is_running():
            ...
            if pulse % 25 == 0:
                if pop_count > 0:
                    avg_coherence = (
                        sum(org.state.coherence for org in dish.organisms.values()) / pop_count
                    )
                    critical_count = sum(
                        1 for org in dish.organisms.values() if org.state.coherence < 20
                    )
                    print(
                        f"Pulse {pulse}: Pop={pop_count}, AvgCoherence={avg_coherence:.1f}%, Critical={critical_count}"
                    )
                else:
